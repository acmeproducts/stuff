...
new17 = '''## 17. Canonical context menu\nThere is one context-menu component and exactly one command order everywhere it appears:\n\n1. **AI POV**\n2. **Data**\n3. **Print**\n4. **Download Markdown**\n5. **Download CSV**\n6. **Download JSON**\n\nIt appears in Section A on ENVIRONMENT, INDEX, COMPONENT, and applicable EXPLORE analytical state.\n\n**Data** is the exact-state readout for the visible chart. It exposes the full visible-horizon observation rows for every plotted series, including native value, Indexed 100 value when computable, and Pearson correlation versus the currently active series. Correlation uses same-date real visible observations only; it does not interpolate, resample, or fabricate pairings. Insufficient paired observations or zero-variance inputs display `N/A`.\n\nNo other view-specific command is permitted inside this menu. Separate statistics controls must live elsewhere if retained.\n\nEach action binds to the exact visible series set, horizon, representation, evidence revision, and chart state.\n'''
...
new22 = '''## 22. Data / statistics / correlation\nThe canonical **Data** command binds to the exact active analytical state and is presentation-only. Closing it must not mutate series, horizon, axes, representation, active selection, or analysis state.\n\nThe Data surface contains the full visible-horizon series rows with native and Indexed 100 values plus correlation versus the active series. Correlation is Pearson correlation over same-date real plotted observations in the exact visible state. No interpolation or synthetic alignment is permitted.\n\nAdditional statistical tools, if retained, are separate from the canonical `…` menu.\n'''
rep(old22, new22, 'Data and correlation contract')

# Turn marker is intentionally narrow: this is the next UX qualification release,
...
- **A context menu without exact-state Data is rejected.** Data is canonical and exposes full visible-horizon native/index values and same-date correlation versus the active series.
...
for item in [
    ...
    'Active-series emphasis begins at selection, not at crosshair activation',
    '2. **Data**',
    'same-date real visible observations only',
