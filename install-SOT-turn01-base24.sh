...
QUAL_DIR="$ARCHIVE_ROOT/$STAMP-turn01-base24-qualification"
...
touch "$LOG" "$SUMMARY"
exec > >(tee -a "$LOG") 2>&1
record(){ printf '%s\t%s\t%s\n' "$1" "$2" "$3" >> "$SUMMARY"; printf '[%s] %-5s %-44s %s\n' "$(date '+%H:%M:%S')" "$1" "$2" "$3"; }
pass(){ record PASS "$1" "$2"; }
info(){ record INFO "$1" "$2"; }
die(){ record FAIL "$1" "$2"; exit 1; }
cleanup(){ rc=$?; if [ "$CUTOVER" -eq 1 ] && [ "$SUCCESS" -ne 1 ] && [ "$HAD_HTML" -eq 1 ]; then info ROLLBACK 'restoring pre-Base24 HTML'; if install -m0644 "$HTML_BACKUP" "$SOT_DIR/SOT-turn01-base.html"; then pass ROLLBACK_HTML restored; else record FAIL ROLLBACK_HTML failed; fi; fi; if [ "$SUCCESS" -eq 1 ]; then pass FINAL 'all Base-24 mechanical gates passed'; else record FAIL FINAL "qualification failed rc=$rc"; fi; echo '=== QUALIFICATION SUMMARY ==='; awk -F '\t' '{printf "%-5s %-44s %s\n",$1,$2,$3}' "$SUMMARY" || true; echo "persistent log: $LOG"; echo "run directory:  $QUAL_DIR"; rm -rf "$TMP"; }
trap cleanup EXIT
trap 'record FAIL INTERRUPTED "signal received; exit=130"; exit 130' INT TERM

cat > "$QUAL_DIR/RUN-MANIFEST.txt" <<EOF
run_id=turn01-base24-$STAMP
...
base24_owner_gate_integrator=083aa1334208b1e6995fa18852e82722a815f331/integrate-SOT-turn01-base24-ui.py
...
echo '=== TURN 01 BASE-24 OWNER-GATE QUALIFICATION ==='
...
if curl --max-time 30 -fsSL "$BASE/603e8a331b13b72a097e9ebb9640e33707279777/integrate-SOT-turn01-base22-ui.py" -o "$TMP/base22-ui.py"; then pass FETCH_CLEAN_INTEGRATOR "bytes=$(stat -c %s "$TMP/base22-ui.py")"; else die FETCH_CLEAN_INTEGRATOR failed; fi
if curl --max-time 30 -fsSL "$BASE/083aa1334208b1e6995fa18852e82722a815f331/integrate-SOT-turn01-base24-ui.py" -o "$TMP/base24-ui.py"; then pass FETCH_BASE24_INTEGRATOR "bytes=$(stat -c %s "$TMP/base24-ui.py")"; else die FETCH_BASE24_INTEGRATOR failed; fi
python3 -m py_compile "$TMP/base22-ui.py" "$TMP/base24-ui.py" >/dev/null 2>&1 || die PYCOMPILE failed
pass PYCOMPILE ok
python3 "$TMP/base22-ui.py" "$TMP/pre.html" "$TMP/clean-intermediate.html" || die GENERATE_CLEAN_INTERMEDIATE failed
pass GENERATE_CLEAN_INTERMEDIATE "sha256=$(sha256sum "$TMP/clean-intermediate.html"|awk '{print $1}')"
python3 "$TMP/base24-ui.py" "$TMP/clean-intermediate.html" "$TMP/SOT-turn01-base.html" || die GENERATE_BASE24_UI failed
pass GENERATE_BASE24_UI "sha256=$(sha256sum "$TMP/SOT-turn01-base.html"|awk '{print $1}')"
...
# Closed branch must contain duplicate cards and restart action before the active branch.
...
# Live backend/storage authority remains Base-22 and is checked without interrupting active fingerprint work.
...
ARCH="$ARCHIVE_ROOT/$STAMP-turn01-base22-before-base24"
mkdir -p "$ARCH"
...
grep -Fq 'TURN01_BASE24_OWNER_GATE' "$QUAL_DIR/public.html" || die PUBLIC_PAGE_MARKER missing
pass PUBLIC_PAGE_MARKER Base24
for marker in '2-copy groups' '3-copy groups' '4+ copy groups' 'Current Plan' 'Previous / Stale Plan' 'availableFolderSearch' 'selectorCommit'; do grep -Fq "$marker" "$QUAL_DIR/public.html" || die PUBLIC_PROTECTED_MARKER "missing $marker"; done
...
SUCCESS=1
pass QUALIFICATION 'Base-24 mechanically qualified'
echo '=== TURN 01 BASE-24 MECHANICALLY QUALIFIED ==='
