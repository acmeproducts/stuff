...
**P3 — No transient action debt.** A completed user action is durable and does not depend on the originating room remaining open. A saved phrase remains saved regardless of what happens to the room it came from. This does **not** prohibit durable, explicitly visible delivery/sync state: offline messages, phrasebook writes, and receipts may remain pending until their external destination acknowledges them. Such work is persisted under its originating room/pair, survives reload, retries without duplication, and is never silently discarded or applied to a later room. “Stateless” elsewhere in this document means no orphaned in-memory state that must be unwound manually; it does not override the durability and retry requirements in P1, P5, §11, or Part Four.
...
**Process status.** This rollback rule and the T13 diff-variance guardrail remain active requirements for the recovery build. They are not historical commentary. A future process change must amend this specification explicitly; silence in a build plan or implementation prompt does not supersede them.
...
### 17.1 Readiness and remaining decisions

This specification is a coherent **requirements and recovery baseline**, but it is intentionally not yet an executable build plan. Part Four supplies the architecture contract—single ownership, captured room/session/call generations, stale-result rejection, idempotent teardown, and durable room/pair-scoped retry state. It does not yet assign those responsibilities to a final module map or sequence the implementation. Saying that the audit found races “but no architecture” is therefore inaccurate; saying that the implementation architecture and build plan remain to be written is accurate.

Before implementation begins, the build plan must:

1. assign every asynchronous resource and durable store to one runtime owner;
2. map each finding in §18.3 to the invariant and T14 gate that closes it;
3. order the work so the lifecycle foundation is proven before unfinished UI in matrix column 4 is added; and
4. name the single output HTML artifact.

**Open naming decision.** The output artifact filename has not been selected in the recorded product decisions. `bridge-turn09-ship.html` is the behavioral reference and must not be overwritten. The filename is therefore a real open build-plan decision—not an internal requirements contradiction—and no builder may invent it or infer it from the next turn number.
