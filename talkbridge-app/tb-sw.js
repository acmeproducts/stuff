...
/* ── E7 (27·base, folder release) — INSTALLABILITY FETCH HANDLER ──────────
   Chrome's install criteria require a registered worker WITH a fetch
   handler; the accepted worker has none, which is why start_url alone
   never produced an install offer (proven by /stuff/tb-skeleton/,
   2026-09-12). Shape copied verbatim from the skeleton that installs on
   desktop Chrome, Android and iPhone: GET only, network first, cache and
   then the app page as fallback. Purely additive — every push,
   notificationclick, message and lifecycle handler above is untouched, and
   the root tb-sw.js is not modified by this release. */
const TB_SHELL = 'tb-app-shell-v1';
const TB_SHELL_ASSETS = ['./', './index.html', './tb-manifest.webmanifest', './icon-192.png', './icon-512.png'];

self.addEventListener('install', (e) => {
  e.waitUntil(caches.open(TB_SHELL).then((c) => c.addAll(TB_SHELL_ASSETS)).catch(() => {}));
});
