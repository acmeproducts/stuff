...
BASE=https://raw.githubusercontent.com/acmeproducts/stuff/681df7332ea95b123f16150c8c0c098cd66ef71a
PUBLIC_URL='https://oc-ref.fell-dojo.ts.net/report/SOT/sot-turn01-r2-wizard.html?v=20260823-project-ui-2'
EXPECTED_BUILD=2026.08.23.sot-project-ui-2
TEMP_ROOT="$(mktemp -d)"
CUTOVER_STARTED=0
SERVICE_STOPPED=0
...
echo '=== FETCH PROJECT UI CANDIDATE ==='
FILES=(
  sot-api.js
  sot-worker.js
  sot-db-manage.js
...
markers=['2026.08.23.sot-project-ui-2','2026.08.23.sot-clean-admin-1']
...
echo '=== VERSION 1 TO VERSION 2 MIGRATION GATE ==='
mkdir -p "$TEMP_ROOT/v1-migrations"
cp "$TEMP_ROOT/sot-db/migrations/001-initial.sql" "$TEMP_ROOT/v1-migrations/"
SOT_MIGRATIONS_DIR="$TEMP_ROOT/v1-migrations" \
SOT_SQLITE_ADAPTER="$TEMP_ROOT/sot-sqlite.py" \
node "$TEMP_ROOT/sot-db-manage.js" create "$TEMP_ROOT/v1.sqlite" >/dev/null
SOT_MIGRATIONS_DIR="$TEMP_ROOT/sot-db/migrations" \
SOT_SQLITE_ADAPTER="$TEMP_ROOT/sot-sqlite.py" \
node "$TEMP_ROOT/sot-db-manage.js" migrate "$TEMP_ROOT/v1.sqlite" > "$TEMP_ROOT/upgrade.json"
python3 - "$TEMP_ROOT/upgrade.json" <<'PY'
import json,sys
x=json.load(open(sys.argv[1]))['status']
if not x.get('ok') or x.get('integrity')!='ok' or x.get('current_version')!=2:
    raise SystemExit('v1 to v2 migration gate failed: '+repr(x))
print('migration: 1 -> 2; integrity: ok')
PY
...
echo '=== LIVE HEALTH + PROJECT LIST GATES ==='
...
if x.get('status')!='ok' or x.get('build')!=sys.argv[2] or x.get('database_version')!=2:
...
if not isinstance(x.get('projects'),list): raise SystemExit('project list response invalid')
if elapsed>=1.0: raise SystemExit(f'project list too slow: {elapsed:.3f}s')
print(f"projects: {len(x['projects'])}; list: {elapsed:.3f}s")
PY
...
if [m.get('version') for m in x.get('migrations',[])] != [1,2]: raise SystemExit('wrong live migration set')
print('database:',x.get('database_path'))
print('integrity:',x['integrity']['result'])
print('migrations: 001, 002')
PY

echo '=== PUBLIC PROJECT UI GATE ==='
for attempt in {1..10}; do
  if curl --max-time 15 -fsS "$PUBLIC_URL" -o "$TEMP_ROOT/public.html" && \
     grep -q '2026.08.23.sot-project-ui-2' "$TEMP_ROOT/public.html"; then
    break
  fi
  sleep 2
done
grep -q '2026.08.23.sot-project-ui-2' "$TEMP_ROOT/public.html"
grep -q 'Folder Count' "$TEMP_ROOT/public.html"
grep -q 'Add entire volume' "$TEMP_ROOT/public.html"

CUTOVER_STARTED=0
echo '=== SOT PROJECT UI 2 INSTALLED ==='
echo 'Existing SOT projects and corpus rows were preserved.'
