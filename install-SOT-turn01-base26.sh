...
QUAL_DIR="$ARCHIVE_ROOT/$STAMP-turn01-base26-qualification"
...
CUTOVER=0
SUCCESS=0
PROBE_NAME="__base26_probe_${STAMP}.html"
...
cleanup(){ rc=$?; rm -f "$PROBE_PATH" 2>/dev/null || true; if [ "$CUTOVER" -eq 1 ] && [ "$SUCCESS" -ne 1 ] && [ "$HAD_HTML" -eq 1 ]; then info ROLLBACK 'restoring pre-Base26 HTML'; if install -m0644 "$HTML_BACKUP" "$SOT_DIR/SOT-turn01-base.html"; then pass ROLLBACK_HTML restored; else record FAIL ROLLBACK_HTML failed; fi; fi; if [ "$SUCCESS" -eq 1 ]; then pass FINAL 'all Base-26 mechanical gates passed'; else record FAIL FINAL "qualification failed rc=$rc"; fi; echo '=== QUALIFICATION SUMMARY ==='; awk -F '\t' '{printf "%-5s %-48s %s\n",$1,$2,$3}' "$SUMMARY" || true; echo "persistent log: $LOG"; echo "run directory:  $QUAL_DIR"; rm -rf "$TMP"; }
trap cleanup EXIT
trap 'record FAIL INTERRUPTED "signal received; exit=130"; exit 130' INT TERM

cat > "$QUAL_DIR/RUN-MANIFEST.txt" <<EOF
run_id=turn01-base26-$STAMP
...
base26_ai_boot_integrator=a531cc7aa60b52224b5f9a8cfa22fc3e3afcbaad/integrate-SOT-turn01-base26-ai.py
...
echo '=== TURN 01 BASE-26 RELEASE-QUALITY QUALIFICATION ==='
...
curl --max-time 30 -fsSL "$BASE/a531cc7aa60b52224b5f9a8cfa22fc3e3afcbaad/integrate-SOT-turn01-base26-ai.py" -o "$TMP/base26-ai.py" || die FETCH_BASE26_INTEGRATOR failed
pass FETCH_BASE26_INTEGRATOR "bytes=$(stat -c %s "$TMP/base26-ai.py")"
python3 -m py_compile "$TMP/base22-ui.py" "$TMP/base24-ui.py" "$TMP/base26-ai.py" >/dev/null 2>&1 || die PYCOMPILE failed
pass PYCOMPILE ok
...
python3 "$TMP/base26-ai.py" "$TMP/clean-base24-behavior.html" "$TMP/SOT-turn01-base.html" || die GENERATE_BASE26_UI failed
CAND_SHA="$(sha256sum "$TMP/SOT-turn01-base.html"|awk '{print $1}')"
pass GENERATE_BASE26_UI "sha256=$CAND_SHA"
printf 'candidate_sha256=%s\n' "$CAND_SHA" >> "$QUAL_DIR/RUN-MANIFEST.txt"
pass GENERATED_ARTIFACT_IDENTITY "$CAND_SHA"

# Static behavioral contract checks on the exact generated HTML.
...
'AI_SUPERVISOR_FIRST_SYSTEM_MESSAGE':"var messages=[{role:'system',content:SOT_SUPERVISOR_PROMPT},{role:'system',content:'SOT PROJECT / EVIDENCE CONTEXT",
...
'AI_VALIDATE_UI':'Validate & activate Venice',
'AI_BOOT_GUARD':'base26BootGuard',
'AI_BOOT_SENTINEL':"data-sot-boot','base26-ok'",
}
...
for bad in ['venice_api_key','openrouter_api_key','provider_api_key','ai_api_key','?.','...cfg','...history']:
    if bad in h: raise SystemExit('prohibited generated token '+bad)
PY
...
import re,sys
h=Path(sys.argv[1]).read_text()
out=Path(sys.argv[2]); combined=Path(sys.argv[3])
scripts=[]
...
parse_scripts(){
  local html="$1" tag="$2"
  local dir="$TMP/scripts-$tag" combined="$TMP/combined-$tag.js" count f
...
run_browser(){
  ...
  grep -Fq 'data-sot-boot="base26-ok"' "$dump" || { echo 'boot sentinel missing'; tail -80 "$err" || true; return 1; }
...
# Pre-cutover browser boot on the exact candidate bytes, temporarily served by the normal report server.
...
# Live backend/storage authority remains unchanged and fingerprint work is not interrupted.
...
ARCH="$ARCHIVE_ROOT/$STAMP-turn01-base25-before-base26"
mkdir -p "$ARCH"
...
SUCCESS=1
pass QUALIFICATION 'Base-26 mechanically qualified only after exact generated/public parse + browser boot'
echo '=== TURN 01 BASE-26 MECHANICALLY QUALIFIED ==='
