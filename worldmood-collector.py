...
USER_AGENT="WorldMoodCollector/1.4 (+https://github.com/acmeproducts/stuff)"
...
def fetch_json(url):
    t=time.monotonic();req=urllib.request.Request(url,headers={"User-Agent":USER_AGENT,"Accept":"application/json"})
...
def source_url(minutes):
    return BASE+"?"+urllib.parse.urlencode({"QUERY":"","OUTPUTFIELDS":"url,name,domain,tone,lang","OUTPUTTYPE":"1","MAXROWS":str(MAXROWS),"TIMESPAN":str(minutes)})
def previous():
...
def main():
    ...
    for minutes in WINDOWS:
        ...
        try:
            payload,http=fetch_json(source_url(minutes));st.update(http);features=payload.get("features") if isinstance(payload,dict) else None
