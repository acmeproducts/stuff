...
new="function freshnessNote(id,series){let h=health(id),cad=String(h.cadence||cat(id).native_cadence||series?.cadence||''),last=(series?.observations||[]).at(-1),latest=h.actualLatestCanonicalObservation||h.latestObservation||(last?new Date(last.t).toISOString().slice(0,10):''),anchor=S.derived.commonMarketAnchor;if(!latest||!cad||latest===anchor)return'';if(h.classification==='current')return`${name(id)} ${cad} · latest ${latest} · current for release cadence`;return`${name(id)} ${cad} · latest ${latest} · ${h.classification||'release cadence'}`}"
