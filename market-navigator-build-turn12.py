...
# Attachment picker accepts images and spreadsheets. XLSX is loaded lazily only when needed so app boot never depends on it.
...
for must in ['TURN 12 PRE-SHIP','Attach image or spreadsheet','analysisBody{min-height:0;display:grid;grid-template-columns:minmax(0,1fr);gap:0;padding:0','id="analysisEvidence" class="hidden"','Register this provider before starting an Analysis','Attached spreadsheet/file','indexedDB.open','plugins=[{id:\'web\'','loadXLSX()']:
    if must not in s: raise SystemExit('missing Turn12 requirement: '+must)
