...
try{
  ...
  const issueUrl=new URL(opened),issueTitle=issueUrl.searchParams.get('title')||'',issueBody=issueUrl.searchParams.get('body')||'';
  assert.equal(issueTitle,'[Market Navigator Source] Dow');assert(issueBody.includes('Query: Dow'));assert(issueBody.includes('Class: index'));
