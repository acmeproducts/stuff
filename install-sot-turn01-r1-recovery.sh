...
for f in sot-backend-turn01-prebase-addon.js sot-backend-turn01-r1-bridge-addon.js sot-ui-turn01-r1-recovery-addon.js; do curl -fsSL "$BASE/$f" -o "$TMP/$f"; done
...
if '2026.08.20.6.9.1-wsl-path-centric-analysis' not in s: raise SystemExit('verified 6.9.1 marker missing')
...
STAMP="$(date +%Y%m%d-%H%M%S)"; API_BAK="$API.before-turn01-r1-$STAMP"; OUT_BAK=""
cp -a "$API" "$API_BAK"
if [[ -f "$OUT_UI" ]]; then OUT_BAK="$OUT_UI.before-turn01-r1-$STAMP"; cp -a "$OUT_UI" "$OUT_BAK"; fi
rollback(){ echo '=== TURN01 R1 GATE FAILED — RESTORING PRE-R1 LIVE STATE ===' >&2; cp -a "$API_BAK" "$API" || true; if [[ -n "$OUT_BAK" && -f "$OUT_BAK" ]]; then cp -a "$OUT_BAK" "$OUT_UI" || true; else rm -f "$OUT_UI" || true; fi; sudo systemctl restart openclaw-report-server.service || true; exit 1; }
trap rollback ERR
cp -a "$TMP/sot-api.candidate.js" "$API"; cp -a "$TMP/turn01-r1.html" "$OUT_UI"; sudo systemctl restart openclaw-report-server.service
for i in {1..20}; do sleep 1; if curl --max-time 3 -fsS http://127.0.0.1:18080/api/sot/health -o "$TMP/health.json" 2>/dev/null; then break; fi; done
...
if not need.issubset(set(x.get('capabilities') or [])): raise SystemExit('capability gate failed')
print(json.dumps(x,indent=2))
PY

echo '=== EXISTING CORPUS EVIDENCE — NONBLOCKING GATE ==='
# Do NOT bulk-rewrite legacy manifests during install. R1 reads them directly as authoritative fallback evidence.
curl --max-time 10 -fsS http://127.0.0.1:18080/api/sot/turn01/r1/evidence-status -o "$TMP/status.json"
curl --max-time 10 -fsS http://127.0.0.1:18080/api/sot/turn01/intelligence -o "$TMP/global-intel.json"
python3 - "$TMP/status.json" "$TMP/global-intel.json" <<'PY'
import json,sys
s,i=[json.load(open(x)) for x in sys.argv[1:]]
print('corpus:',{k:s.get(k) for k in ['projects','sources','manifests','inventory','observations']})
print('effective intelligence:',{k:i.get(k) for k in ['observations','unique_content','bytes','duplicate_bytes']})
if s.get('projects',0)<1: raise SystemExit('FAIL: no projects visible')
if s.get('sources',0)<1: raise SystemExit('FAIL: no sources visible')
if s.get('manifests',0)>0 and i.get('observations',0)<1: raise SystemExit('FAIL: legacy manifests exist but intelligence exposes zero effective observations')
PY
...
import json,sys
ps=json.load(open(sys.argv[1])).get('projects') or []; print(ps[0]['project_token'] if ps else '')
PY
TOKEN="$(cat "$TMP/token.txt")"; [[ -n "$TOKEN" ]] || { echo 'FAIL: no real project token available' >&2; false; }
Q="$(python3 -c 'import urllib.parse,sys;print(urllib.parse.quote(sys.argv[1],safe=""))' "$TOKEN")"
curl --max-time 10 -fsS "http://127.0.0.1:18080/api/sot/turn01/projects/$Q/sources" -o "$TMP/sources.json"
curl --max-time 10 -fsS "http://127.0.0.1:18080/api/sot/turn01/projects/$Q/intelligence" -o "$TMP/intel.json"
curl --max-time 10 -fsS "http://127.0.0.1:18080/api/sot/turn01/projects/$Q/plan" -o "$TMP/plan.json"
...
if 'totals' not in p or 'items' not in p: raise SystemExit('FAIL: plan route invalid')
...
grep -q 'TURN01 R1' "$TMP/public.html"; grep -q 'SOT Project' "$TMP/public.html"

echo '=== TURN01 R1 RECOVERY INSTALLED ==='
echo "Backend: $BUILD"
echo 'Evidence model: existing manifests read directly; no blocking migration during install.'
