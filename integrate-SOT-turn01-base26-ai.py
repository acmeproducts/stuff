...
if len(sys.argv)!=3:
    raise SystemExit('usage: integrate-SOT-turn01-base26-ai.py <clean-base24-behavior.html> <base26.html>')
...
for marker in ['TURN01_BASE24_OWNER_GATE','function openConfig(){','function renderInsights(p){','2-copy groups','3-copy groups','4+ copy groups','availableFolderSearch','selectorCommit','Current Plan','Previous / Stale Plan']:
    if marker not in src:
        raise SystemExit('Base26 clean input contract failed: '+marker)
...
# Install a tiny independent boot guard before the application script. If the main
# script parses but throws during initialization, the browser DOM records it.
boot_guard=r'''<script id="base26BootGuard">
(function(){
...
async function aiProjectContext(p){var detail={},review={},plan={},dups={};var parts=await Promise.all([api(`/turn01/projects/${encodeURIComponent(p.project_token)}`).catch(function(){return{}}),api(`/turn01/projects/${encodeURIComponent(p.project_token)}/review`).catch(function(){return{}}),api(`/turn01/projects/${encodeURIComponent(p.project_token)}/plan`).catch(function(){return{}}),api(`/turn01/projects/${encodeURIComponent(p.project_token)}/duplicates`).catch(function(){return{}})]);detail=parts[0]||{};review=parts[1]||{};plan=parts[2]||{};dups=parts[3]||{};var sources=(detail.sources||[]).map(function(x){return x.normalized_path});return{project:{project_token:p.project_token,project_name:p.project_name,evidence_revision:p.evidence_revision,processing_state:p.processing_state||p.status||null,sources:sources},evidence:{files:review.files||0,bytes:review.bytes||0,duplicate_groups:review.duplicate_groups||0,warnings:review.warning_count||0,errors:review.error_count||0},duplicates:dups.counts||{},plan:{state:plan.state||'none',evidence_revision:plan.evidence_revision==null?null:plan.evidence_revision,item_count:(plan.items||[]).length}}}
async function aiInfer(p,user){var active=aiActive();if(!active)throw new Error('No validated AI provider is active. Open Configuration and validate Venice or OpenRouter first.');var ctx=await aiProjectContext(p),history=aiHistory[p.project_token]||[];var messages=[{role:'system',content:SOT_SUPERVISOR_PROMPT},{role:'system',content:'SOT PROJECT / EVIDENCE CONTEXT (authoritative supplied data):\\n'+JSON.stringify(ctx,null,2)}].concat(history).concat([{role:'user',content:user}]);var url=active.provider==='venice'?'https://api.venice.ai/api/v1/chat/completions':'https://openrouter.ai/api/v1/chat/completions';var r=await fetch(url,{method:'POST',cache:'no-store',headers:{'Content-Type':'application/json',Authorization:'Bearer '+active.key},body:JSON.stringify({model:active.model,max_tokens:3000,messages:messages})}),j={};try{j=await r.json()}catch(_){}if(!r.ok)throw new Error((j.error&&(j.error.message||j.error))||(active.provider+' '+r.status));var out=j&&j.choices&&j.choices[0]&&j.choices[0].message&&j.choices[0].message.content;if(!out)throw new Error('Provider returned no answer');history.push({role:'user',content:user},{role:'assistant',content:out});aiHistory[p.project_token]=history.slice(-12);return{out:out,active:active,ctx:ctx}}
'''
...
# Replace Base-24 polling tail so successful initial load writes a browser-visible boot sentinel.
...
new="load().then(function(){document.documentElement.setAttribute('data-sot-boot','base26-ok')}).catch(function(e){document.documentElement.setAttribute('data-sot-boot-error','initial-load: '+String(e&&e.message?e.message:e))});setInterval(async()=>{try{let [p,r]=await Promise.all([api('/projects'),api('/rollup')]);state.projects=Array.isArray(p)?p:(p.projects||[]);state.rollup=r;renderCards();let psel=selectedProject();if(psel&&state.tab==='index'&&['Queued','WIP','Paused'].includes(projectState(psel)))renderIndex(psel,true)}catch{}},3000);"
if old not in src: raise SystemExit('Base26 polling/bootstrap tail not found')
src=src.replace(old,new,1)

required=['TURN01_BASE26_OPERATIONAL_AI','base26BootGuard','data-sot-boot','SOT_SUPERVISOR_PROMPT','aiLoadModels','aiValidate','aiSaveValidated','aiActive','aiProjectContext','aiInfer','Validate & activate Venice','Validate & activate OpenRouter',"var messages=[{role:'system',content:SOT_SUPERVISOR_PROMPT},{role:'system',content:'SOT PROJECT / EVIDENCE CONTEXT",'No validated AI provider is active.','https://api.venice.ai/api/v1/models','https://openrouter.ai/api/v1/auth/key']
for m in required:
    if m not in src: raise SystemExit('Base26 AI/boot marker missing '+m)
for bad in ['?.','...cfg','...history']:
    if bad in src: raise SystemExit('Base26 compatibility syntax prohibited: '+bad)
Path(sys.argv[2]).write_text(src)
print('Base-26 AI + browser boot instrumentation integrated from clean Base-24 behavior')
