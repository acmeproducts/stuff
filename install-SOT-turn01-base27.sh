...
QUAL_DIR="$ARCHIVE_ROOT/$STAMP-turn01-base27-qualification"
...
CUTOVER=0
SUCCESS=0
PROBE_NAME="__base27_probe_${STAMP}.html"
...
cleanup(){ rc=$?; rm -f "$PROBE_PATH" 2>/dev/null || true; if [ "$CUTOVER" -eq 1 ] && [ "$SUCCESS" -ne 1 ] && [ "$HAD_HTML" -eq 1 ]; then info ROLLBACK 'restoring pre-Base27 HTML'; if install -m0644 "$HTML_BACKUP" "$SOT_DIR/SOT-turn01-base.html"; then pass ROLLBACK_HTML restored; else record FAIL ROLLBACK_HTML failed; fi; fi; if [ "$SUCCESS" -eq 1 ]; then pass FINAL 'all Base-27 mechanical gates passed'; else record FAIL FINAL "qualification failed rc=$rc"; fi; echo '=== QUALIFICATION SUMMARY ==='; awk -F '\t' '{printf "%-5s %-48s %s\n",$1,$2,$3}' "$SUMMARY" || true; echo "persistent log: $LOG"; echo "run directory:  $QUAL_DIR"; rm -rf "$TMP"; }
trap cleanup EXIT
trap 'record FAIL INTERRUPTED "signal received; exit=130"; exit 130' INT TERM

cat > "$QUAL_DIR/RUN-MANIFEST.txt" <<EOF
run_id=turn01-base27-$STAMP
...
base27_ai_boot_integrator=56dd189287cd32066b2fac82907ac8846d8f42c9/integrate-SOT-turn01-base27-ai.py
...
echo '=== TURN 01 BASE-27 RELEASE-QUALITY QUALIFICATION ==='
...
curl --max-time 30 -fsSL "$BASE/56dd189287cd32066b2fac82907ac8846d8f42c9/integrate-SOT-turn01-base27-ai.py" -o "$TMP/base27-ai.py" || die FETCH_BASE27_INTEGRATOR failed
pass FETCH_BASE27_INTEGRATOR "bytes=$(stat -c %s "$TMP/base27-ai.py")"
python3 -m py_compile "$TMP/base22-ui.py" "$TMP/base24-ui.py" "$TMP/base27-ai.py" >/dev/null 2>&1 || die PYCOMPILE failed
pass PYCOMPILE ok
...
python3 "$TMP/base27-ai.py" "$TMP/clean-base24-behavior.html" "$TMP/SOT-turn01-base.html" || die GENERATE_BASE27_UI failed
CAND_SHA="$(sha256sum "$TMP/SOT-turn01-base.html"|awk '{print $1}')"
pass GENERATE_BASE27_UI "sha256=$CAND_SHA"
...
'AI_BOOT_GUARD':'base27BootGuard',
'AI_BOOT_SENTINEL':"data-sot-boot','base27-ok'",
}
...
for bad in ['venice_api_key','openrouter_api_key','provider_api_key','ai_api_key']:
    if bad in h: raise SystemExit('AI_KEYS_BROWSER_LOCAL_ONLY prohibited field '+bad)
PY
...
run_browser(){
  ...
  grep -Fq 'data-sot-boot="base27-ok"' "$dump" || { echo 'boot sentinel missing'; tail -80 "$err" || true; return 1; }
...
ARCH="$ARCHIVE_ROOT/$STAMP-turn01-pre-base27-cutover"
mkdir -p "$ARCH"
...
SUCCESS=1
pass QUALIFICATION 'Base-27 mechanically qualified after exact generated/public parse + browser boot'
echo '=== TURN 01 BASE-27 MECHANICALLY QUALIFIED ==='
