...
s=plan.read_text()
s=s.replace('Next release scope: Turn 20','Next release scope: Turn 21',1)

old='- tapping any enabled **legend chip** drills directly to a standalone analytical chart for that exact series, including tapping the INDEX chip itself;'
new='- tapping any enabled **raw component legend chip** drills to COMPONENT with that raw/source series as the leaf/root while retaining the current derived INDEX as plotted context;\n- tapping the current INDEX own legend chip selects/references that INDEX in place and never creates a redundant COMPONENT level;'
if old in s:
    s=s.replace(old,new,1)
elif 'raw component legend chip' not in s:
    raise SystemExit('Turn 21 INDEX navigation anchor missing')
...
if old in s:
    s=s.replace(old,new,1)
elif '`ENV / RSK / RSK`' not in s:
    raise SystemExit('Turn 21 COMPONENT hierarchy anchor missing')
...
if old in s:
    s=s.replace(old,new,1)
elif 'SPY Indexed 100 must also rise' not in s:
    raise SystemExit('Turn 21 Indexed 100 anchor missing')
...
if '## Turn 21 source-index / hierarchy regressions — rejected' not in t:
    t=t.rstrip()+section_g+'\n'
g.write_text(t)

p=Path('data/market-backend/derived-index-definition.json')
