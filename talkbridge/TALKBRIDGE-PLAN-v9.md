<!-- TALKBRIDGE-PLAN v21.35.0 -->
# TALKBRIDGE MASTER PLAN v21.35.0
...
| 27·ship | **Video call surface, rescoped** — corner-band PiP removed; tap-swap big/small (KEPT, proven correct in candidate 3); camera-flip icon moved onto the big video surface (was wrongly in the top ribbon); small video draggable anywhere including near/off-screen with an edge handle. Back-button reduced view (V2) DROPPED entirely — back falls through to plain default navigation, nothing call-specific wired to popstate. Screen share still dropped. | Spec §7.6, rescoped 2026-09-14 (V2 removed, V4 relocated, V3 extended) | **REJECTED candidate 3 → buried G54 (icon placement, V2 unreliable). Address rolled back. Rebuilding to corrected scope on owner order (rebuild, not patch).** | https://acmeproducts.github.io/stuff/bridge-turn27-ship.html (accepted pre-ship bytes) |
