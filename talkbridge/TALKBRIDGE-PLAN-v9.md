<!-- TALKBRIDGE-PLAN v21.57.0 -->
# TALKBRIDGE MASTER PLAN v21.57.0
...
**0c-1 · FLATTENING EXCEPTION — IN FORCE. Approved by the owner 2026-09-21,
exact words: “approving the §0c-1 rule exception.”** A stage the ledger names a FLATTENING stage
...
Everything else in 0c stands. Approval: “approving the §0c-1 rule exception.” — owner, 2026-09-21.
...
| D-10 | **Phrasebook: Enter in a card's tag field moves focus to the NEXT card's source field instead of adding the tag and staying in the tag field** (owner, 2026-09-21). **Not a 27·post-ship regression — proven by A/B on the same phone: identical at the accepted c8 address (`bridge-turn27-ship.html`).** Pre-existing. What is known: the handler (`body` keydown → `pbAddTagTo` → `pbTouch` → `pbRerenderCard` → refocus the new tag input, G9) touches none of the 27·post-ship parts; the same scripted action in the jsdom rig on BOTH builds adds the tag and leaves focus in the tag field. So the cause is on-device only: either the keyboard's Enter does not arrive as `key === 'Enter'` on keydown (the handler then never runs and the keyboard's own “next field” action moves focus), or focus is lost when the card is redrawn (the focused input is removed from the DOM by `replaceWith`) and the phone lands it elsewhere. The app logs nothing on this path and does not log uncaught errors, so no device log can decide it. Which phone(s) is not yet recorded. | **CLOSED 2026-09-21 (owner: “passed” on device).** Was: PATCHED in 27·post-ship candidate 2 (owner 2026-09-21: “I went back far enough to determine it’s been a bug for such a long time I can’t find a working version of the modern PB era so just patch it” — no instrument, no known-good). The patch covers both possible causes at once: the keyboard’s Enter not arriving as a key (form submit + keyCode + keyup ears, `enterkeyhint=enter`) and focus lost on the redraw (put back at 0/60 ms). Rig proves all four paths and the focus restore; the phone decides at the device gate. |
| D-11 | **`relayConnect` with no active room still hooks the previous socket.** Found while flattening (28·base): when `activeRoom()` is empty the base body builds nothing, but the three “after” layers (T-net close log, CR3 open hook, V2 ramp) still ran on whatever `_relayWs` held — a second close listener on a stale socket, `_cr3Room` overwritten with null. Reproduced exactly in FL-1 (§0c-1 b) and covered by the differential script. Harmless in practice (the socket is closed or replaced), untidy in the log. | OPEN — fix as its own release after the flattening turn; one line, once the relay path has a single owner. |
...
**v21.57.0 · 2026-09-21.** 28·base BUILT — flattening cluster 1, the relay path. 27 layers out, one flat part in, proven identical by the differential harness on every axis; mutations 19/19. `log` deferred to cluster 5 (its middle layer is the device-log uploader). D-11 recorded (no-room connect re-hooks the old socket), reproduced not fixed.

**v21.56.0 · 2026-09-21.** §0c-1 flattening exception APPROVED by the owner (“approving the §0c-1 rule exception.”). 28·base (flatten the relay path) unblocked and building; 28·pre-ship (flatten the call) queued behind it.

**v21.55.0 · 2026-09-21.** 27·post-ship candidate 2 ACCEPTED by the owner (“passed”). D-10 closed. Technical debt & concurrency closed out. 28·pre-base banked as a byte-identical copy. Next on the chain: 28·base (flatten the relay path), blocked on the owner’s written approval of §0c-1; then 28·pre-ship (flatten the call), then multi-user at turn 29.

**v21.54.0 · 2026-09-21.** 27·post-ship candidate 2 = c1 + D-10 patch, at the owner’s order (“just patch it”; the bug predates every build they could find, so there is no known-good to roll to and no instrument first). Belt and braces on the tag field: enterkeyhint, one-field form so the keyboard’s Enter arrives as a submit, keyCode-13 and keyup ears with a text guard against double adds, focus put back at 0/60 ms. Harness 57/57, checks 4/4, mutations 46/46. `TB_MUT_ONLY` added to the mutation gate for iterating on a subset.
...
**v21.51.0 · 2026-09-21.** 27·ship candidate 8 ACCEPTED by the owner: back disabled, swap, flip judged on the far side, clean call, lag gone on cellular and wifi. Video is closed out; §7.15 is the record. D-7, D-8, D-9 closed. Baseline is now `bridge-turn27-ship.html` `8f017cf47b64`; the 27·post-ship address holds the same bytes until the technical-debt candidate is written. Chain to beta: technical debt → multi-user (relay, app) → directory release → IndexedDB (maybe) → beta readiness.
...
- L-2 (2026-09-04, REVISED 2026-09-12) Presence flicker = iOS background socket lifecycle. Owner ruling: NO client-side damping timer — the relay is the authority on presence. If the flicker is to be addressed at all, it is a RELAY question (distinguishing a clean departure from a transport drop in the relay's own membership state), specced and gated as relay work, never masked in the app.
...
────────────────────────────────────────────────────────────────────────
## §7.11 [ROLLED BACK 2026-09-14, buried G56 — solved the wrong problem, resilience not capacity] — was: STORAGE CUTOVER, SINGLE SHOT (27·post-ship)
────────────────────────────────────────────────────────────────────────
...
The "never a start_url" rule came from invite-install breakage. It is also the exact reason Chrome would not offer desktop install. Corrected rule: the folder manifest carries `start_url: "./"`; grant preservation is achieved by the invite being consumed on first visit (Chrome only offers install after 30 s engagement + a click). Root manifests keep no start_url.


────────────────────────────────────────────────────────────────────────
## §7.12 SKELETON PWA — installability proven before the app moves
────────────────────────────────────────────────────────────────────────
Owner ruling 2026-09-06: no more install experiments on the real app. A
throwaway skeleton at /stuff/tb-skeleton/ carries a manifest, worker and
flag-motif icons and must demonstrably install on desktop Chrome, Android
and iPhone. Only what the skeleton PROVES gets copied into the app's
folder release. Patching the app to chase install is forbidden.
S1 icons: generated from the app's flag motif (flags.png) — 180, 192, 512,
   maskable 512 (safe-zone padded), all PNG, opaque background.
S2 manifest: exactly the Chrome criteria set, nothing else.
S3 worker: minimal, with a fetch handler (some installability paths still
   require one).
...
## G51 (not a burial — a deletion) 2026-09-13
The legacy inferred-presence engine (traffic lights the dot, 75s time-to-live darkens it) and the socket-close presence calls were removed from the accepted body at owner order: "old legacy code that should have been taken out in the first place — an oversight." This is the class buried as §4.6 A3 ("a socket is not a person") finally removed from the dot as well as from push. One owner of the dot, forever: the relay's declared-presence announcement.
...
### §7.6 addendum — R13 (owner process rule after two rejections)
Before any candidate 3: a complete, single-pass inventory of every site in
the accepted body that references call video, PiP, or the reduced/corner
view — CSS (head stylesheet AND any JS-injected stylesheet such as
R8B_CSS), every `CALL.*` wrap, every classList add/remove touching
...
## §7.6 addendum — open backlog items from 27·ship (owner-accepted with known gaps, 2026-09-14)
- Small-video drag: confined by a z-index/stacking bug to the large video
  pane instead of the full screen. Fix scope: the drag positioning already
  targets `position:fixed`, so this is a stacking-context issue — likely
  the video pane or an ancestor establishes its own stacking context that
  clips a fixed-position descendant; needs investigation before a fix.
- Camera flip (R17): not confirmed working on device; not re-attempted
  this release.
- (Already recorded above) calls do not survive app backgrounding at all.

## §7.6 addendum — V2 dropped to backlog, 2026-09-14
The back-button reduced-9:16-view feature is deferred, unscheduled, pending
a redesign on real browser Picture-in-Picture (`documentPictureInPicture`/
`requestPictureInPicture`) rather than an in-page CSS box — the CSS-only
approach cannot survive the app backgrounding, which is the entire point
of the feature. Not re-attempted until that foundation is specced.


## NEW FINDING 2026-09-14 — calls do not survive backgrounding, at all
Larger than the video-surface release: this app has no path to resume an
active call after the OS reloads a backgrounded page (common on mobile).
The only resume handler in the codebase is scoped to the chat view and
never checks call state. The other party has no signal that this
happened. This needs its own spec — likely requires either (a) real
reconnect-to-an-in-progress-call logic, or (b) at minimum, detecting the
loss and telling both parties the call ended, rather than one side being
...
FILE: `bridge-turn27-ship.html` = bytes of ACCEPTED 27·ship candidate 5
...
`#local-video` in `#call-videos`; TAP the big video → the two streams swap
(`tbSwapTap`, `.videos.swapped`); a flip button bottom-left of the video
...
mount, nothing listening, second press leaves the app (G57). Ruling: back
does NOTHING during a call. Part S-2 (`s2-back-absorb.js`) adds one
`popstate` listener that, while `CALL.active`, re-pushes `{tbCall:1}` so
there is always one more entry to absorb; outside a call it does nothing.
It replaces nothing and never touches swap, flip or PiP (gated).
The corner-band rebuild (c6, G59) is withdrawn; its transport parts are
carried unchanged. Input to c7 is the banked c5 fixture
`talkbridge/fixtures/bridge-turn27-ship-c5-accepted-2026-09-14.html`
(sha256 956ceb381585…), byte-checked by the assembler.
...
no DOM write; no popstate/pushState/enterPip/exitPip in any TRANSPORT part; S-2 adds exactly one popstate listener and never names swap/flip/PiP;
...
open, never retries a replaced socket, stops outside a room; (g) S-2: real
