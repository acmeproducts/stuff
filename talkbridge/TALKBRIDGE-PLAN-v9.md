<!-- TALKBRIDGE-PLAN v21.41.0 -->
# TALKBRIDGE MASTER PLAN v21.41.0
...
| 27·post-ship | **Storage cutover, single shot** — IndexedDB becomes primary; no export, no migration/seeding, IDB starts empty; X3 restores IDB→localStorage on boot by gating p2Entry (the same entry P3 already wraps); X4 writes IDB first then localStorage; raw credential keys keep their own 30s sweep | Spec §7.11 | **BUILT 2026-09-14 — device gate pending (G1–G3).** Gates M1–M4 pass; both spec-mandated mutations (write-order reversal, seeding-code reintroduction) caught. | https://acmeproducts.github.io/stuff/bridge-turn27-post-ship.html |
