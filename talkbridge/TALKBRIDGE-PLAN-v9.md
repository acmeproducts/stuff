<!-- TALKBRIDGE-PLAN v21.15.0 -->
# TALKBRIDGE MASTER PLAN v21.15.0
...
| 27·base | **BLOCKED 2026-09-12 (owner).** Both candidates buried (G50): candidate 1 included a presence timer never agreed; candidate 2 was an in-place edit of a released artifact — a process violation. Address rolled back byte-exact to accepted 26·post-ship. Nothing builds until (a) presence is root-caused against the historical build where it worked, (b) a single spec covering notifications AND presence is agreed in writing, (c) owner GO. | 27·base | **Declared presence + notifications** — PR-1 relay v6.4 reads presence from each device's declared {visible,inRoom} keyed by device (a socket is not a person); PR-2 app declares on room enter/leave, hide/show (browser tabs too, a gap in the accepted build) and pagehide; K1 alert icon/badge + call renotify via new tb-sw2.js with exact-match old-worker retirement; C3 render coalescing | Spec §7.13 | **BUILT 2026-09-12, wink root-caused and FIXED 2026-09-13 — device gate pending (G1–G8).** Root cause: the dot had THREE owners — the relay signal, a legacy traffic-lit 75s time-to-live engine, and socket-close calls that darkened it on every reconnect. Owner ruling: legacy code, an oversight, delete it. Done — the inferred-presence engine and both socket-level presence calls are removed from the body (touchPresence keeps only its last-seen bookkeeping so frozen callers stay safe); the relay announcement is now the sole owner. Dot contrast also raised (10px, darker gray with border vs saturated green with halo). Gates M1–M6 PASS, 8/8 presence harness (two sockets = one person; background/room-list/force-quit all dark; no ghosts), mutations 4/4. No timers anywhere in the presence path. Accepted worker and manifests untouched. | https://acmeproducts.github.io/stuff/bridge-turn27-base.html |
