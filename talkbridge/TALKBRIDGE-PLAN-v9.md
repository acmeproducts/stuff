<!-- TALKBRIDGE-PLAN v21.29.0 -->
# TALKBRIDGE MASTER PLAN v21.29.0
...
| 27·ship | **Video call surface** — corner-band PiP removed; tap-swap big/small; back-button reduced 9:16 draggable box with red hang-up + expand; small video independently draggable in normal calls; camera flip. Screen share DROPPED from scope by owner order 2026-09-14, unscheduled — no number assigned; owner to sequence when wanted. | Spec §7.6, trimmed (R7–R9 share rules removed; new rule R12: any new call-state control must reuse #rb-hangup's `.st-phone`/`.st-video` visibility CSS — not assume a call-only row exists) | **REJECTED 2026-09-14 → buried G52 (icons visible outside a call — no CSS gating). Address rolled back byte-exact to accepted 27·pre-ship.** Rebuild pending. | https://acmeproducts.github.io/stuff/bridge-turn27-ship.html (accepted pre-ship bytes) |
