<!-- TALKBRIDGE-PLAN v21.38.0 -->
# TALKBRIDGE MASTER PLAN v21.38.0
...
| 27·ship | **Video call surface, candidate 5** — tap-swap (kept); camera-flip stops the old track before requesting the new one (matches this codebase's own established pattern, everywhere else already does this); drag gets `touch-action:none` on the video elements. Screen share still dropped, V2 still dropped. | Spec §7.6 + R17 (stop-before-acquire) + R18 (touch-action:none) | **BUILT 2026-09-14, candidate 5 — device gate pending.** Gates+mutations pass, including direct verification that the old track is stopped before the new one is requested (R17) and touch-action:none is present (R18). iPhone camera-flip visibility still UNRESOLVED, no cause found — gate must report exactly what's on screen. | https://acmeproducts.github.io/stuff/bridge-turn27-ship.html |
