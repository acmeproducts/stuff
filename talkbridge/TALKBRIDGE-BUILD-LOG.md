...
### SPEC · Chrome strip · Jul 29 2026 PT
talkbridge/talkbridge-chrome-strip-spec.html - now the layout authority for the strip, superseding the strip sections of the mode spec.
ZONE MODEL: left=identity, centre=broadcast (what I am sending), right=call lifecycle, then ... Mic PINNED to the true centre line, camera in a reserved slot to its right (empty in chat/phone) so the mic never shifts.
