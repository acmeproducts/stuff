#!/usr/bin/env node
/* The four pre-push structural checks for 27·ship candidate 8: syntax, HTML
...
const MARKERS = ['GAP PART · D1-call-diagnostics.js', 'GAP PART · C1-signal-queue.js', 'GAP PART · V2-relay-retry.js', 'GAP PART · C3-joiner-restart.js', 'GAP PART · C2-stall-frames.js', 'GAP PART · S2-back-absorb.js', 'GAP PART · F1-flip-keeps-sender.js'];
...
function checkStructure(html) {
  ...
  return 'tags balanced, seven parts present in order (D1, V-1, V-2, V-3, V-4, S-2, F-1)';
}
function checkWire(html) {
  ...
  for (const sym of ['function relaySend(', 'function relaySendWhenOpen(', 'function relayConnect(', 'CALL.runRecovery = function', 'CALL.startVideoWatchdog = function', 'CALL.stopVideoWatchdog = function', 'onSignal:async function', 'function tbSwapTap', 'function tbFlipCamera', 'history.pushState({tbCall:1}', 'function camSenders()', 'function replaceSenderTrack(']) if (html.slice(0, start).indexOf(sym) === -1) throw new Error('baseline symbol missing: ' + sym);
