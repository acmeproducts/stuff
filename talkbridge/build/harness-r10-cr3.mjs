#!/usr/bin/env node
/* R10-CR3 app gate (plan v20.9.0 §4.11.5). The assembled candidate runs in a
...
function makeWorld(opts) {
  ...
  const dom = new JSDOM(CURRENT_HTML, {
    ...
    beforeParse(window) {
      ...
      world.swAnnounced = [];
      const reg = { scope: 'https://acmeproducts.github.io/stuff/', active: { postMessage: (m) => world.swAnnounced.push(m) }, showNotification: (t, o) => { world.swShown.push({ t, o }); return Promise.resolve(); }, getNotifications: () => Promise.resolve([]), pushManager: { getSubscription: () => Promise.resolve(null), subscribe: () => Promise.reject(Object.assign(new Error('denied'), { name: 'NotAllowedError' })) } };
  ...
  world.blur = () => world.win.dispatchEvent(new world.win.Event('blur'));
...
async function runScenarios(html) {
  ...
  await scenario('S5b hidden on home → call → one OS request, no ring, no device-side alert; return presents the active call once', async () => {
    ...
    assert.equal(w.dbg('cr3_call_presented').length, 1);
    w.setHidden(true); await settle(); w.setHidden(false); await settle(250);
    assert.equal(w.dbg('cr3_call_presented').length, 1, 'never a second surface for the same call');
  ...
  await scenario('S6a warm tap on an active call → exact Accept/Decline surface, once; on an ended call → room with durable missed outcome, no ring', async () => {
    ...
    assert.equal(w.ring(), true, 'active call: the Accept/Decline surface'); assert.equal(w.dbg('cr3_call_presented').length, 1);
  ...
  await scenario('S6b cold tap → the app opens on the exact event: active call rings; ended call shows the room and outcome', async () => {
    ...
    assert.equal(w.ring(), true, 'active call: the answer surface'); assert.equal(w.dbg('cr3_call_presented').length, 1, 'presented once: ' + JSON.stringify(w.dbg().filter((l) => /^cr3_/.test(l.ev)).map((l) => l.ev)));
  ...
  await scenario('S8 leave the room → a call two seconds later → in-app surface, no OS request (lane continuity, G21)', async () => {
    const w = await boot({ store: roomSeed() });
    await subscribe(ROOM, B);
    w.win.enterRoom(ROOM); await settle(120);
    assert.equal(w.win.S.view, 'room');
    w.win.exitToStart(); await settle(150);
    assert.equal(w.win.S.view, 's1'); assert.equal(w.win.S.roomId, null);
    assert.equal(w.liveSockets(ROOM).length, 1, 'the room keeps a lane the moment it is left');
    const a = await sender(ROOM, 'dev-A');
    await a.say({ type: 'call-start', kind: 'voice', callId: 'k8', name: 'Ana' }); await settle();
    assert.equal(pushes.length, 0, 'no OS request beside a visible home');
    assert.equal(w.ring(), true, 'the in-app Accept/Decline surface, at once');
    assert.equal(record(ROOM, 'k8').rcp[B].p, 'in_app');
    assert.ok(w.dbg('cr3_leave_lane').length >= 1);
  });

  await scenario('S9 the installed app announces its window to the worker on boot and on every return (G22)', async () => {
    const w = await boot({ store: roomSeed() });
    await settle(100);
    assert.equal(w.swAnnounced.filter((m) => m.t === 'tb-app' && m.why === 'boot').length, 1, 'announced at boot');
    w.setHidden(true); await settle(); w.setHidden(false); await settle(150);
    assert.equal(w.swAnnounced.filter((m) => m.t === 'tb-app' && m.why === 'visible').length, 1, 'announced on return');
  });

  await scenario('S10 blur with the visibility flag stuck visible (iPhone) → chat is pushed, never acknowledged; focus return = one explicit open (G23)', async () => {
    const w = await boot({ store: roomSeed() });
    await subscribe(ROOM, B);
    w.win.enterRoom(ROOM); await settle(120);
    w.blur(); await settle();                                     /* no visibilitychange ever fires */
    assert.equal(w.hidden, false, 'the DOM still says visible');
    const st = sessionFor(ROOM).states[B]; assert.equal(st && st.visible, false, 'the relay was told at once: not watching');
    w.win.cr3Heartbeat(); await settle();
    assert.equal(sessionFor(ROOM).states[B].visible, false, 'the heartbeat carries the attended truth');
    const a = await sender(ROOM, 'dev-A');
    await a.say({ type: 'chat-msg', chatId: 'c9', srcText: 'hola' }); await settle();
    assert.equal(pushes.length, 1, 'OS request raised for an unattended page');
    assert.equal(record(ROOM, 'c9').rcp[B].p, 'os_requested');
    assert.equal(record(ROOM, 'c9').rcp[B].s, 'unseen', 'nothing acknowledged while unattended');
    assert.equal(wordsFrom(B, 'ev-seen', ROOM).length, 0);
    await a.say({ type: 'call-start', kind: 'voice', callId: 'k9', name: 'Ana' }); await settle();
    assert.equal(w.ring(), false, 'no ring while unattended'); assert.equal(pushes.length, 2);
    relay.words.length = 0;
    w.focus(); await settle(250);
    assert.equal(wordsFrom(B, 'ev-open', ROOM).length, 1, 'focus return in the routed room is one explicit open');
    assert.equal(record(ROOM, 'c9').rcp[B].s, 'seen');
    assert.equal(w.ring(), true, 'the active call is presented once on return');
    w.blur(); await settle(); w.focus(); await settle(250);
    assert.equal(pushes.length, 2, 'blur/focus within seconds raises no push storm');
  });

  await scenario('S11 relaunch with a grant on disk → the key resolver returns the grant; expired → empty; own key wins (§11)', async () => {
    const grant = { tb_grant: JSON.stringify({ room: ROOM, expires: Date.now() + 10 * 86400000, at: Date.now() }), tb_grant_dg: 'dg-grant-key', tb_grant_tid: 'tid-g', tb_grant_tok: 'tok-g' };
    let w = await boot({ store: Object.assign(roomSeed({ role: 'joiner' }), grant) });
    let k = JSON.parse(JSON.stringify(w.win.CALL.keys()));
    assert.deepEqual(k, { dg: 'dg-grant-key', tid: 'tid-g', tok: 'tok-g' }, 'a fresh session reads the stored grant');
    assert.equal(w.dbg('cr3_grant_keys_used').length, 1);
    const expired = Object.assign({}, grant, { tb_grant: JSON.stringify({ room: ROOM, expires: Date.now() - 1000, at: Date.now() - 86400000 }) });
    w = await boot({ store: Object.assign(roomSeed({ role: 'joiner' }), expired) });
    k = JSON.parse(JSON.stringify(w.win.CALL.keys()));
    assert.equal(k.dg, '', 'an expired grant yields nothing');
    w = await boot({ store: Object.assign(roomSeed({ role: 'joiner' }), grant, { tb_dg_key: 'dg-own', tb_cf_tid: 'tid-own', tb_cf_tok: 'tok-own' }) });
    k = JSON.parse(JSON.stringify(w.win.CALL.keys()));
    assert.deepEqual(k, { dg: 'dg-own', tid: 'tid-own', tok: 'tok-own' }, 'the device\'s own key wins');
  });
  ...
  await scenario('S7 baseline parity: the frozen bytes are carried verbatim; one part appended; no buried mechanism; no new secret', async () => {
    ...
    const part = fs.readFileSync(path.join(root, 'talkbridge/parts/r10-cr3-event-state.js'), 'utf8');
    ...
    assert.equal(w.win.osNotify._cr3Original.name === '' || typeof w.win.osNotify._cr3Original === 'function', true);
  });
...
const APP_MUTATIONS = [
  ...
  ['ring-while-hidden · a hidden app presents the ring screen', "    if (!cr3Attended()) { cr3Log('ring_deferred_hidden'", "    if (false) { cr3Log('ring_deferred_hidden'"],
  ['local-missed-pill · the browser writes its own missed pill again', "    if (typeof text === 'string' && text.indexOf('Missed ') === 0) { cr3Log('local_missed_pill_dropped', { room: roomId }); return; }", "    /* local pill restored */"],
  ['duplicate-lane · concurrent signals open a second socket', "      if (ws && ws.readyState === 0 && room && ws._cr3Room === room.id) { cr3Log('connect_coalesced', { room: room.id }); return; }", "      /* no coalescing */"],
  ['no-seen · a visibly handled chat is never acknowledged', "cr3Send(S.roomId, { type: 'ev-seen', ids: [String(d.eventId)] });", "void 0;"],
  ['cold-route-lost · a cold tap lands on the homepage', "        setTimeout(function () { cr3RouteEvent(roomId, eventId, 'tap_cold'); }, 0);", "        void roomId;"],
  ['mute-unheard · a mute toggle is not reported until the next reconnect', "      if (cr3State.muteSig !== undefined && cr3State.muteSig !== sig) {", "      if (false) {"],
  ['hidden-unannounced · going hidden is not reported to the relay', "if (document.hidden) { cr3SetAttended(false, 'hidden'); cr3Announce('hidden'); }", "if (document.hidden) { cr3SetAttended(false, 'hidden'); }"],
  ['device-alert · the app raises its own OS notification beside the relay push', "  osNotify = function (title, body, roomId) { cr3Log('os_notify_owned_by_relay', { room: roomId }); };", "  osNotify = function (title, body, roomId) { return _cr3OsNotify.apply(this, arguments); };"],
  ['second-surface · an active call is re-presented on every return', "    if (!c || !c.callId || cr3State.rung[c.callId]) return;", "    if (!c || !c.callId) return;"],
  ['leave-laneless · leaving a room does not open its listener lane', "    try { LISTEN.sync(); cr3Log('leave_lane', { room: left }, 'ok'); cr3Recover('leave'); }", "    try { void left; }"],
  ['no-announce · the installed app never announces its window', "      cr3AnnounceWindow('boot');", "      void 0;"],
  ['blur-ignored · leaving the app without a visibility change is not reported', "      window.addEventListener('blur', function () { cr3SetAttended(false, 'blur'); cr3Announce('blur'); });", "      void 0;"],
  ['seen-while-unattended · a running page acknowledges chats nobody is watching', "d.type !== 'call-start' && wasAttended && S.view === 'room' && S.roomId)", "d.type !== 'call-start' && S.view === 'room' && S.roomId)"],
  ['grant-fallback-removed · the resolver ignores the stored grant', "      var out = { dg: k.dg || g.dg || '', tid: k.tid || g.tid || '', tok: k.tok || g.tok || '' };", "      var out = k;"],
  ['expired-grant-used · an expired grant still feeds the resolver', "      if (!rec || (typeof grantExpired === 'function' && grantExpired(rec))) return k;", "      if (!rec) return k;"],
  ['stale-lane · an open callback from a replaced socket reconciles anyway', "  if (!current()) { cr3Log('open_stale', { room: roomId }, 'warn'); return; }", "  void current;"]
];
