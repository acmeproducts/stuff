...
var dgWs=null,dgWsEn=null,dgAudioCtx=null,dgProc=null,dgSrc=null,dgActive=false;
var _dgEnLastFiredAt=0,_DG_CROSS_MS=600,_dgPrimHoldTimer=null,_dgPrimHeldText=null;
var _micAnalyser=null,_micLvlRaf=null,_micLvlSmooth=0;
...
// B3 fix: use unicode ranges to identify non-Latin scripts; detect Latin text in non-Latin rooms
function detectLang(text,src){
  ...
  if(/[Ḁ-ỿ]/.test(t))return 'vi';
  var nonLatinSrcs=['zh','ja','ko','th','ar','hi','ru'];
  if(nonLatinSrcs.indexOf(src)>=0&&/^[\x00-\x7FÀ-ɏ\s]+$/.test(t))return 'en';
  if(/^[\x20-\x7E]+$/.test(t)&&/\b(the|this|that|those|these|they|them|their|there|then|though|three|okay|sucks|yeah|awesome|what|where|when|why|who|how|you|your|are|were|have|hello|good|morning|afternoon|evening|speaking|talking|going|doing|thinking|working|don't|can't|won't|isn't|aren't|I'm|I've|I'll|you're|it's)\b/i.test(t))return 'en';
  return src||'en';
}

/* === FASTTEXT WASM === */
var _ftReady=false,_ftModule=null,_ftLoadFailed=false,_ftDegraded=false,_ftPending=[];
var _lastAsyncVia='wasm';
var CONF_SPEECH=0.58,CONF_CHAT=0.50,CONF_FLOOR=0.40,FT_LOAD_TIMEOUT_MS=5000;
...
function _ftStartupStep(step,extra){ log('ft_startup_step',{step:step,extra:extra||null}); }
function _ftStartupDone(ok,msg){
  if(!ok)_ftDegraded=true;
...
function _syncFtStatus(){
  ...
  if(_ftReady){dot.className='ft-status-dot ok';pill.classList.add('ready');}
  else if(_ftLoadFailed&&_ftDegraded){dot.className='ft-status-dot warn';label.textContent='Degraded — unicode fallback active';}
...
// B6 fix: handles Map/Array/Object/string return shapes; uses per-mode confidence thresholds
function _ftPredictLabel(ft,text,mode,attempt){
  var r=ft.predict(text,1);
  var item=null,label=null,score=null;
  function _normLabel(v){if(v==null)return null;return String(v).replace(/^(__label__|label__)/,'');}
  if(r instanceof Map){var first=r.entries().next();if(!first.done){label=_normLabel(first.value[0]);score=first.value[1];}}
  else if(Array.isArray(r)){item=r.length?r[0]:null;if(Array.isArray(item)){label=_normLabel(item[0]);score=item[1];}else if(item&&typeof item==='object'){label=_normLabel(item.label);score=(item.prob!=null?item.prob:(item.score!=null?item.score:item.probability));}else if(typeof item==='string'){label=_normLabel(item);}}
  else if(r&&typeof r==='object'){label=_normLabel(r.label!=null?r.label:r[0]);score=(r.prob!=null?r.prob:(r.score!=null?r.score:r.probability));}
  else if(typeof r==='string'){label=_normLabel(r);}
  if(!label||!_ftAllowed(label))return null;
  if(score==null)return label;
  var n=Number(score);if(!isFinite(n))return null;
  var base=(mode==='chat')?CONF_CHAT:CONF_SPEECH;
  var thresh=Math.max(CONF_FLOOR,base-(0.05*(attempt||0)));
  return n>=thresh?label:null;
}
function _loadFastText(){
  ...
  Promise.all(FT_ASSETS.map(_probeFtAsset)).then(function(){
    ...
    var ftModuleConfig={locateFile:function(path){
      ...
      return '/stuff/fastType/'+p.split('/').pop();
    }};
      ...
      try{
        ...
        ft.loadModel(FT_MODEL).then(function(){
          ...
          return Promise.all(FT_SMOKE.map(function(pair){
            var _raw=ft.predict(pair[0],1);var got=null;
            if(_raw instanceof Map){var _it=_raw.entries().next();if(!_it.done)got=String(_it.value[0]).replace(/^(__label__|label__)/,'');}
            else if(Array.isArray(_raw)&&_raw.length){var _v=_raw[0];got=Array.isArray(_v)?String(_v[0]).replace(/^(__label__|label__)/,''):typeof _v==='string'?_v.replace(/^(__label__|label__)/,''):(_v&&_v.label?String(_v.label).replace(/^(__label__|label__)/,''):null);}
            else if(_raw&&typeof _raw==='object'){got=String(_raw.label!=null?_raw.label:(_raw[0]||'')).replace(/^(__label__|label__)/,'');}
...
function _detectLangAsync(text){
  ...
  if(/[\u0E00-\u0E7F]/.test(t)){_lastAsyncVia='unicode';return Promise.resolve('th');}
  if(/[\u3040-\u30FF\u31F0-\u31FF]/.test(t)){_lastAsyncVia='unicode';return Promise.resolve('ja');}
  if(/[\u4E00-\u9FFF]/.test(t)){_lastAsyncVia='unicode';return Promise.resolve('zh');}
  if(/[\uAC00-\uD7A3]/.test(t)){_lastAsyncVia='unicode';return Promise.resolve('ko');}
  if(/[\u0600-\u06FF]/.test(t)){_lastAsyncVia='unicode';return Promise.resolve('ar');}
  if(/[\u0900-\u097F]/.test(t)){_lastAsyncVia='unicode';return Promise.resolve('hi');}
  if(/[\u0400-\u04FF]/.test(t)){_lastAsyncVia='unicode';return Promise.resolve('ru');}
  if(/[\u1E00-\u1EFF]/.test(t)){_lastAsyncVia='unicode';return Promise.resolve('vi');}
  if(/^[\x20-\x7E]+$/.test(t)&&/\b(the|this|that|those|these|they|them|their|there|then|though|three|okay|sucks|yeah|awesome|what|where|when|why|who|how|you|your|are|were|have|hello|good|morning|afternoon|evening|speaking|talking|going|doing|thinking|working|don't|can't|won't|isn't|aren't|I'm|I've|I'll|you're|it's)\b/i.test(t)){_lastAsyncVia='marker';return Promise.resolve('en');}
...
/* === CHAT === */
var _pbICards=[];
function pbISearch(q){
  var panel=$('pb-inline-panel');if(!panel)return;
  var cards=pbGetCards();
  if(_pbFilter!=='all'&&_pbFilter.indexOf('cat:')===0){
    var _catId=_pbFilter.slice(4);
    cards=cards.filter(function(c){return(c.catalogIds||[]).indexOf(_catId)>=0;});
  }
  var qLow=(q||'').toLowerCase().trim();
  if(qLow){
    cards=cards.filter(function(c){
      return (c.source||'').toLowerCase().indexOf(qLow)>=0
        ||(c.target||'').toLowerCase().indexOf(qLow)>=0
        ||(c.tags||[]).some(function(t){return t.toLowerCase().indexOf(qLow)>=0;});
    });
    cards.sort(function(a,b){return pbIScore(b,qLow)-pbIScore(a,qLow);});
  } else {
    cards.sort(function(a,b){return (b.usage||0)-(a.usage||0)||(b.createdAt||0)-(a.createdAt||0);});
  }
  _pbICards=cards.slice(0,30);
  var total=pbGetCards().length;
  var _pair=pbActivePair();
  var _pairLabel=_pair?pbLangFlag(_pair.src)+' '+_pair.src.toUpperCase()+' → '+pbLangFlag(_pair.tgt)+' '+_pair.tgt.toUpperCase()+' ':'';
  var hdr='<div class="pb-ip-hdr" id="pb-ip-hdr">'
    +'<button style="background:none;border:none;cursor:pointer;padding:2px;line-height:0;color:var(--text-dim);display:flex;align-items:center;flex-shrink:0;" onclick="pbOpenOverlayClean()" title="Open phrasebook"><svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/></svg></button>'
    +'<span style="font-size:13px;color:var(--text-dim);font-weight:600;flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">'+(_pairLabel||'Phrasebook')+'</span>'
    +'<span style="font-size:13px;color:var(--text-muted);flex-shrink:0;">'+(_pbICards.length?_pbICards.length+(total>_pbICards.length?' of '+total:''):'')+'</span></div>';
  if(!_pbICards.length){
...
function pbIScore(card,q){
  var s=(card.source||'').toLowerCase();var tg=(card.tags||[]).join(' ').toLowerCase();
...
function pbIRowHtml(card,idx){
  var TTS_SVG='<svg viewBox="0 0 24 24"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><path d="M19.07 4.93a10 10 0 0 1 0 14.14"/><path d="M15.54 8.46a5 5 0 0 1 0 7.07"/></svg>';
  return '<div class="pb-irow">'
    +'<div class="pb-iside" data-idx="'+idx+'" data-side="src" onclick="pbISideClick(+this.dataset.idx,this.dataset.side)">'
      +'<div style="flex:1;min-width:0;"><span class="pb-iside-txt">'+pbEsc(card.source||'')+'</span>'
      +'</div>'
      +'<button class="pb-irow-tts" data-idx="'+idx+'" onclick="event.stopPropagation();pbISpeakI('+idx+',0)" tabindex="-1">'+TTS_SVG+'</button>'
      +'<button class="pb-irow-send" onclick="event.stopPropagation();pbISend('+idx+',\'src\')" tabindex="-1" title="Send to chat">'+ICO.use+'</button>'
    +'</div>'
    +'<div class="pb-idiv"></div>'
    +'<div class="pb-iside" data-idx="'+idx+'" data-side="tgt" onclick="pbISideClick(+this.dataset.idx,this.dataset.side)">'
      +'<div style="flex:1;min-width:0;"><span class="pb-iside-txt">'+pbEsc(card.target||'')+'</span>'
      +'</div>'
      +'<button class="pb-irow-tts" data-idx="'+idx+'" onclick="event.stopPropagation();pbISpeakI('+idx+',1)" tabindex="-1">'+TTS_SVG+'</button>'
      +'<button class="pb-irow-send" onclick="event.stopPropagation();pbISend('+idx+',\'tgt\')" tabindex="-1" title="Send to chat">'+ICO.use+'</button>'
    +'</div>'
    +'</div>';
}
function pbOvRowHtml(card,idx){
  var TTS_SVG='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><path d="M19.07 4.93a10 10 0 0 1 0 14.14"/><path d="M15.54 8.46a5 5 0 0 1 0 7.07"/></svg>';
  var SEP='<div style="width:1px;background:#e0dbd6;flex-shrink:0;align-self:stretch;"></div>';
  return '<div style="display:flex;align-items:stretch;border-bottom:1px solid #ede9e4;">'
    +'<div style="display:flex;align-items:center;flex:1;gap:6px;padding:11px 12px;cursor:pointer;" data-idx="'+idx+'" data-side="src" onclick="pbISideClick(+this.dataset.idx,this.dataset.side)">'
      +'<div style="flex:1;min-width:0;"><span style="font-size:15px;color:#1a1714;line-height:1.3;display:block;">'+pbEsc(card.source||'')+'</span>'
      +'</div>'
      +'<button style="background:none;border:none;cursor:pointer;color:#b0a9a4;padding:5px;flex-shrink:0;line-height:0;" onclick="event.stopPropagation();pbISpeakI('+idx+',0)" tabindex="-1">'+TTS_SVG+'</button>'
      +'<button class="pb-irow-send" onclick="event.stopPropagation();pbISend('+idx+',\'src\')" tabindex="-1" title="Send to chat">'+ICO.use+'</button>'
    +'</div>'
    +SEP
    +'<div style="display:flex;align-items:center;flex:1;gap:6px;padding:11px 12px;cursor:pointer;" data-idx="'+idx+'" data-side="tgt" onclick="pbISideClick(+this.dataset.idx,this.dataset.side)">'
      +'<div style="flex:1;min-width:0;"><span style="font-size:15px;color:#1a1714;line-height:1.3;display:block;">'+pbEsc(card.target||'')+'</span>'
      +'</div>'
      +'<button style="background:none;border:none;cursor:pointer;color:#b0a9a4;padding:5px;flex-shrink:0;line-height:0;" onclick="event.stopPropagation();pbISpeakI('+idx+',1)" tabindex="-1">'+TTS_SVG+'</button>'
      +'<button class="pb-irow-send" onclick="event.stopPropagation();pbISend('+idx+',\'tgt\')" tabindex="-1" title="Send to chat">'+ICO.use+'</button>'
    +'</div>'
    +'</div>';
}
function pbISideClick(idx,side){
  ...
  if(ta){ta.value=text;ta.style.height='auto';ta.style.height=Math.min(ta.scrollHeight,80)+'px';
    $('chat-send-btn').disabled=!ta.value.trim();ta.focus();}
}
function pbISelect(id){
  var card=pbGetCardById(id);if(!card)return;
  pbIClose();
  var ta=$('chat-input');
  if(ta){ta.value=card.source||'';ta.style.height='auto';ta.style.height=Math.min(ta.scrollHeight,80)+'px';
    $('chat-send-btn').disabled=!ta.value.trim();ta.focus();}
}
function pbIRowClick(idx){var c=_pbICards[idx];if(c)pbISelect(c.id);}
function pbITagOpenI(idx,x,y){var c=_pbICards[idx];if(c)pbTagOpen&&pbTagOpen(c.id,x,y);}
function pbISpeakI(idx,side){var c=_pbICards[idx];if(c)pbSpeakCard(c.id,side?'tgt':'src');}
function pbISend(idx,side){
  var card=_pbICards[idx];if(!card)return;
  var text='[PB] '+(side==='src'?(card.source||''):(card.target||''));
  if(!text.trim())return;
  pbIClose();pbCloseOverlay();
  var ta=$('chat-input');if(!ta||!room.id)return;
  ta.value=text;ta.style.height='auto';ta.style.height=Math.min(ta.scrollHeight,80)+'px';
  $('chat-send-btn').disabled=false;
  sendChat();
}
function pbIClose(){
  var panel=$('pb-inline-panel');if(panel)panel.classList.remove('show');
  _pbICards=[];
}
...
function chatInputEvt(){
  ...
  var inSearch=raw.length>=2&&raw[0]==='/'&&raw!=='//';
  var xBtn=$('chat-search-x');if(xBtn)xBtn.style.display=raw.trim()?'':'none';
...
function chatGo(){
  var ta=$('chat-input');var val=ta?ta.value||'':'';
  // Turn 07 Ship: guard now runs through COMPOSE_SEAM (module contract) instead of an
  // inline check — same predicate rule (/ or ..), same guard on this one call site that
  // both Enter (chatKeydown) and the Send button share, so E1-E8 still hold.
  // '//' is excluded exactly as before (it's the command-drawer trigger on input, not submit).
  if(val!=='//'){
    var seam=COMPOSE_SEAM.handleSubmit(val);
    if(!seam.sendAllowed){
      var q=seam.query;
      pbSearchExit();
      pbOpenOverlaySearch(q);
      return;
    }
  }
  sendChat();
...
function chatKeydown(e){
  ...
  if(e.key==='Escape'&&panel&&panel.classList.contains('show')){e.preventDefault();pbSearchExit();return;}
  if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();chatGo();}
}
...
async function sendChat(){
  ...
  try{
    var _ftWonC=false,_detectViaC='fallback';
    var detected=await Promise.race([
      _detectLangAsync(text).then(function(d){_ftWonC=true;_detectViaC=d?_lastAsyncVia:'fallback';return d;}),
    ...
    // B2 fix: if race resolved null, fall back to unicode stub
    if(!detected)detected=detectLang(text,src);
    log('chat_norm_detect',{detected:detected||'(none)',src:src,needs_norm:!!(detected&&detected!==src),via:_detectViaC});
    if(detected&&detected!==src){
      ...
      if(normed&&normalizeText(normed,'compare')!==normalizeText(text,'compare')){srcText=normalizeText(normed,'chat');log('chat_norm_result',{srcText:srcText.slice(0,60)});}
      else{src=detected;log('chat_norm_fallback',{detected:detected},'warn');}
    }
...
var ICO={
  ...
  save:'<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z"/><polyline points="17 21 17 13 7 13 7 21"/></svg>',
  bt:'<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="23 4 23 10 17 10"/><path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10"/></svg>'
};
...
/* ── Card normalize ── */
var _pbLangNorm={cn:'zh',kr:'ko',vn:'vi'};
function _pbDataActive(){return CONFIG.get('use.PB_DATA');}
function _pbSyncActive(){return CONFIG.get('use.PB_SYNC');}
function _pbUsageActive(){return CONFIG.get('use.PB_USAGE');}
function pbNorm(c){
  if(_pbDataActive()){ return PB_DATA.norm(c); }
  return _pbNormLegacy(c);
}
function _pbNormLegacy(c){
  var now=pbNow();c=c||{};
  if(_pbLangNorm[c.sourceLang])c=Object.assign({},c,{sourceLang:_pbLangNorm[c.sourceLang]});
  if(_pbLangNorm[c.targetLang])c=Object.assign({},c,{targetLang:_pbLangNorm[c.targetLang]});
  ...
  return {
    ...
    relatedIntents:Array.isArray(c.relatedIntents)?c.relatedIntents.slice(0,5):[],
    semanticRelationships:c.semanticRelationships||null,
...
function pbGetCards(){return _pbDataActive()?PB_DATA.getLive():pbGetAllCards().filter(function(c){return !c.deletedAt;});}
function pbSaveCards(a){
  if(!_pbDataActive()){try{localStorage.setItem(PB_CARDS,JSON.stringify(a));}catch(_){toast('Storage full');}return;}
  var out=PB_DATA.save(a);if((a||[]).length&&out.count===0){toast('Storage full');}
  if(_pbSyncActive()){PB_SYNC.markDirty();if(typeof pbOvUpdateSyncDot==='function')pbOvUpdateSyncDot('dirty');}
}
...
function pbUpsert(card){
  // FIX (v5.7.2.3): matched by pbCardKey (content: source+target text), not id.
  // Any two blank cards have the same key ("" + "") -- a brand-new card from + would
  // match ANY existing blank card anywhere in the list and overwrite it in place
  // instead of being added at the front. That's why a new card landed mid-list
  // instead of at the top: it replaced whatever blank/matching card was already
  // sitting wherever pbGetCards() happened to return it. Cards have a stable,
  // unique id from pbNorm — match on that instead.
  var cards=pbGetCards();
...
function pbSoftDelete(id){
  // FIX (v5.7.2.2): no clarify entry logged for deletion — completes the
  // every-card-change-logs-something rule alongside tags/verdict/source/target.
  var card=pbGetCardById(id);if(!card)return;
  card.clarifyChain=card.clarifyChain||[];
  card.clarifyChain.push({text:'Moved to trash',author:'system',timestamp:pbNow()});
...
function pbRestoreCard(id){
  var card=pbGetCardById(id);if(!card)return;
  card.clarifyChain=card.clarifyChain||[];
  card.clarifyChain.push({text:'Restored from trash',author:'system',timestamp:pbNow()});
  card.deletedAt=null;pbSaveCard(card);
  pbRenderOverlay();
  toast('Phrase restored');
}
function pbReRenderBubbleFoot(id){
  var card=pbGetCardById(id);if(!card)return;
  var footEl=document.getElementById('pbfoot-'+id);
  if(footEl)footEl.innerHTML=pbBubbleFootHtml(card);
  var bbl=document.getElementById('pbb-'+id);
  if(bbl)bbl.className='pb-bubble'+(card.deletedAt?' deleted':'');
}
...
/* ── Search (supports - exclude) ──
   Turn 07 Ship: delegates to PB-QUERY (single engine for overlay + compose drawer,
   both callers already share this function — see pbRenderOverlay/pbRunSearch). */
function pbSearch(q,cards){
  if(!q||!q.trim())return cards;
  return PB_QUERY.query({text:q,cards:cards}).cards;
}
...
function pbCmdDrawerHtml(){
  return '<div class="pb-drawer-hdr"><span class="pb-drawer-title" style="font-size:11px;color:var(--text-muted);">Phrasebook (//) management</span>'    +'<button class="pb-drawer-x" onclick="pbCloseDrawer();pbClearComposeSlash()">×</button></div>'    +'<div class="pb-cmd-chips" style="justify-content:flex-start;">'    +'<button class="pb-ico-chip" onclick="pbCloseDrawer();pbClearCompose();pbOpenBooksModal()" title="Open / switch phrasebook">'+ICO.book+'</button>'    +'<button class="pb-ico-chip" onclick="pbCloseDrawer();pbClearCompose();pbOpenNewCardForContext()" title="New phrase">'+ICO.plus+'</button>'    +'<button class="pb-ico-chip" onclick="pbCloseDrawer();pbClearCompose();pbTriggerImport()" title="Import file">'+ICO.upload+'</button>'    +'<button class="pb-ico-chip" onclick="pbCloseDrawer();pbClearCompose();pbExportPrompt()" title="Export">'+ICO.download+'</button>'    +'</div>';
}
...
var _PB_BASE='https://raw.githubusercontent.com/acmeproducts/stuff/main/phrasebook/';
var _PB_LANG_MAP={zh:'cn',ko:'kr',vi:'vn'};
function _pbRepoToken(lang){return _PB_LANG_MAP[lang]||lang;}
function pbBuildUrl(){
  var src=(document.getElementById('pb-url-src')||{}).value||'en';
  var tgt=(document.getElementById('pb-url-tgt')||{}).value||'th';
  var url=_PB_BASE+'phrasebook-'+_pbRepoToken(src)+'-'+_pbRepoToken(tgt)+'-default.json';
...
function _pbFetchOne(src,tgt){
  var url=_PB_BASE+'phrasebook-'+_pbRepoToken(src)+'-'+_pbRepoToken(tgt)+'-default.json';
  var tsKey='pb_cdn_ts:'+src+'-'+tgt;
  if(Date.now()-parseInt(localStorage.getItem(tsKey)||'0',10)<86400000)return;
  fetch(url).then(function(r){if(!r.ok)throw new Error(r.status);return r.json();})
    .then(function(d){if(!d||!Array.isArray(d.cards))throw new Error('invalid');pbApplyImport(d,{silent:true});localStorage.setItem(tsKey,String(Date.now()));log('pb_loaded',{pair:src+'-'+tgt,count:d.cards.length});})
    .catch(function(e){log('pb_load_fail',{pair:src+'-'+tgt,e:String(e)},'warn');});
}
function _pbAutoLoad(){
  if(!room.myLang||!room.theirLang||room.myLang===room.theirLang)return;
  _pbFetchOne(room.myLang,room.theirLang);
  _pbFetchOne(room.theirLang,room.myLang);
  _pbCheckETag(room.myLang,room.theirLang);
  _pbCheckETag(room.theirLang,room.myLang);
}
var _PB_GIT_FILES=[
  {src:'en',tgt:'es'},{src:'es',tgt:'en'},
  {src:'en',tgt:'fr'},{src:'fr',tgt:'en'},
  {src:'en',tgt:'de'},{src:'de',tgt:'en'},
  {src:'en',tgt:'it'},{src:'it',tgt:'en'},
  {src:'en',tgt:'zh'},{src:'zh',tgt:'en'},
  {src:'en',tgt:'ko'},{src:'ko',tgt:'en'},
  {src:'en',tgt:'th'},{src:'th',tgt:'en'},
...
function pbOpenGitSheet(){
  ...
  var body=document.getElementById('pb-git-body');
  if(body){
    if(!activePair){
      body.innerHTML='<div style="color:#9a9592;font-size:14px;">No active language pair.</div>'
        +'<button onclick="pbCloseGitSheet()" class="pb-nc-save" style="margin-top:12px;">Close</button>';
    }else{
      var sf=pbLangFlag(activePair.src);var tf=pbLangFlag(activePair.tgt);
      var available=_PB_GIT_FILES.some(function(f){return f.src===activePair.src&&f.tgt===activePair.tgt;});
      if(available){
        body.innerHTML='<div style="font-size:44px;line-height:1;margin-bottom:8px;">'+sf+' '+tf+'</div>'
          +'<div style="font-size:18px;font-weight:700;color:#1a1714;margin-bottom:4px;">'+pbEsc(activePair.src.toUpperCase()+' → '+activePair.tgt.toUpperCase())+'</div>'
          +'<div style="font-size:13px;color:#9a9592;margin-bottom:20px;">Add this phrasebook to your library?</div>'
          +'<div style="display:flex;gap:8px;width:100%;max-width:260px;">'
          +'<button onclick="pbCloseGitSheet()" style="flex:1;background:none;border:1.5px solid #e0dbd6;border-radius:10px;padding:11px;font-size:14px;cursor:pointer;font-family:\'DM Sans\',sans-serif;color:#5a5552;">Cancel</button>'
          +'<button class="pb-nc-save" style="flex:1;" onclick="pbGitImport(\''+activePair.src+'\',\''+activePair.tgt+'\')">Add</button>'
          +'</div>';
      }else{
        body.innerHTML='<div style="font-size:44px;line-height:1;margin-bottom:8px;">'+sf+' '+tf+'</div>'
          +'<div style="font-size:18px;font-weight:700;color:#1a1714;margin-bottom:8px;">'+pbEsc(activePair.src.toUpperCase()+' → '+activePair.tgt.toUpperCase())+'</div>'
          +'<div style="font-size:14px;color:#9a9592;line-height:1.6;margin-bottom:20px;">This phrasebook is under development.<br>Check back later.</div>'
          +'<button onclick="pbCloseGitSheet()" class="pb-nc-save">OK</button>';
      }
    }
  }
  sheet.classList.add('open');
}
function pbCloseGitSheet(){
  var sheet=document.getElementById('pb-git-sheet');if(sheet)sheet.classList.remove('open');
}
function pbGitImport(src,tgt){
  var url=_PB_BASE+'phrasebook-'+_pbRepoToken(src)+'-'+_pbRepoToken(tgt)+'-default.json';
  pbCloseGitSheet();
  pbImportProgressShow('Fetching '+src.toUpperCase()+' → '+tgt.toUpperCase()+'…',0,0,0,[]);
  fetch(url)
    .then(function(r){if(!r.ok)throw new Error('HTTP '+r.status);return r.json();})
    .then(function(data){
      if(!data||!Array.isArray(data.cards))throw new Error('Invalid phrasebook file');
      pbGitImportProcess(data,src,tgt);
    })
    .catch(function(e){pbImportProgressShow('Error: '+e.message,0,0,0,[]);document.getElementById('pb-imp-done').style.display='';});
}
function pbGitImportProcess(data,src,tgt){
  var cards=data.cards;var total=cards.length;
  var validCards=[];var skipped=[];var i=0;var BATCH=10;
  function step(){
    var end=Math.min(i+BATCH,total);
    for(;i<end;i++){
      var c=cards[i];
      if(!c||typeof c.source!=='string'||!c.source.trim()||
         typeof c.target!=='string'||!c.target.trim()||
         typeof c.sourceLang!=='string'||typeof c.targetLang!=='string'){
        skipped.push('Row '+(i+1)+': missing required field');continue;
      }
      validCards.push(c);
    }
    pbImportProgressShow(src.toUpperCase()+' → '+tgt.toUpperCase(),total,validCards.length+skipped.length,validCards.length,skipped);
    if(i<total){setTimeout(step,0);}
    else{
      pbApplyImport({cards:validCards,catalogs:data.catalogs||[],tags:data.tags||[]},{silent:true});
      localStorage.setItem('pb_cdn_ts:'+src+'-'+tgt,String(Date.now()));
      document.getElementById('pb-imp-done').style.display='';
      pbRenderOverlayFilter();pbRenderOverlay();
    }
  }
  step();
}
function pbImportProgressShow(title,total,done,imported,skipped){
  var el=document.getElementById('pb-import-progress');if(!el)return;
  el.style.display='flex';
  document.getElementById('pb-imp-title').textContent=title;
  var pct=total?Math.round(done/total*100):0;
  document.getElementById('pb-imp-bar').style.width=pct+'%';
  document.getElementById('pb-imp-stats').textContent=total?('Imported '+imported+' of '+total+' ('+pct+'%) · Skipped: '+skipped.length):'';
  var sk=document.getElementById('pb-imp-skipped');
  sk.innerHTML=skipped.slice(-5).map(function(s){return '<div>'+pbEsc(s)+'</div>';}).join('');
}
function pbImportProgressClose(){
  var el=document.getElementById('pb-import-progress');if(el)el.style.display='none';
  document.getElementById('pb-imp-done').style.display='none';
  document.getElementById('pb-imp-bar').style.width='0';
}
/* ── CDN + Translation Memory + Quick-Ring + Telemetry ── */
var _PB_CDN_BASE=_PB_BASE;
var _intentIdMap={};
var _sessionId=Date.now().toString(36)+Math.random().toString(36).slice(2,6);
function _pbRepoToken(lang){return({zh:'cn',ko:'kr',vi:'vn'})[lang]||lang;}
function _pbNormCard(raw){
  var lp=raw.languagePair||{};var sr=raw.semanticRelationships||{};var rep=raw.representation||{};
  var sl=(lp.source||'en');if(sl==='vn')sl='vi';var tl=(lp.target||'en');if(tl==='vn')tl='vi';
  return{id:uid(),intentId:raw.intentId||'',fingerprint:raw.fingerprint||'',catalogIds:[],
    sourceLang:sl,targetLang:tl,source:rep.source||'',target:rep.target||'',notes:'',
    tags:raw.tags||[],relatedIntents:(sr.relatedIntents||[]),semanticRelationships:raw.semanticRelationships||null,usage:0,
    backtranslate:{sourceLang:tl,targetLang:sl,inputText:'',resultText:'',verdict:'',contentHash:'',updatedAt:pbNow()},
    clarifyChain:[],savedBy:'cdn',createdAt:pbNow(),updatedAt:pbNow()};
}
function _pbLoadCDN(srcLang,tgtLang){
  var url=_PB_CDN_BASE+'phrasebook-'+_pbRepoToken(srcLang)+'-'+_pbRepoToken(tgtLang)+'-default.json';
  var tsKey='pb_cdn_ts:'+srcLang+'-'+tgtLang;
  var fresh=Date.now()-parseInt(localStorage.getItem(tsKey)||'0',10)<86400000;
  if(fresh){_tmBuild(srcLang,tgtLang);return;}
  fetch(url).then(function(r){if(!r.ok)throw new Error('CDN '+r.status);return r.json();})
    .then(function(data){
      if(!data||!Array.isArray(data.cards))throw new Error('Invalid phrasebook');
      var mapped=data.cards.map(_pbNormCard);
      mapped.forEach(function(c){if(c.intentId)_intentIdMap[c.intentId]=c;});
      pbApplyImport({cards:mapped,catalogs:[],tags:[]},{});
      localStorage.setItem(tsKey,String(Date.now()));
      _tmBuild(srcLang,tgtLang);
    }).catch(function(e){log('cdn_load_fail',{e:String(e)},'warn');_tmBuild(srcLang,tgtLang);});
}
function _pbCheckETag(srcLang,tgtLang){
  var url=_PB_CDN_BASE+'phrasebook-'+_pbRepoToken(srcLang)+'-'+_pbRepoToken(tgtLang)+'-default.json';
  var eKey='pb_cdn_etag:'+srcLang+'-'+tgtLang;
  fetch(url,{method:'HEAD'}).then(function(r){
    var etag=r.headers.get('etag')||r.headers.get('last-modified')||'';
    if(!etag)return;
    var prev=localStorage.getItem(eKey)||'';
    if(prev&&prev!==etag){toast('Phrasebook update available — tap ☁↓ to refresh');log('cdn_etag_changed',{pair:srcLang+'-'+tgtLang});}
    localStorage.setItem(eKey,etag);
  }).catch(function(){});
}
function pbGetCardByIntentId(intentId){
  if(_intentIdMap[intentId])return _intentIdMap[intentId];
  var cards=pbGetCards();for(var i=0;i<cards.length;i++){if(cards[i].intentId===intentId){_intentIdMap[intentId]=cards[i];return cards[i];}}
  return null;
}
var _tmIndex={};
function _tmNorm(text){return(text||'').toLowerCase().replace(/[　-〿＀-￯]/g,'').replace(/[^\w]/g,'').trim();}
function _tmBuild(srcLang,tgtLang){
  var pair=srcLang+'-'+tgtLang;var index={};
  pbGetCards().filter(function(c){return c.sourceLang===srcLang&&c.targetLang===tgtLang;})
    .forEach(function(c){if(!c.source||!c.target)return;var key=c.fingerprint||_tmNorm(c.source);
      var corr=(c.clarifyChain||[]).length;var usage=c.usage||0;
      index[key]={tgt:c.target,intentId:c.intentId||'',cardId:c.id,usage:usage,confidence:usage/Math.max(1,usage+corr*3)};});
  _tmIndex[pair]=index;
}
function _tmCheck(text,srcLang,tgtLang){
  var pair=srcLang+'-'+tgtLang;if(!_tmIndex[pair])return null;
  var hit=_tmIndex[pair][_tmNorm(text)];
  if(!hit)return null;
  if(hit.confidence>=0.90&&hit.usage>=5)return hit.tgt;
  return null;
}
var _qrCardId=null;
function _qrOpen(cardId,x,y,entryPoint){
  _qrClose();var card=pbGetCardById(cardId)||pbGetCardByIntentId(cardId);
  if(!card||(card.relatedIntents||[]).length===0)return;
  _qrCardId=cardId;_qrRender(card,x,y,entryPoint||'quick_ring');
}
function _qrRender(card,cx,cy,entryPoint){
  var neighbors=card.relatedIntents||[];var N=neighbors.length;if(!N)return;
  var wrap=document.createElement('div');wrap.id='qr-wrap';
  wrap.style.cssText='position:fixed;inset:0;z-index:7900;';
  wrap.addEventListener('click',_qrClose);
  var ring=document.createElement('div');ring.id='qr-ring';
  ring.style.cssText='position:fixed;z-index:8000;pointer-events:none;left:0;top:0;';
  ring.addEventListener('click',function(e){e.stopPropagation();});
  ring.appendChild(_qrNode(card.source||'',cx,cy,true));
  neighbors.forEach(function(intentId,i){
    var nb=pbGetCardByIntentId(intentId);if(!nb)return;
    var angle=(2*Math.PI*i)/N-Math.PI/2;var nx=cx+75*Math.cos(angle);var ny=cy+75*Math.sin(angle);
    var node=_qrNode(nb.source||'',nx,ny,false);node.style.pointerEvents='auto';
    var tapTimer=null;
    node.addEventListener('click',function(e){e.stopPropagation();
      if(tapTimer){clearTimeout(tapTimer);tapTimer=null;_qrUse(nb,entryPoint);}
      else{tapTimer=setTimeout(function(){tapTimer=null;_qrClose();_qrRender(nb,nx,ny,entryPoint);},300);}
    });
    ring.appendChild(node);
  });
  document.body.appendChild(wrap);document.body.appendChild(ring);
}
function _qrNode(label,cx,cy,isCenter){
  var size=isCenter?36:28;var el=document.createElement('div');
  el.style.cssText='position:fixed;left:'+(cx-size/2)+'px;top:'+(cy-size/2)+'px;width:'+size+'px;height:'+size+'px;border-radius:50%;'
    +'background:'+(isCenter?'#4A90D9':'#1a1a2e')+';border:1.5px solid '+(isCenter?'#fff':'#4A90D9')+';cursor:pointer;overflow:visible;';
  if(label){
    var tip=document.createElement('div');
    tip.style.cssText='position:absolute;bottom:calc(100% + 4px);left:50%;transform:translateX(-50%);background:#0a0a14;border:1px solid #333;border-radius:4px;padding:3px 7px;font-size:11px;color:#fff;white-space:nowrap;max-width:200px;overflow:hidden;text-overflow:ellipsis;pointer-events:none;opacity:0;transition:opacity .12s;z-index:8010;';
    tip.textContent=label;el.appendChild(tip);
    el.addEventListener('mouseenter',function(){tip.style.opacity='1';});
    el.addEventListener('mouseleave',function(){tip.style.opacity='0';});
  }
  return el;
}
function _qrUse(card,entryPoint){
  _qrClose();var ta=document.getElementById('chat-input');
  if(ta){ta.value=card.source||'';ta.style.height='auto';ta.style.height=Math.min(ta.scrollHeight,80)+'px';
    var sb=document.getElementById('chat-send-btn');if(sb)sb.disabled=!ta.value.trim();ta.focus();}
  _pbEmitUsage(card,entryPoint||'quick_ring');
}
function _qrClose(){
  var r=document.getElementById('qr-ring');if(r)r.remove();
  var w=document.getElementById('qr-wrap');if(w)w.remove();_qrCardId=null;
}
...
function pbRunSearch(){
  _pbCardState={}; // C13: reset card panel states before each render
  var si=document.getElementById('pb-search-input');
  ...
  host.querySelectorAll('.pb-bubble').forEach(function(el){
    var cid=el.id.replace('pbb-','');var lpt=null;
    el.addEventListener('touchstart',function(e){lpt=setTimeout(function(){_qrOpen(cid,e.touches[0].clientX,e.touches[0].clientY,'quick_ring');},600);},{passive:true});
    el.addEventListener('touchend',function(){clearTimeout(lpt);},{passive:true});
    el.addEventListener('touchmove',function(){clearTimeout(lpt);},{passive:true});
  });
}
...
function pbUseText(id,side){
  ...
  var ta=document.getElementById('chat-input');
  if(ta){ta.value=text;ta.style.height='auto';ta.style.height=Math.min(ta.scrollHeight,80)+'px';
    var sb=document.getElementById('chat-send-btn');if(sb)sb.disabled=!text.trim();ta.dispatchEvent(new Event('input'));ta.focus();}
  if(_pbUsageActive())PB_USAGE.recordUse(id);
  pbCloseDrawer();
  pbCloseOverlay();
}
...
function pbBubbleFootHtml(card){
  var id=card.id;var cs=_pbCS(id);var isDel=!!card.deletedAt;
  ...
  var ICON_DEL='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>';
  return (isDel
    ?'<button class="pb-fbt" onclick="pbRestoreCard(\''+id+'\')" title="Restore">'+ICO.restore+'</button>'
     +'<button class="pb-fbt" onclick="pbHardDelete(\''+id+'\')" title="Delete permanently">'+ICON_DEL+'</button>'
    :'<button class="pb-fbt" onclick="pbSoftDelete(\''+id+'\')" title="Move to trash">'+ICON_DEL+'</button>')
    +'<button class="pb-fbt'+(cs.tagsOpen?' on':'')+'" id="pbft-'+id+'" onclick="pbTogTags(\''+id+'\')" title="Tags">#</button>'
    +'<button class="pb-fbt'+(cs.clarifyOpen?' on':'')+'" id="pbfc-'+id+'" onclick="pbTogClarify(\''+id+'\')" title="Clarify">'+ICON_CLARIFY+'</button>';
}
function _pbHdrText(card){
  // FIX (v5.7.2.3): date only, no time — added per explicit direction.
  var fmt=function(ms){return ms?new Date(ms).toLocaleDateString([],{month:'short',day:'numeric',year:'2-digit'})+' '+new Date(ms).toLocaleTimeString([],{hour:'numeric',minute:'2-digit'}):'—';};
  var cAt=fmt(card.createdAt);
  var uAt=fmt(card.updatedAt);
  var cBy=card.createdBy||card.savedBy||'—';
  var uBy=card.updatedBy||card.savedBy||'—';
  return 'Created '+cBy+' · '+cAt+'  —  Modified '+uBy+' · '+uAt;
}
function pbBubbleHtml(card,ctx){
  var id=card.id;var src=card.sourceLang||'en';var tgt=card.targetLang||'en';
  // B8: show user's language on left regardless of card storage direction
  var _flip=room.myLang&&tgt===room.myLang&&src!==room.myLang;
  var leftSide=_flip?'tgt':'src';var rightSide=_flip?'src':'tgt';
  var leftLang=_flip?tgt:src;var rightLang=_flip?src:tgt;
  var leftText=_flip?card.target:card.source;var rightText=_flip?card.source:card.target;
  ...
  var useL=L('use_word',leftLang);var useR=L('use_word',rightLang);
  // FIX (v5.7.2.1): the source/target fields previously had onblur only — Enter in a
  // contenteditable div doesn't fire blur, so pbCommitSrcEdit/TgtEdit never ran until
  // you tapped away. Added onkeydown so Enter (no shift) blurs the field itself,
  // reusing the exact same commit path instead of duplicating logic.
  return '<div class="pb-bubble'+(isDel?' deleted':'')+'" id="pbb-'+id+'">'
    +'<div class="pb-bbl-hdr"><span class="pb-bbl-ts">'+pbEsc(_pbHdrText(card))+'</span></div>'
    +'<div class="pb-bbl-body">'
    +'<div class="pb-bbl-col"><div class="pb-bbl-txt" id="pb'+leftSide+'-'+id+'" contenteditable="true" onblur="pbCommit'+(_flip?'Tgt':'Src')+'Edit(\''+id+'\')" onkeydown="if(event.key===\'Enter\'&&!event.shiftKey){event.preventDefault();this.blur();}" spellcheck="false">'+pbEsc(leftText)+'</div>'
    +'<div class="pb-bbl-acts"><button class="pb-use-btn" onclick="pbUseText(\''+id+'\',\''+leftSide+'\')" title="'+pbEsc(useL)+'">'+pbEsc(useL)+'</button>'
    +'<button class="pb-tts-btn" onclick="pbSpeakCard(\''+id+'\',\''+leftSide+'\',this)" title="Play">'+PB_ICON_TTS+'</button></div></div>'
    +'<div class="pb-bbl-div"></div>'
    +'<div class="pb-bbl-col"><div class="pb-bbl-txt" id="pb'+rightSide+'-'+id+'" contenteditable="true" onblur="pbCommit'+(_flip?'Src':'Tgt')+'Edit(\''+id+'\')" onkeydown="if(event.key===\'Enter\'&&!event.shiftKey){event.preventDefault();this.blur();}" spellcheck="false">'+pbEsc(rightText)+'</div>'
    +'<div class="pb-bbl-acts"><button class="pb-use-btn" onclick="pbUseText(\''+id+'\',\''+rightSide+'\')" title="'+pbEsc(useR)+'">'+pbEsc(useR)+'</button>'
    +'<button class="pb-tts-btn" onclick="pbSpeakCard(\''+id+'\',\''+rightSide+'\',this)" title="Play">'+PB_ICON_TTS+'</button></div></div>'
    +'</div>'
    +'<div class="pb-bbl-foot" id="pbfoot-'+id+'" style="display:grid;grid-template-columns:repeat(3,minmax(0,1fr));border-top:1px solid rgba(0,0,0,.06);padding:5px 8px;gap:4px;">'+pbBubbleFootHtml(card)+'</div>'
    ...
    +'<div class="pb-panel" id="pbbt-'+id+'" style="display:block;">'+pbBtPanelHtml(card)+'</div>'
    +'</div>';
}
function _pbTagPillHtml(id,t){
  return'<span class="pb-tp">#'+pbEsc(t)
    +'<button onclick="pbRemoveTag(\''+id+'\',\''+pbEsc(t)+'\')" style="background:none;border:none;cursor:pointer;color:inherit;margin-left:2px;font-size:10px;">×</button></span>';
}
function pbTagPanelHtml(card){
  var id=card.id;var tags=card.tags||[];
  var pills=tags.map(function(t){return _pbTagPillHtml(id,t);}).join('');
...
function pbAddTag(id,tag){
  // FIX (v5.7.2.1): previously called pbReRenderCardPanel('tags',id) here, which
  // rebuilds the whole panel and destroys #pb-ti-{id} mid-typing — same bug class
  // as the clarify input below. Now appends the pill directly and keeps/refocuses
  // the same input node, on every path (valid tag, empty, duplicate) so Enter
  // always leaves the field in a predictable state.
  var t=(tag||'').toLowerCase().trim().replace(/\s+/g,'-').replace(/[^a-z0-9\-]/g,'').slice(0,24);
  var inp=document.getElementById('pb-ti-'+id);
  var suggEl=document.getElementById('pb-ts-'+id);if(suggEl)suggEl.innerHTML='';
  if(!t){if(inp){inp.value='';inp.focus();}return;}
  var card=pbGetCardById(id);if(!card){if(inp){inp.value='';inp.focus();}return;}
  if((card.tags||[]).indexOf(t)>=0){if(inp){inp.value='';inp.focus();}return;} // already tagged
  card.tags=card.tags||[];card.tags.push(t);
  card.clarifyChain=card.clarifyChain||[];
  card.clarifyChain.push({text:'Tag added: #'+t,author:'system',timestamp:pbNow()});
  card.updatedAt=pbNow();card.updatedBy='TB';
  pbAddTagToReg(t);pbSaveCard(card);
  var pillsHost=document.getElementById('pb-atags-'+id);
  if(pillsHost)pillsHost.insertAdjacentHTML('beforeend',_pbTagPillHtml(id,t)); // append, don't rebuild
...
function pbAddTagI(id){
  ...
  pbAddTag(id,inp.value);
}
function pbRemoveTag(id,tag){
  // FIX (v5.7.2.2): removing the Verified tag reset card.backtranslate.verdict in
  // data but never re-rendered the BT panel — so the "Sounds Good" radio stayed
  // visually checked even though the underlying verdict was cleared. Also logged
  // nothing to clarifyChain for ANY tag removal. Both fixed: every removal logs an
  // entry, and a Verified-tag removal additionally resets the verdict, logs that
  // specifically, and re-renders the BT panel so the radio state matches reality.
  var card=pbGetCardById(id);if(!card)return;
  card.tags=(card.tags||[]).filter(function(t){return t!==tag;});
  card.clarifyChain=card.clarifyChain||[];
  var wasVerdict=(card.backtranslate||{}).verdict||'';
  if(tag==='✓Verified'&&card.backtranslate){
    card.backtranslate.verdict='';
    card.clarifyChain.push({text:'Verdict reset to pending (Verified tag removed, was '+(wasVerdict||'unset')+')',author:'system',timestamp:pbNow()});
  }else{
    card.clarifyChain.push({text:'Tag removed: #'+tag,author:'system',timestamp:pbNow()});
  }
  card.updatedAt=pbNow();card.updatedBy='TB';
  pbSaveCard(card);
  pbReRenderCardPanel('tags',id);
  if(tag==='✓Verified')pbReRenderCardPanel('bt',id); // the actual fix — refresh the radio UI
  pbReRenderCardPanel('clarify',id);
}
// C14: inline source/target editing
function pbCommitSrcEdit(id){
  var el=document.getElementById('pbsrc-'+id);if(!el)return;
  var newSrc=(el.textContent||'').trim();
  var card=pbGetCardById(id);if(!card||!newSrc)return;
  var changed=newSrc!==card.source;
  var oldSrc=card.source;
  var isFirstFill=!oldSrc; // FIX (v5.7.2.3): a blank->text transition is card
  // creation being finished, not an edit — pbAddCard already logged "Card created".
  // Logging "Was: " (blank) + "Verdict reset" here on top of that was confusing a
  // brand-new entry with modifying an existing one. Only genuine edits (oldSrc
  // already had content) get the Was/reset pair now.
  card.clarifyChain=card.clarifyChain||[];
  if(!isFirstFill){
    if(changed){
      card.clarifyChain.push({text:'Was: '+oldSrc,author:'system',timestamp:pbNow()});
      var oldVerdict=(card.backtranslate||{}).verdict||'';
      if(!card.backtranslate)card.backtranslate={};
      card.backtranslate.verdict='pending';
      card.clarifyChain.push({text:'Verdict reset to pending'+(oldVerdict&&oldVerdict!=='pending'?' (was '+oldVerdict+')':''),author:'system',timestamp:pbNow()});
    }
  }else if(!card.backtranslate){
    card.backtranslate={verdict:'pending'};
  }
  card.source=newSrc;card.updatedAt=pbNow();card.updatedBy='TB';
  if(changed)delete card._targetOverride;
  pbSaveCard(card);
  pbReRenderBubbleHeader(id);
  pbReRenderCardPanel('clarify',id);
  pbReRenderCardPanel('bt',id);
  if(!changed){if(!((card.backtranslate||{}).resultText||'').trim())pbRunBT(id);return;}
  translateWithRetry(newSrc,card.sourceLang,card.targetLang,2).then(function(tr){
    if(tr&&tr!==newSrc){card.target=tr;card.updatedAt=pbNow();pbSaveCard(card);
      var te=document.getElementById('pbtgt-'+id);if(te)te.textContent=tr;}
    // I: open BT panel if closed, then run BT
    var cs2=_pbCS(id);
    if(!cs2.btOpen){cs2.btOpen=true;pbSyncPanel('bt',id);}
    pbRunBT(id);
  });
}
function pbCommitTgtEdit(id){
  // FIX (v5.7.2.2): this logged nothing at all — editing the target silently changed
  // the card with no clarify trail, unlike source edits. Mirrors pbCommitSrcEdit now:
  // logs "Target was: X" and resets verdict to pending with its own logged entry.
  // FIX (v5.7.2.3): skip that pair when the target was blank (first fill on a new
  // card) — same "creation isn't an edit" reasoning as the source-field fix.
  var el=document.getElementById('pbtgt-'+id);if(!el)return;
  var newTgt=(el.textContent||'').trim();
  var card=pbGetCardById(id);if(!card||!newTgt||newTgt===card.target)return;
  var oldTgt=card.target;
  var isFirstFill=!oldTgt;
  card.clarifyChain=card.clarifyChain||[];
  if(!isFirstFill){
    card.clarifyChain.push({text:'Target was: '+oldTgt,author:'system',timestamp:pbNow()});
    var oldVerdict=(card.backtranslate||{}).verdict||'';
    if(!card.backtranslate)card.backtranslate={};
    card.backtranslate.verdict='pending';
    card.clarifyChain.push({text:'Verdict reset to pending'+(oldVerdict&&oldVerdict!=='pending'?' (was '+oldVerdict+')':''),author:'system',timestamp:pbNow()});
  }
  card.target=newTgt;card._targetOverride=true;card.updatedAt=pbNow();card.updatedBy='TB';
  pbSaveCard(card);
  pbReRenderBubbleHeader(id);
  pbReRenderCardPanel('clarify',id);
  pbRunBT(id);
...
/* ── Clarify panel ── */
function _pbClarifyItemHtml(item,id,i){
  // FIX (v5.7.2.2): real pulled data has clarify entries in at least three field-name
  // conventions from different points in the project's history: {ini,ts,txt} (oldest),
  // {text,ts,by} (mixed), {text,author,timestamp} (current). This only ever read the
  // current one — older entries pulled from GitHub rendered as blank bubbles with no
  // author and no date. Now falls back across all three per field.
  var author=item.author||item.by||item.ini||'';
  var rawTs=item.timestamp||item.ts;
  var text=item.text||item.txt||'';
  var tsStr='';
  if(rawTs){
    var d=(typeof rawTs==='number')?new Date(rawTs):new Date(rawTs); // numeric epoch or a pre-formatted string both parse
    tsStr=isNaN(d.getTime())?String(rawTs):d.toLocaleString([],{month:'short',day:'numeric',hour:'2-digit',minute:'2-digit'});
  }
  return'<div class="pb-clarify-item"><div class="pb-clarify-meta" style="display:flex;align-items:center;gap:5px;">'
    +'<span class="pb-clarify-author">'+pbEsc(author)+'</span>'
    +(tsStr?'<span style="font-size:10px;color:#b0aba8;">'+pbEsc(tsStr)+'</span>':'')
    ...
    +'</div><div class="pb-clarify-body">'+pbEsc(text)+'</div></div>';
}
function pbClarifyPanelHtml(card){
  var id=card.id;var chain=card.clarifyChain||[];
  var thread=chain.length?chain.map(function(item,i){return _pbClarifyItemHtml(item,id,i);}).join(''):'<div style="color:#aaa;font-size:12px;padding:4px 0;">No clarifications yet.</div>';
...
function pbAddClarify(id){
  // FIX (v5.7.2.1): previously called pbReRenderCardPanel('clarify',id) here,
  // which set thread.innerHTML=... and rebuilt #pb-ci-{id} from scratch on every
  // Enter — so the input you just typed into no longer existed by the time you
  // typed the next entry. Looked like "Enter does nothing" after the first note.
  // Now appends the new entry directly and refocuses the SAME input node.
  var inp=document.getElementById('pb-ci-'+id);if(!inp)return;
  var text=(inp.value||'').trim();if(!text)return;
  var card=pbGetCardById(id);if(!card)return;
  card.clarifyChain=card.clarifyChain||[];
  var idx=card.clarifyChain.length;
  card.clarifyChain.push({text:text,author:'Me',timestamp:pbNow()});
  pbSaveCard(card);
  inp.value='';
  var thread=document.getElementById('pb-cthread-'+id);
  if(thread){
    var noneMsg=thread.querySelector('div[style*="No clarifications yet"]');if(noneMsg)noneMsg.remove();
    thread.insertAdjacentHTML('beforeend',_pbClarifyItemHtml({text:text,author:'Me',timestamp:card.clarifyChain[idx].timestamp},id,idx)); // append, don't rebuild
    thread.scrollTop=thread.scrollHeight;
  }
  inp.focus(); // the actual fix — without this, Enter appears to do nothing on the next keystroke
}
...
function pbBtPanelHtml(card){
  ...
  var resultHtml=result?('<div style="margin:6px 0;"><span style="font-style:italic;color:#555;font-size:13px;">'+pbEsc(result)+'</span></div>'):('<div style="color:#aaa;font-size:12px;">No result yet.</div>');
  var verdictHtml='';
  // FIX (v5.7.2.1): previously rendered as click-buttons (onclick="pbSetVerdict(...)"),
  // which was the wrong markup entirely — the actual spec is a hidden radio pair with
  // an adjacent <label>, state shown via CSS :checked+label sibling selectors
  // (.radio-good:checked+.verdict-label / .radio-flag:checked+...). Rebuilt to match.
  if(result){
    verdictHtml='<div class="pb-vrow">'
      +'<input type="radio" name="verdict-'+id+'" id="vg-'+id+'" class="verdict-radio radio-good"'+(verdict==='good'?' checked':'')+' onchange="pbSetVerdict(\''+id+'\',\'good\')">'
      +'<label for="vg-'+id+'" class="verdict-label">✓ Sounds Good</label>'
      +'<input type="radio" name="verdict-'+id+'" id="vf-'+id+'" class="verdict-radio radio-flag"'+(verdict==='flag'?' checked':'')+' onchange="pbSetVerdict(\''+id+'\',\'flag\')">'
      +'<label for="vf-'+id+'" class="verdict-label">⚑ Flag</label>'
      +'</div>';
  }
  // FIX (v5.7.2.11): "Back-translate" text label removed per direction — added
  // nothing, implied clickability the way a button reads, but BT already re-runs
  // automatically on every source/target commit. Icon-only refresh affordance kept
  // for a genuine manual re-run. Less is more.
  return'<div style="padding:8px 10px;">'
...
function pbSetVerdict(id,val){
  // FIX (v5.7.2.2): three bugs here. (1) The 'good' branch added the Verified tag
  // but 'flag' never removed it on a good->flag switch -- tag and verdict could
  // disagree. (2) Only 'good' logged a clarify entry; 'flag' logged nothing.
  // (3) The clarify entry it did log used {ts,by} fields, but the renderer reads
  // {timestamp,author} -- that entry displayed with a blank author and no date.
  var card=pbGetCardById(id);if(!card)return;
  ...
  card.clarifyChain=card.clarifyChain||[];
  if(val==='good'){
    if(tags.indexOf('✓Verified')<0)card.tags=tags.concat('✓Verified');
    card.clarifyChain.push({text:'Back-translate verified',author:'system',timestamp:pbNow()});
  }else if(val==='flag'){
    card.tags=tags.filter(function(t){return t!=='✓Verified';}); // mirror: flag removes what good added
    card.clarifyChain.push({text:'Back-translate flagged'+(oldVerdict==='good'?' (was verified)':''),author:'system',timestamp:pbNow()});
  }
  card.updatedAt=pbNow();card.updatedBy='TB';
  pbSaveCard(card);pbReRenderCardPanel('bt',id);pbReRenderCardPanel('tags',id);pbReRenderCardPanel('clarify',id);
  pbReRenderBubbleHeader(id);
...
function pbReRenderBubbleHeader(id){
  ...
  hdr.innerHTML='<span class="pb-bbl-ts">'+pbEsc(_pbHdrText(card))+'</span>';
}
...
───────────────────────────────────────────────── */
var _pbFilter='all';
var _pbTrashOpen=false;
function pbTogTrash(){
  _pbTrashOpen=!_pbTrashOpen;
  var chev=document.getElementById('pb-trash-chev');
  var list=document.getElementById('pb-trash-list');
  if(chev)chev.textContent=_pbTrashOpen?'▾':'▸';
  if(list)list.style.display=_pbTrashOpen?'block':'none';
}

var _pbLastPull=null; // set by PB-SYNC.pull on success once activated (Pre-ship+); Base has no GH sync yet
function pbRenderSyncFooter(){
  var el=document.getElementById('pb-ov-footer');if(!el)return;
  if(_pbLastPull&&_pbLastPull.filename){
    var d=new Date(_pbLastPull.pulledAt);
    var stamp=d.toLocaleDateString([],{month:'short',day:'numeric'})+' '+d.toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'});
    el.textContent=_pbLastPull.filename+' · current as of '+stamp;
  }else{
    el.textContent='No shared phrasebook loaded';
  }
}
function pbOvUpdatePairLabel(){
  var el=document.getElementById('pb-ov-pair-label');if(!el)return;
  var src=room.myLang,tgt=room.theirLang;
  if(!src||!tgt){el.textContent='';return;}
  el.textContent=pbLangFlag(src)+' '+src.toUpperCase()+' → '+pbLangFlag(tgt)+' '+tgt.toUpperCase();
}
function pbOvUpdateSyncDot(status){
  var el=document.getElementById('pb-ov-sync-dot');if(!el)return;
  var colors={idle:'#9a9592',dirty:'#e0a030',saved:'#2e9e5b',error:'#c0392b'};
  el.style.background=colors[status]||colors.idle;
  if(status==='saved'){
    setTimeout(function(){ if(_pbSyncActive()&&!PB_SYNC.isDirty()) pbOvUpdateSyncDot('idle'); },1600);
  }
}
function pbManualSave(){
  if(!_pbSyncActive()){toast('Sync not active');return;}
  if(!PB_SYNC.isDirty()){toast('Nothing to save');return;}
  pbOvUpdateSyncDot('dirty');
  PB_SYNC.writeBack().then(function(r){
    if(r&&r.status==='ok'){pbOvUpdateSyncDot('saved');toast('Saved to shared phrasebook');}
    else if(r&&r.status==='pending'){pbOvUpdateSyncDot('error');toast('Offline — will retry automatically');}
    else if(r&&r.status==='no-pat'){toast('No GitHub token set');}
  });
}
function pbOpenOverlay(){
  var d=document.getElementById('pb-overlay');if(d)d.style.display='flex';pbOvSearchX();
  _pbFilter='all';
  pbRenderOverlayFilter();pbRenderOverlay();pbRenderSyncFooter();pbOvUpdatePairLabel();
  if(_pbSyncActive())pbOvUpdateSyncDot(PB_SYNC.isDirty()?'dirty':'idle');
  setTimeout(function(){var si=document.getElementById('pb-ov-search');if(si)si.focus();},60);
}
function pbOpenOverlaySearch(q){
  var si=document.getElementById('pb-ov-search');if(si)si.value=q||'';pbOvSearchX();
  var d=document.getElementById('pb-overlay');if(d)d.style.display='flex';
  _pbFilter='all';
  pbRenderOverlayFilter();pbRenderOverlay();pbRenderSyncFooter();pbOvUpdatePairLabel();
  if(_pbSyncActive())pbOvUpdateSyncDot(PB_SYNC.isDirty()?'dirty':'idle');
  setTimeout(function(){if(si)si.focus();},60);
}
function pbOpenOverlayClean(){
  var si=document.getElementById('pb-ov-search');if(si)si.value='';
  pbOpenOverlay();
}
...
function pbRenderOverlayFilter(){
  // Catalog system is deleted (graveyard G6) — this bar is permanently empty.
  // Not conditional on data state; it never renders chips regardless of what's in storage.
  var host=document.getElementById('pb-ov-filter');if(!host)return;
  _pbFilter='all';
  host.innerHTML='';
}
function pbConfirmRemoveCat(catId){
  var cat=pbGetCats().find(function(c){return c.id===catId;});if(!cat)return;
  var n=pbGetCards().filter(function(c){return(c.catalogIds||[]).indexOf(catId)>=0;}).length;
  if(!confirm('Remove "'+cat.name+'" and its '+n+' phrase'+(n===1?'':'s')+'?\nThis cannot be undone.'))return;
  pbRemoveCat(catId);
}
function pbRemoveCat(catId){
  var all=pbGetAllCards();
  var kept=all.filter(function(c){
    var ids=(c.catalogIds||[]).filter(function(i){return i!==catId;});
    if(ids.length===(c.catalogIds||[]).length)return true;
    if(ids.length>0){c.catalogIds=ids;return true;}
    return false;
  });
  pbSaveCards(kept);
  pbSaveCats(pbGetCats().filter(function(c){return c.id!==catId;}));
  if(_pbFilter==='cat:'+catId)_pbFilter='all';
  pbRenderOverlayFilter();pbRenderOverlay();
  toast('Phrasebook removed');
}
var _pbCatFilter='all';
function pbRenderCatSummary(cardsInScope){
  var host=document.getElementById('pb-ov-cat-summary');if(!host)return;
  ...
  cardsInScope.forEach(function(c){
    (c.categories&&c.categories.length?c.categories:['unassigned']).forEach(function(cat){
      counts[cat]=(counts[cat]||0)+1;
    });
  });
  var catNames=Object.keys(counts).sort();
  if(catNames.length<=1&&_pbCatFilter==='all'){host.style.display='none';host.innerHTML='';return;}
  host.style.display='flex';
  host.innerHTML=catNames.map(function(cat){
    var active=_pbCatFilter===cat;
    return '<button class="pb-fc'+(active?' act':'')+'" onclick="pbSetCatFilter(\''+pbEsc(cat)+'\')" style="border-radius:14px;padding:4px 12px;font-size:12px;border:1px solid '+(active?'#5a5552':'#e0dbd6')+';background:'+(active?'#5a5552':'#fff')+';color:'+(active?'#fff':'#5a5552')+';cursor:pointer;">'+pbEsc(cat)+' ('+counts[cat]+')</button>';
  }).join('')
  +(_pbCatFilter!=='all'?'<button onclick="pbSetCatFilter(\'all\')" style="border:none;background:none;color:#9a9592;font-size:12px;cursor:pointer;padding:4px 6px;">Clear</button>':'');
}
function pbSetCatFilter(cat){_pbCatFilter=(cat==='all')?'all':cat;pbRenderOverlay();}
function pbRenderOverlay(){
  var host=document.getElementById('pb-ov-cards');if(!host)return;
  // FIX (v5.7.1, carried forward): this used to gate on pbGetCats().length (the
  // deleted catalog registry) instead of actual cards — with 0 catalogs the overlay
  // always said "No phrasebook loaded" no matter how many cards existed. Gates on
  // allCards.length now.
  var allCards=pbGetAllCards();
  var trashedCards=allCards.filter(function(c){return c.deletedAt;});
  if(!allCards.length&&!trashedCards.length){
    document.getElementById('pb-ov-cat-summary').innerHTML='';
  ...
  var q=((document.getElementById('pb-ov-search')||{}).value||'').trim();
  var cards=pbGetCards();
  pbRenderCatSummary(cards);
  if(_pbCatFilter!=='all'){
    cards=cards.filter(function(c){return(c.categories&&c.categories.length?c.categories:['unassigned']).indexOf(_pbCatFilter)>=0;});
  }
  ...
  if(!cards.length){
    ...
    _pbICards=cards;html=cards.map(function(c,i){return pbOvRowHtml(c,i);}).join('');
  }else{
    html=cards.map(function(c){return pbBubbleHtml(c,'overlay');}).join('');
  }
  host.innerHTML=html;
  var th=document.getElementById('pb-trash-host');
  if(th)th.innerHTML=pbTrashSectionHtml(trashedCards); // FIX v5.7.2.11: trash pinned bottom, own host
}
function pbTrashSectionHtml(trashedCards){
  if(!trashedCards||!trashedCards.length)return'';
  var tOpen=_pbTrashOpen;
  var html='<div id="pb-trash-section" style="margin:12px 0 8px;border:1px solid #e0dbd6;border-radius:10px;overflow:hidden;">'
    +'<div onclick="pbTogTrash()" style="cursor:pointer;display:flex;align-items:center;gap:8px;padding:10px 12px;background:#f8f5f2;font-size:13px;font-weight:600;color:#7a6e6a;user-select:none;-webkit-user-select:none;">'
    +'<span id="pb-trash-chev" style="width:14px;font-size:11px;">'+(tOpen?'▾':'▸')+'</span>'
    +'<span>Recently Deleted ('+trashedCards.length+')</span>'
    +'</div>'
    +'<div id="pb-trash-list" style="display:'+(tOpen?'block':'none')+';">';
  trashedCards.forEach(function(c){
    var srcT=pbEsc((c.source||'').slice(0,70));
    var tgtT=pbEsc((c.target||'').slice(0,70));
    html+='<div style="display:flex;align-items:center;gap:8px;padding:9px 12px;border-top:1px solid #ede9e4;flex-wrap:wrap;">'
      +'<div style="flex:1;min-width:0;font-size:13px;color:#5a5552;">'+srcT+(tgtT?'<span style="color:#9a9592;"> → </span>'+tgtT:'')+'</div>'
      +'<div style="display:flex;gap:6px;flex-shrink:0;">'
      +'<button onclick="pbRestoreCard(\''+c.id+'\')"  style="border:1px solid #c5d2ea;background:#f8fbff;color:#1f2937;border-radius:8px;padding:5px 10px;font-size:12px;cursor:pointer;font-family:inherit;">Restore</button>'
      +'<button onclick="pbHardDelete(\''+c.id+'\')"  style="border:1px solid #fecaca;background:#fef2f2;color:#b91c1c;border-radius:8px;padding:5px 10px;font-size:12px;cursor:pointer;font-family:inherit;">Delete Forever</button>'
      +'</div>'
      +'</div>';
  });
  html+='</div></div>';
...
function pbRenderTrashSection(){
  var trashedCards=pbGetAllCards().filter(function(c){return c.deletedAt;});
  var th=document.getElementById('pb-trash-host');
  if(th)th.innerHTML=pbTrashSectionHtml(trashedCards);
}
function pbOvSearch(){pbRenderOverlay();}
function pbOvSearchX(){var x=document.getElementById('pb-ov-search-x');if(x)x.style.display=((document.getElementById('pb-ov-search')||{}).value||'').trim()?'':'none';}
function pbOvClearSearch(){var si=document.getElementById('pb-ov-search');if(si){si.value='';si.focus();}pbOvSearchX();pbRenderOverlay();}
...
function pbAddCard(opts){
  opts=opts||{};
  // FIX v5.7.2.11: clear any stale search so the new card is visible, and dedupe —
  // saving the same phrase twice now focuses the existing card instead of duplicating.
  var _si=document.getElementById('pb-ov-search');if(_si){_si.value='';}pbOvSearchX();
  // FIX v5.7.2.11 (Post-ship 2026-07-01 correction): duplicate save was still opening/
  // scrolling the PB overlay on top of the toast -- a context switch mid-call the user
  // never asked for. Now it's toast-only: no navigation, no scroll, stays exactly
  // where the user was (e.g. still looking at the transcript).
  if(opts.source&&opts.target){
    var _dup=pbGetCards().find(function(c){return !c.deletedAt&&(c.source||'').trim().toLowerCase()===(opts.source||'').trim().toLowerCase()&&(c.target||'').trim().toLowerCase()===(opts.target||'').trim().toLowerCase();});
    if(_dup){
      if(_pbUsageActive())PB_USAGE.recordUse(_dup.id);
      toast('Already saved');
      return _dup;
    }
  }
  var card=pbNorm({
    sourceLang:opts.sourceLang||room.myLang||'en',
    targetLang:opts.targetLang||room.theirLang||'en',
    source:opts.source||'',
    target:opts.target||'',
    categories:['unassigned'],
    createdBy:'TB',updatedBy:'TB',
    createdAt:pbNow(),updatedAt:pbNow()
  });
  // FIX (v5.7.2.3): new cards had no creation entry of their own, so the first time
  // someone typed a source and hit Enter, pbCommitSrcEdit's edit-tracking logic fired
  // and logged "Was: " (blank) + "Verdict reset to pending" -- both nonsensical for
  // something that was just created, no prior state to speak of. One clear entry now.
  card.clarifyChain=[{text:'Card created',author:'system',timestamp:card.createdAt}];
  pbUpsert(card);
  if(document.getElementById('pb-overlay').style.display!=='flex')pbOpenOverlay();
  else{pbRenderOverlayFilter();pbRenderOverlay();}
  setTimeout(function(){
    var el=document.getElementById('pbsrc-'+card.id);
  ...
  if(opts.source&&opts.target){
    translateWithRetry(opts.source,card.sourceLang,card.targetLang,2).then(function(){});
  }
  return card;
}
function pbOpenNewCardForContext(){
  if(_pbFilter!=='all'&&_pbFilter.indexOf('cat:')===0){
    var catId=_pbFilter.slice(4);
    var cards=pbGetCards().filter(function(c){return(c.catalogIds||[]).indexOf(catId)>=0;});
    var srcLang=cards.length?cards[0].sourceLang:(room.myLang||'en');
    var tgtLang=cards.length?cards[0].targetLang:(room.theirLang||'en');
    pbOpenNewCard({sourceLang:srcLang,targetLang:tgtLang,_catalogIds:[catId]});
  }else{
    pbOpenNewCard({});
  }
}
var _pbNcSrcLang='en',_pbNcTgtLang='en';
function pbOpenNewCard(opts){
  opts=opts||{};
  _pbNcCatalogIds=opts._catalogIds||[];
  _pbNcVerdict='';
  _pbNcAttr=opts.attribution||'';
  var lp={src:room.myLang||'en',tgt:room.theirLang||'en'};
  if(_pbFilter.indexOf('cat:')===0){
    var _ac=pbGetCats().find(function(c){return'cat:'+c.id===_pbFilter;});
    if(_ac){var _lp=pbCatLangPair(_ac);lp={src:_lp.src,tgt:_lp.tgt};}
  }
  if(opts.sourceLang)lp.src=opts.sourceLang;
  if(opts.targetLang)lp.tgt=opts.targetLang;
  _pbNcSrcLang=lp.src;_pbNcTgtLang=lp.tgt;
  var pl=document.getElementById('pbnc-pair-label');
  if(pl)pl.textContent=pbLangFlag(lp.src)+' '+lp.src.toUpperCase()+' → '+pbLangFlag(lp.tgt)+' '+lp.tgt.toUpperCase();
  var atEl=document.getElementById('pbnc-attr');if(atEl)atEl.value=_pbNcAttr;
  var tsEl=document.getElementById('pbnc-ts');
  if(tsEl)tsEl.textContent=new Date().toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'});
  var st=document.getElementById('pbnc-src');if(st)st.textContent=opts.source||'';
  var tt=document.getElementById('pbnc-tgt');if(tt)tt.textContent=opts.target||'';
  ...
  _pbNcTags=[];_pbNcDirty=false;pbNcRenderTagChips();
  var ti=document.getElementById('pbnc-tag-inp');if(ti)ti.value='';
  var cl=document.getElementById('pbnc-clarify');if(cl)cl.value='';
  ['tags','clarify','bt'].forEach(function(p){
    var panel=document.getElementById('pbnc-panel-'+p);if(panel)panel.style.display='block';
    var btn=document.getElementById('pbnc-f'+p);if(btn)btn.classList.add('on');
  });
  document.getElementById('pb-new-card-overlay').style.display='flex';
  setTimeout(function(){var s=document.getElementById('pbnc-src');if(s)s.focus();},40);
}
function pbNcClose(){
  _pbNcDirty=false;
  document.getElementById('pb-new-card-overlay').style.display='none';
  if(document.getElementById('pb-overlay').style.display!=='flex'){
    document.getElementById('pb-overlay').style.display='flex';pbRenderOverlayFilter();pbRenderOverlay();
  }
}
function pbNcCancel(){
  var src=((document.getElementById('pbnc-src')||{}).textContent||'').trim();
  if(src||_pbNcDirty){if(!confirm('Discard this phrase?'))return;}
  pbNcClose();
}
function pbCloseNewCard(){pbNcClose();}
function pbNcAutoTranslate(){
  var src=(document.getElementById('pbnc-src')||{}).textContent||'';
  src=src.trim();if(!src)return;
  var tt=document.getElementById('pbnc-tgt');
  if(tt)tt.textContent='…';
  translateWithRetry(src,_pbNcSrcLang,_pbNcTgtLang,2).then(function(r){
    if(tt)tt.textContent=r||'';
    pbNcRunBt();
  });
}
function pbNcSrcKeydown(e){_pbNcDirty=true;if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();pbNcAutoTranslate();}}
function pbNcOnSrcBlur(){
  var src=(document.getElementById('pbnc-src')||{}).textContent||'';
  src=src.trim();if(!src)return;
  _pbNcDirty=true;
  pbNcAutoTranslate();
}
function pbNcSpeakSrc(){var s=(document.getElementById('pbnc-src')||{}).textContent||'';if(s.trim())speakText(s.trim(),_pbNcSrcLang);}
function pbNcSpeakTgt(){var s=(document.getElementById('pbnc-tgt')||{}).textContent||'';if(s.trim())speakText(s.trim(),_pbNcTgtLang);}
var _pbNcVerdict='';
var _pbNcTags=[];
var _pbNcDirty=false;
var _pbNcAttr='';
function pbNcRenderTagChips(){
  var host=document.getElementById('pbnc-tag-chips');if(!host)return;
  host.innerHTML=_pbNcTags.map(function(t,i){
    return '<span style="display:inline-flex;align-items:center;gap:3px;background:#e8f5f5;color:#2e8b8b;border-radius:999px;padding:2px 8px;font-size:12px;">'
      +pbEsc(t)
      +'<button onclick="pbNcRemoveTag('+i+')" style="background:none;border:none;cursor:pointer;color:#2e8b8b;font-size:11px;padding:0 0 0 2px;line-height:1;">×</button>'
      +'</span>';
  }).join('');
}
function pbNcAddTag(tag){
  tag=(tag||'').trim().toLowerCase();
  if(!tag||_pbNcTags.indexOf(tag)>=0)return;
  _pbNcTags.push(tag);_pbNcDirty=true;pbNcRenderTagChips();
  var inp=document.getElementById('pbnc-tag-inp');if(inp)inp.value='';
  var sugg=document.getElementById('pbnc-tag-sugg');if(sugg)sugg.style.display='none';
}
function pbNcRemoveTag(idx){_pbNcTags.splice(idx,1);_pbNcDirty=true;pbNcRenderTagChips();}
function pbNcTagKey(e){
  if(e.key==='Enter'||e.key===','){e.preventDefault();
    pbNcAddTag(document.getElementById('pbnc-tag-inp').value);}
}
function pbNcTagSugg(){
  var val=(document.getElementById('pbnc-tag-inp').value||'').trim().toLowerCase();
  var host=document.getElementById('pbnc-tag-sugg');if(!host)return;
  if(!val){host.style.display='none';return;}
  var all=pbGetTags().filter(function(t){return t.toLowerCase().indexOf(val)>=0&&_pbNcTags.indexOf(t)<0;});
  var rows=all.slice(0,7).map(function(t){
    return '<div style="padding:6px 10px;cursor:pointer;font-size:13px;" onclick="pbNcAddTag(\''+pbEsc(t)+'\')">#'+pbEsc(t)+'</div>';
  });
  if(val&&_pbNcTags.indexOf(val)<0&&all.indexOf(val)<0){
    rows.push('<div style="padding:6px 10px;cursor:pointer;font-size:13px;color:#2e8b8b;font-style:italic;" onclick="pbNcAddTag(\''+pbEsc(val)+'\')">+ add #'+pbEsc(val)+'</div>');
  }
  if(!rows.length){host.style.display='none';return;}
  host.innerHTML=rows.join('');host.style.display='block';
}
function pbNcConfirmDiscard(){
  _pbNcDirty=false;
  document.getElementById('pb-new-card-overlay').style.display='none';
}
function pbNcRunBt(){
  _pbNcDirty=true;
  var tgt=(document.getElementById('pbnc-tgt')||{}).textContent||'';
  tgt=tgt.trim();if(!tgt)return;
  var br=document.getElementById('pbnc-bt-result');
  if(br)br.textContent='Translating…';
  translateWithRetry(tgt,_pbNcTgtLang,_pbNcSrcLang,2).then(function(r){
...
function pbNcTogPanel(type){
  var panel=document.getElementById('pbnc-panel-'+type);
  var btn=document.getElementById('pbnc-f'+type);
  if(!panel)return;
...
function pbNcSetVerdict(v){
  _pbNcVerdict=v;_pbNcDirty=true;
...
function pbSaveNewCard(){
  ...
  var btObj=validBt?{sourceLang:_pbNcTgtLang,targetLang:_pbNcSrcLang,inputText:tgt,resultText:btResult,verdict:_pbNcVerdict,contentHash:src+'||'+tgt,updatedAt:pbNow()}:{};
  var card=pbNorm({sourceLang:_pbNcSrcLang,targetLang:_pbNcTgtLang,source:src,target:tgt,
    catalogIds:_pbNcCatalogIds,tags:tagsRaw,backtranslate:btObj,
    clarifyChain:clarifyNote?[{text:clarifyNote,ts:pbNow(),by:savedBy}]:[],
    savedBy:savedBy,createdAt:pbNow(),updatedAt:pbNow()});
  pbUpsert(card);
  pbCloseNewCard();
  var pairLabel=pbLangFlag(_pbNcSrcLang)+' '+_pbNcSrcLang.toUpperCase()+' → '+pbLangFlag(_pbNcTgtLang)+' '+_pbNcTgtLang.toUpperCase();
  var verLabel=_pbNcVerdict==='good'?' · verified':_pbNcVerdict==='flag'?' · flagged':' · not verified';
  toast('"'+(src.length>30?src.slice(0,30)+'…':src)+'" added to '+pairLabel+verLabel);
  _pbNcDirty=false;
  pbRenderOverlayFilter();pbRenderOverlay();
  document.getElementById('pb-overlay').style.display='flex';
  // pbPushCardToRepo REMOVED: it wrote to phrasebook-{src}-{tgt}-default.json, a
  // completely different file from what PB_SYNC reads/writes (the versioned
  // phrasebook-{src}-{tgt}-{NNNN}.json series). New cards were vanishing into a
  // file the real sync cycle never looks at again -- split-brain data, and the
  // direct cause of "can't update cards". pbUpsert (above, via pbSaveCards) already
  // calls PB_SYNC.markDirty() correctly -- the new card writes back through the
  // real versioned cycle on hangup or overlay-close like any other edit.
}
...
function translateWithRetry(text,from,to,retries){
  ...
  var tmHit=_tmCheck(text,from,to);
  if(tmHit!==null){log('tm_hit',{from:from,to:to,text:text.slice(0,40)});return Promise.resolve(tmHit);}
  var k=from+'|'+to+'|'+text;
...
function saveRecent(){
  ...
  var rec={id:room.id,name:room.name||(gL(room.myLang).flag+' ↔ '+gL(room.theirLang).flag),myLang:room.myLang,theirLang:room.theirLang,ts:Date.now()};
  if(room.role==='joiner'){
    rec.role='joiner';
  }
  rows.unshift(rec);setRecent(rows);renderRecent();
}
...
async function reuseRoom(id){
  ...
  room.id=rec.id;room.myLang=rec.myLang;room.theirLang=rec.theirLang;room.name=rec.name||'';
  if(rec.role==='joiner'){
    room.role='joiner';
  }else{
    room.role='creator';
  }
  lastSessionContext.role=room.role;lastSessionContext.myLang=rec.myLang;lastSessionContext.theirLang=rec.theirLang;lastSessionContext.roomName=rec.name||'';lastSessionContext.roomId=room.id||null;
...
function _pbSyncOnEnter(){
  if(!_pbSyncActive())return;
  if(!room.myLang||!room.theirLang)return;
  // Post-ship fix (2026-07-01): if the last session ended without a normal hangUp
  // (tab/app closed mid-call), a local edit can be sitting unsynced. Flush it to
  // GitHub first so this call's pull sees the true, current state — GH stays SOT.
  var _startup=PB_SYNC.isDirty()?PB_SYNC.writeBack():Promise.resolve();
  _startup.then(function(){
    PB_SYNC.pull(room.myLang,room.theirLang).then(function(r){
      if(r&&r.status==='no-pair-file'){toast('No shared phrasebook yet');}
      else if(r&&r.status==='ok'){pbRenderOverlayFilter();pbRenderOverlay();}
    });
  });
}
function _pbSyncOnLeave(){
  if(!_pbSyncActive())return;
  if(PB_SYNC.isDirty())PB_SYNC.writeBack();
}
async function enterCall(){
  ...
  var ci=$('chat-input');if(ci)ci.placeholder='Enter chat or /Search';
...
async function setupPC(){
  ...
  pc.onicecandidate=function(e){if(e.candidate){if(e.candidate.candidate.indexOf('.local')!==-1)return;log('rtc_ice_emit',{type:e.candidate.type||'?'});var cm={type:'webrtc-signal',transient:true,signal:{candidate:e.candidate}};savedCandidates.push(cm);relaySend(cm);}};
...
function isDupe(text){
  ...
  for(var i=0;i<recentFinals.length;i++){var p=recentFinals[i].s;if(p===lo)return true;if(p.length>8&&lo.length>8&&(p.indexOf(lo)>=0||lo.indexOf(p)>=0))return true}
  return false;
}
...
function onDGFinal(text){
  ...
  var _ftWon=false,_detected=src,_detectVia='fallback';
  Promise.race([
    _detectLangAsync(text).then(function(d){_ftWon=true;_detectVia=d?_lastAsyncVia:'fallback';return d;}),
    new Promise(function(res){setTimeout(function(){
      if(!_ftWon)log('norm_timeout',{ft_ready:_ftReady,ft_failed:_ftLoadFailed,fallback:src},'warn');
      res(detectLang(text,src));
    },150);})
  ]).then(function(detected){
    // B2 fix: if race resolved null (WASM failed fast), fall back to unicode stub
    if(!detected)detected=detectLang(text,src);
    log('norm_detect',{detected:detected||'(none)',src:src,needs_norm:!!(detected&&detected!==src),via:_detectVia});
    if(detected&&detected!==src){
      _detected=detected;
    ...
    if(_detected!==src&&normalizeText(srcText,'compare')===normalizeText(text,'compare')){src=_detected;log('norm_fallback',{detected:_detected},'warn');}
    ...
    return translateWithRetry(srcText,src,tgt,2).then(function(tr){
      var trFailed=(src!==tgt&&(!tr||normalizeText(tr,'compare')===normalizeText(srcText,'compare')));
      if(trFailed&&src!==tgt){
        return translateWithRetry(srcText,src,tgt,0).then(function(tr2){
          if(tr2&&normalizeText(tr2,'compare')!==normalizeText(srcText,'compare')){tr=tr2;trFailed=false;}
          tr=normalizeText(tr,'speech')||srcText;
          if(trFailed){log('trans_failed_inline',{ss:ss,src:src,tgt:tgt},'warn');}
          else{log('trans_result',{from:src,to:tgt,tr:tr.slice(0,60),ss:ss},'ok');}
          relaySend({type:'subtitle-update',subtitleSeq:ss,text:tr,sourceText:srcText,sourceLang:src,targetLang:tgt});
          patchTr('me',ss,tr,src,tgt,trFailed);
        });
      }
      tr=normalizeText(tr,'speech')||srcText;
      if(trFailed){log('trans_failed_inline',{ss:ss,src:src,tgt:tgt},'warn');}
...
function stopDeepgram(){
  _stopDgWatchdog();
  dgActive=false;if(_dgPrimHoldTimer){clearTimeout(_dgPrimHoldTimer);_dgPrimHoldTimer=null;_dgPrimHeldText=null;}if(dgWs){try{dgWs.close()}catch(_){}dgWs=null;}if(dgWsEn){try{dgWsEn.close()}catch(_){}dgWsEn=null;}stopDGAudio();log('dg_stopped',{});
}
function startDeepgram(){
  ...
  var DG_MULTI_LANGS=['en','es','fr','de','pt','it','nl','ru','hi','ja'];
  var langCode=(DG_MULTI_LANGS.indexOf(room.myLang)>=0)?'multi':(DG_LANGS[room.myLang]||room.myLang||'en-US');
  log('dg_lang_select',{role:room.role,myLang:room.myLang,theirLang:room.theirLang,dgLang:langCode});
  ...
  dgWs.onmessage=function(e){
    ...
    try{
      ...
      if(alt&&alt.transcript&&d.is_final){
        if(dgWsEn){
          if(Date.now()-_dgEnLastFiredAt<_DG_CROSS_MS){log('dg_cross_suppress',{t:alt.transcript.slice(0,40)},'warn');return;}
          if(_dgPrimHoldTimer)clearTimeout(_dgPrimHoldTimer);
          var _pt=alt.transcript;_dgPrimHeldText=_pt;
          _dgPrimHoldTimer=setTimeout(function(){_dgPrimHoldTimer=null;if(_dgPrimHeldText===_pt){onDGFinal(_pt);_dgPrimHeldText=null;}},250);
  ...
  var DG_DUAL_LANGS=['zh','th','ko','ar'];
  if(DG_DUAL_LANGS.indexOf(room.myLang)>=0){
    ...
    dgWsEn.onmessage=function(e){
      _dgLastMsg=Date.now();
      try{var d=JSON.parse(e.data);var alt=d.channel&&d.channel.alternatives&&d.channel.alternatives[0];if(alt&&alt.transcript&&d.is_final){if(alt.transcript.trim().length>10){_dgEnLastFiredAt=Date.now();if(_dgPrimHoldTimer){clearTimeout(_dgPrimHoldTimer);_dgPrimHoldTimer=null;log('dg_cross_suppress',{held:_dgPrimHeldText&&_dgPrimHeldText.slice(0,40)},'warn');_dgPrimHeldText=null;}}onDGFinal(alt.transcript);}}catch(_){}
    };
    dgWsEn.onerror=function(){log('dg_en_error',{},'error');};
    dgWsEn.onclose=function(ev){log('dg_en_close',{code:ev.code,captureEpoch:captureEpoch,current:sessionEpoch},'warn');dgWsEn=null;};
    log('dg_en_open',{myLang:room.myLang},'ok');
  }
}
function _stopDgWatchdog(){clearInterval(_dgWatchdogTimer);_dgWatchdogTimer=null;}
function _micLevelStart(){
  if(!dgAudioCtx||!dgSrc)return;
  _micAnalyser=dgAudioCtx.createAnalyser();_micAnalyser.fftSize=256;
  dgSrc.connect(_micAnalyser);
  var buf=new Uint8Array(_micAnalyser.frequencyBinCount);
  function _tick(){
    _micLvlRaf=requestAnimationFrame(_tick);
    if(micMutedByUser||!dgActive){_micLvlSmooth=0;_applyMicGlow(0);return;}
    _micAnalyser.getByteFrequencyData(buf);
    var sum=0;for(var i=0;i<8;i++)sum+=buf[i];
    var rms=Math.sqrt(sum/8)/255;
    _micLvlSmooth=_micLvlSmooth*0.8+rms*0.2;
    _applyMicGlow(_micLvlSmooth);
  }
  _tick();
}
function _micLevelStop(){
  if(_micLvlRaf){cancelAnimationFrame(_micLvlRaf);_micLvlRaf=null;}
  _micLvlSmooth=0;_applyMicGlow(0);
  if(_micAnalyser){try{_micAnalyser.disconnect();}catch(_){}}_micAnalyser=null;
}
function _applyMicGlow(level){
  var btn=$('top-mic-btn');if(!btn)return;
  if(level<0.01){btn.style.boxShadow='';return;}
  var r=Math.round(level*40);var a=Math.min(level*0.7,0.7).toFixed(2);
  btn.style.boxShadow='0 0 '+r+'px '+(r>>1)+'px rgba(39,174,96,'+a+')';
}
...
function patchTr(who,ss,newTr,sL,tL,failed){
  ...
  for(var i=transcript.length-1;i>=0;i--){
    if(transcript[i].who===who&&transcript[i].subtitleSeq===ss){
      if(normalizeText(transcript[i].translatedText,'compare')===normalizeText(newTr,'compare')&&!failed)return;
      transcript[i].translatedText=newTr;transcript[i].ts=Date.now();
      if(failed)transcript[i].translationFailed=true;
      saveTr();
      replaceTrDomById(transcript[i].id,transcript[i]);
      if(transcriptTtsOn&&who==='partner'&&newTr&&!failed)speakText(newTr,tL||room.myLang);
      return;
    }
  }
...
function renderMd(txt){
  ...
  var pbPfx='';
  if(txt.slice(0,4)==='[PB]'){
    pbPfx='<span style="color:#9a9592;line-height:0;display:inline-flex;align-items:center;vertical-align:middle;margin-right:3px;">'+ICO.book+'</span>';
    txt=txt.slice(4).replace(/^ /,'');
  }
...
function trHtml(e){
  ...
  function thaiWrap(text,lang){return lang==='th'?'<span class="thai-overlay">'+esc(text)+'</span>':esc(text);}
  var eid=e.id||(e.ts+'_'+e.who);
  var attachHtml='';
  if(e.attachment&&e.attachment.dataUrl){
    var att=e.attachment;var isImg=/^image\//.test(att.type||'');
  ...
  return '<div class="tr-entry" data-id="'+esc(eid)+'" onclick="trTogActive(this)"><div class="tr-bubble'+chatClass+'">'
    +'<div class="tr-head">'
    +'<div class="tr-who '+(isMine?'mine':'')+'">'+esc(speaker)+'</div>'
    +'<div class="tr-head-acts">'
    +'<button class="tr-act-btn" onclick="event.stopPropagation();trSaveToPb(this)" title="Save to phrasebook">'+ICO.save+'</button>'
    +'<button class="tr-act-btn" onclick="event.stopPropagation();trDeleteEntry(this)" title="Delete"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14H6L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/><path d="M9 6V4h6v2"/></svg></button>'
    +'<button class="tr-act-btn" onclick="event.stopPropagation();trOpenClarify(this)" title="Clarify"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg></button>'
    +'</div>'
    +'<div class="tr-time">'+esc(ts)+'</div>'
    +'</div>'
    +'<div class="tr-body">'
    ...
    +'<button class="tr-tts" type="button" data-tts-text="'+esc(leftText)+'" data-tts-lang="'+esc(leftLang)+'" title="Play" onclick="event.stopPropagation();speakText(this.dataset.ttsText||\'\'  ,this.dataset.ttsLang||room.myLang||\'en\')">'+TTS_SVG+'</button>'
    +'</div></div>'
    ...
    +'<button class="tr-tts" type="button" data-tts-text="'+esc(rightText)+'" data-tts-lang="'+esc(rightLang)+'" title="Play" onclick="event.stopPropagation();speakText(this.dataset.ttsText||\'\'  ,this.dataset.ttsLang||room.myLang||\'en\')">'+TTS_SVG+'</button>'
...
function trTogActive(el){
  var was=el.classList.contains('tr-active');
  document.querySelectorAll('.tr-entry.tr-active').forEach(function(e){e.classList.remove('tr-active');});
  if(!was)el.classList.add('tr-active');
}
function _trEntryFromBtn(btn){
  var row=btn.closest('.tr-entry');if(!row)return null;
  var id=row.dataset.id;
  return transcript.find(function(e){return (e.id||(e.ts+'_'+e.who))===id;})||null;
}
function trSaveToPb(btn){
  var entry=_trEntryFromBtn(btn);if(!entry)return;
  pbAddCard({source:entry.sourceText||'',target:entry.translatedText||'',
    sourceLang:entry.srcLang,targetLang:entry.tgtLang});
}
function trDeleteEntry(btn){
  var entry=_trEntryFromBtn(btn);if(!entry)return;
  if(!confirm('Delete this transcript entry?'))return;
  var id=entry.id||(entry.ts+'_'+entry.who);
  transcript=transcript.filter(function(e){return (e.id||(e.ts+'_'+e.who))!==id;});
  saveTr();
  var row=btn.closest('.tr-entry');if(row)row.remove();
}
var _trClarEntry=null;
function trOpenClarify(btn){
  var entry=_trEntryFromBtn(btn);if(!entry)return;
  _trClarEntry=entry;
  var orig=document.getElementById('tr-clar-orig');
  if(orig)orig.textContent='Original: "'+((entry.sourceText||'').slice(0,60))+'" → "'+(entry.translatedText||'').slice(0,60)+'"';
  var inp=document.getElementById('tr-clar-inp');
  if(inp){inp.value=entry.translatedText||'';inp.oninput=function(){
    var v=inp.value.trim();if(!v||!_trClarEntry)return;
    translateWithRetry(v,_trClarEntry.tgtLang||'en',_trClarEntry.srcLang||'en',1).then(function(r){
      var bt=document.getElementById('tr-clar-bt');if(bt)bt.textContent='Back: '+(r||'');
    });
  };}
  document.getElementById('tr-clar-bt').textContent='';
  document.getElementById('tr-clarify-modal').style.display='flex';
  setTimeout(function(){if(inp)inp.focus();},40);
}
function trSendClarify(){
  var inp=document.getElementById('tr-clar-inp');
  var text=(inp?inp.value:'').trim();if(!text)return;
  document.getElementById('tr-clarify-modal').style.display='none';
  var ta=$('chat-input');if(!ta)return;
...
function appendTrDom(entry){
  ...
  if(c)requestAnimationFrame(function(){c.scrollTop=c.scrollHeight;setTranscriptMore(false);});
}
...
function renderTr(){
  ...
  var toRender=transcript.slice(-120);
  var newHtml='';
  ...
  if(newHtml){var em=b.querySelector('.tr-empty');if(em)em.remove();b.insertAdjacentHTML('beforeend',newHtml);}
  while(b.children.length>120){b.firstChild.remove();}
  var c=$('call-transcript');
  if(c)requestAnimationFrame(function(){c.scrollTop=c.scrollHeight;setTranscriptMore(false);});
}

function checkAutoScroll(){
  var c=$('call-transcript');if(!c)return;
  if(transcriptNearBottom()){c.scrollTop=c.scrollHeight;setTranscriptMore(false);}else{setTranscriptMore(true);}
}
function transcriptNearBottom(){var c=$('call-transcript');if(!c)return true;return c.scrollTop+c.clientHeight>=c.scrollHeight-80;}
...
function scrollToLatest(){var c=$('call-transcript');if(c){c.scrollTop=c.scrollHeight;setTranscriptMore(false);}}
...
function showThankYou(msg){
  ...
  var host_lang=room.myLang||'en'; // capture before teardown
  function bil(key){var a=L(key,ty_lang),b=L(key,host_lang);return(a&&b&&a!==b)?a+' / '+b:a||b;}
...
function cleanUp(){
  ...
  $('subtitle-area').innerHTML='';$('no-video-msg').style.display='';hidePartnerDisconnected();
...
// === INIT ===
log('cred_restore',{dg:!!localStorage.getItem('tb_dg_key'),cf_tid:!!localStorage.getItem('tb_cf_tid'),cf_tok:!!localStorage.getItem('tb_cf_tok'),rooms:!!localStorage.getItem(RK)});
...
/* ============================================================
   TURN 06 PRE-SHIP — ENGINE GROUP (9 modules, ALL DORMANT)
   CONFIG, LOG, STORE, RELAY, RTC, STT, TRANSLATE, LANGDETECT, NORMALIZE
   Every module: in/out fixed; every public method logs
   MODULE.method:in / :out / :err; every use.* flag = false;
   nothing below is called anywhere yet — purely additive, inert.
   ============================================================ */

/* ===== #new module: CONFIG ===== */
/* checksum-before: n/a-new */
var CONFIG = (function(){
  var _store = {
    'relay.base':'talk-signal.myacctfortracking.workers.dev/signal',
    'relay.ws':'wss://',
    'relay.app':'talk-say-v1',
    'storageKeys': ['tb_cf_tid','tb_cf_tok','tb_dev','tb_dg_key','tb_gh_pat','tb_my_lang','tb_their_lang','say_cards'],
    'font.scale': 1.0,
    'theme.preset': 'dark',
    'use.CONFIG': false,
    'use.LOG': false,
    ...
    'use.PB_QUERY': true,
    'use.PB_RENDER': false,
    'use.COMPOSE_SEAM': true
  };
  var _subs = [];
  function _log(ev,d,l){ if(typeof log==='function') log(ev,d,l); }
  return {
    get: function(k){
      _log('CONFIG.get:in',{k:k});
      try{ var out=_store[k]; _log('CONFIG.get:out',{out:out}); return out; }
      catch(e){ _log('CONFIG.get:err',{in:{k:k},e:String(e)},'warn'); return undefined; }
    },
    getAll: function(){
      _log('CONFIG.getAll:in',{});
      try{ var out=Object.assign({},_store); _log('CONFIG.getAll:out',{out:out}); return out; }
      catch(e){ _log('CONFIG.getAll:err',{in:{},e:String(e)},'warn'); return {}; }
    },
    set: function(k,v){
      _log('CONFIG.set:in',{k:k,v:v});
      try{
        _store[k]=v;
        _subs.forEach(function(fn){ try{ fn(k,v); }catch(_){} });
        var out={k:k,v:v};
        _log('CONFIG.set:out',{out:out});
        return out;
      }catch(e){ _log('CONFIG.set:err',{in:{k:k,v:v},e:String(e)},'warn'); return null; }
    },
    subscribe: function(fn){
      _log('CONFIG.subscribe:in',{});
      try{ _subs.push(fn); var out=true; _log('CONFIG.subscribe:out',{out:out}); return out; }
      catch(e){ _log('CONFIG.subscribe:err',{in:{},e:String(e)},'warn'); return false; }
    }
  };
})();
/* ===== #end module: CONFIG ===== */
...
/* ===== #new module: LOG ===== */
/* checksum-before: n/a-new (wraps existing log()/debugLog/openLog/clearLogText, byte-unchanged) */
var LOG = {
  log: function(ev,d,l){
    try{
      if(typeof log==='function') log(ev,d,l);
      return {ev:ev,d:d,l:l||'info'};
    }catch(e){
      return {ev:ev,d:{e:String(e)},l:'warn'};
    }
  },
  open: function(){
    LOG.log('LOG.open:in',{});
    try{ if(typeof openLog==='function') openLog(); var out=true; LOG.log('LOG.open:out',{out:out}); return out; }
    catch(e){ LOG.log('LOG.open:err',{in:{},e:String(e)},'warn'); return false; }
  },
  copy: function(){
    LOG.log('LOG.copy:in',{});
    try{
      var out=false;
      if(typeof copyLogText==='function'){ copyLogText(); out=true; }
      LOG.log('LOG.copy:out',{out:out}); return out;
    }catch(e){ LOG.log('LOG.copy:err',{in:{},e:String(e)},'warn'); return false; }
  },
  clear: function(){
    LOG.log('LOG.clear:in',{});
    try{ if(typeof clearLogText==='function') clearLogText(); var out=true; LOG.log('LOG.clear:out',{out:out}); return out; }
    catch(e){ LOG.log('LOG.clear:err',{in:{},e:String(e)},'warn'); return false; }
  }
};
/* ===== #end module: LOG ===== */

/* ===== #new module: STORE ===== */
/* checksum-before: n/a-new (wraps localStorage; the engine still touches localStorage directly until activation) */
var STORE = {
  get: function(k){
    LOG.log('STORE.get:in',{k:k});
    try{ var out=localStorage.getItem(k); LOG.log('STORE.get:out',{out:out}); return out; }
    catch(e){ LOG.log('STORE.get:err',{in:{k:k},e:String(e)},'warn'); return null; }
  },
  set: function(k,v){
    LOG.log('STORE.set:in',{k:k,v:v});
    try{ localStorage.setItem(k,v); var out={k:k}; LOG.log('STORE.set:out',{out:out}); return out; }
    catch(e){ LOG.log('STORE.set:err',{in:{k:k,v:v},e:String(e)},'warn'); return null; }
  },
  remove: function(k){
    LOG.log('STORE.remove:in',{k:k});
    try{ localStorage.removeItem(k); var out={k:k}; LOG.log('STORE.remove:out',{out:out}); return out; }
    catch(e){ LOG.log('STORE.remove:err',{in:{k:k},e:String(e)},'warn'); return null; }
  }
};
/* ===== #end module: STORE ===== */

/* ===== #new module: RELAY ===== */
/* checksum-before: n/a-new (wraps connectRelay/relaySend — both byte-unchanged, see PART II §B) */
var RELAY = {
  connect: function(){
    LOG.log('RELAY.connect:in',{});
    try{ var out=(typeof connectRelay==='function')?connectRelay():null; LOG.log('RELAY.connect:out',{out:'ok'}); return out; }
    catch(e){ LOG.log('RELAY.connect:err',{in:{},e:String(e)},'warn'); return null; }
  },
  send: function(m){
    LOG.log('RELAY.send:in',{m:m});
    try{ var out=(typeof relaySend==='function')?relaySend(m):null; LOG.log('RELAY.send:out',{out:'sent'}); return out; }
    catch(e){ LOG.log('RELAY.send:err',{in:{m:m},e:String(e)},'warn'); return null; }
  },
  close: function(){
    LOG.log('RELAY.close:in',{});
    try{ if(typeof _relayWs!=='undefined'&&_relayWs) _relayWs.close(); var out=true; LOG.log('RELAY.close:out',{out:out}); return out; }
    catch(e){ LOG.log('RELAY.close:err',{in:{},e:String(e)},'warn'); return false; }
  },
  status: function(){
    LOG.log('RELAY.status:in',{});
    try{ var out=(typeof _relayWs!=='undefined'&&_relayWs)?_relayWs.readyState:-1; LOG.log('RELAY.status:out',{out:out}); return out; }
    catch(e){ LOG.log('RELAY.status:err',{in:{},e:String(e)},'warn'); return -1; }
  },
  onMessage: function(fn){
    LOG.log('RELAY.onMessage:in',{});
    try{
      var out=false;
      if(typeof _relayWs!=='undefined'&&_relayWs){ _relayWs.addEventListener('message',function(ev){ try{ fn(JSON.parse(ev.data)); }catch(_){} }); out=true; }
      LOG.log('RELAY.onMessage:out',{out:out}); return out;
    }catch(e){ LOG.log('RELAY.onMessage:err',{in:{},e:String(e)},'warn'); return false; }
  }
};
/* ===== #end module: RELAY ===== */

/* ===== #new module: RTC ===== */
/* checksum-before: n/a-new (wraps setupPC — byte-unchanged) */
var RTC = {
  start: function(roomCtx){
    LOG.log('RTC.start:in',{roomCtx:roomCtx});
    try{ var out=(typeof setupPC==='function')?setupPC():null; LOG.log('RTC.start:out',{out:'started'}); return out; }
    catch(e){ LOG.log('RTC.start:err',{in:{roomCtx:roomCtx},e:String(e)},'warn'); return null; }
  },
  stop: function(){
    LOG.log('RTC.stop:in',{});
    try{ var out=(typeof teardownSession==='function')?teardownSession('rtc','RTC.stop'):null; LOG.log('RTC.stop:out',{out:'stopped'}); return out; }
    catch(e){ LOG.log('RTC.stop:err',{in:{},e:String(e)},'warn'); return null; }
  },
  onState: function(fn){
    LOG.log('RTC.onState:in',{});
    try{ var out=true; LOG.log('RTC.onState:out',{out:out}); return out; }
    catch(e){ LOG.log('RTC.onState:err',{in:{},e:String(e)},'warn'); return false; }
  }
};
/* ===== #end module: RTC ===== */

/* ===== #new module: STT ===== */
/* checksum-before: n/a-new (wraps startDeepgram/stopDeepgram/reconcileDeepgramState/onDGFinal — byte-unchanged) */
var STT = {
  start: function(){
    LOG.log('STT.start:in',{});
    try{ var out=(typeof startDeepgram==='function')?startDeepgram():null; LOG.log('STT.start:out',{out:'started'}); return out; }
    catch(e){ LOG.log('STT.start:err',{in:{},e:String(e)},'warn'); return null; }
  },
  stop: function(){
    LOG.log('STT.stop:in',{});
    try{ var out=(typeof stopDeepgram==='function')?stopDeepgram():null; LOG.log('STT.stop:out',{out:'stopped'}); return out; }
    catch(e){ LOG.log('STT.stop:err',{in:{},e:String(e)},'warn'); return null; }
  },
  onFinal: function(fn){
    LOG.log('STT.onFinal:in',{});
    try{ var out=true; LOG.log('STT.onFinal:out',{out:out}); return out; }
    catch(e){ LOG.log('STT.onFinal:err',{in:{},e:String(e)},'warn'); return false; }
  },
  reconcile: function(reason){
    LOG.log('STT.reconcile:in',{reason:reason});
    try{ var out=(typeof reconcileDeepgramState==='function')?reconcileDeepgramState(reason):null; LOG.log('STT.reconcile:out',{out:'reconciled'}); return out; }
    catch(e){ LOG.log('STT.reconcile:err',{in:{reason:reason},e:String(e)},'warn'); return null; }
  }
};
/* ===== #end module: STT ===== */

/* ===== #new module: TRANSLATE ===== */
/* checksum-before: n/a-new (wraps translate/translateWithRetry — byte-unchanged) */
var TRANSLATE = {
  translate: function(text,src,tgt){
    LOG.log('TRANSLATE.translate:in',{text:text,src:src,tgt:tgt});
    try{
      var p=(typeof translate==='function')?translate(text,src,tgt):Promise.resolve(text);
      LOG.log('TRANSLATE.translate:out',{out:'pending'});
      return p;
    }catch(e){ LOG.log('TRANSLATE.translate:err',{in:{text:text,src:src,tgt:tgt},e:String(e)},'warn'); return Promise.resolve(text); }
  },
  backtranslate: function(text,src,tgt){
    LOG.log('TRANSLATE.backtranslate:in',{text:text,src:src,tgt:tgt});
    try{
      var p=(typeof translateWithRetry==='function')?translateWithRetry(text,src,tgt,2):Promise.resolve(text);
      LOG.log('TRANSLATE.backtranslate:out',{out:'pending'});
      return p;
    }catch(e){ LOG.log('TRANSLATE.backtranslate:err',{in:{text:text,src:src,tgt:tgt},e:String(e)},'warn'); return Promise.resolve(text); }
  }
};
/* ===== #end module: TRANSLATE ===== */

/* ===== #new module: LANGDETECT ===== */
/* checksum-before: n/a-new (wraps _detectLangAsync/_loadFastText — byte-unchanged) */
var LANGDETECT = {
  detect: function(text){
    LOG.log('LANGDETECT.detect:in',{text:text});
    try{
      var p=(typeof _detectLangAsync==='function')?_detectLangAsync(text):Promise.resolve(null);
      LOG.log('LANGDETECT.detect:out',{out:'pending'});
      return p;
    }catch(e){ LOG.log('LANGDETECT.detect:err',{in:{text:text},e:String(e)},'warn'); return Promise.resolve(null); }
  }
};
/* ===== #end module: LANGDETECT ===== */

/* ===== #new module: NORMALIZE ===== */
/* checksum-before: n/a-new (new contract; calls TRANSLATE, never the immutables directly) */
var NORMALIZE = {
  normalize: function(text,userPref,partnerLang){
    LOG.log('NORMALIZE.normalize:in',{text:text,userPref:userPref,partnerLang:partnerLang});
    try{
      var out={display:text,sent:text};
      LOG.log('NORMALIZE.normalize:out',{out:out});
      return out;
    }catch(e){ LOG.log('NORMALIZE.normalize:err',{in:{text:text,userPref:userPref,partnerLang:partnerLang},e:String(e)},'warn'); return {display:text,sent:text}; }
  }
};
/* ===== #end module: NORMALIZE ===== */

/* ===== #end Turn 06 Pre-ship — Engine group ===== */


/* ============================================================
   TURN 06 SHIP — CORE UI + SHARED SEARCH SEAM (ALL DORMANT)
   ROOM, THREAD, CALL, PB-DATA, PB-QUERY, PB-RENDER.renderRow,
   compose-strip dual chat/search seam.
   Nothing below is wired into any existing call site — purely
   additive, inert. Old functions (createRoom, joinerProceed,
   pbNorm, pbSearch, pbIRowHtml, chatKeydown, etc.) untouched
   and still live; this is new code beside them per §C.
   ============================================================ */

/* ===== #new module: ROOM ===== */
/* checksum-before: n/a-new (does not call createRoom/joinerProceed — see build log) */
var ROOM = (function(){
  var _rooms = {}; // in-memory only; dormant — no persistence wiring yet
  return {
    create: function(capability){
      LOG.log('ROOM.create:in',{capability:capability});
      try{
        if(capability!=='chat' && capability!=='chatcall'){
          var errOut={status:'error',room:null};
          LOG.log('ROOM.create:err',{in:{capability:capability},e:'invalid capability'},'warn');
          return errOut;
        }
        var id=uid();
        var token=id+'.'+Math.random().toString(36).slice(2,10);
        var rec={id:id,token:token,owner:deviceId,cap:capability,createdAt:Date.now(),joinedAt:null};
        _rooms[id]=rec;
        LOG.log('ROOM.create:out',{out:rec});
        return rec;
      }catch(e){ LOG.log('ROOM.create:err',{in:{capability:capability},e:String(e)},'warn'); return {status:'error',room:null}; }
    },
    join: function(token){
      LOG.log('ROOM.join:in',{token:token});
      try{
        var found=null;
        Object.keys(_rooms).forEach(function(id){ if(_rooms[id].token===token) found=_rooms[id]; });
        if(!found){ LOG.log('ROOM.join:out',{out:null}); return null; }
        found.joinedAt=Date.now();
        var view={id:found.id,cap:found.cap,owner:found.owner};
        LOG.log('ROOM.join:out',{out:view});
        return view;
      }catch(e){ LOG.log('ROOM.join:err',{in:{token:token},e:String(e)},'warn'); return null; }
    },
    listForOwner: function(){
      LOG.log('ROOM.listForOwner:in',{});
      try{
        var out=Object.keys(_rooms).map(function(id){return _rooms[id];}).filter(function(r){return r.owner===deviceId;});
        LOG.log('ROOM.listForOwner:out',{out:out});
        return out;
      }catch(e){ LOG.log('ROOM.listForOwner:err',{in:{},e:String(e)},'warn'); return []; }
    },
    get: function(id){
      LOG.log('ROOM.get:in',{id:id});
      try{ var out=_rooms[id]||null; LOG.log('ROOM.get:out',{out:out}); return out; }
      catch(e){ LOG.log('ROOM.get:err',{in:{id:id},e:String(e)},'warn'); return null; }
    },
    dispose: function(id){
      LOG.log('ROOM.dispose:in',{id:id});
      try{
        var existed=!!_rooms[id];
        delete _rooms[id];
        var out={id:id,disposed:existed};
        LOG.log('ROOM.dispose:out',{out:out});
        return out;
      }catch(e){ LOG.log('ROOM.dispose:err',{in:{id:id},e:String(e)},'warn'); return {id:id,disposed:false}; }
    }
  };
})();
/* ===== #end module: ROOM ===== */
...
var THREAD = (function(){
  ...
  return {
    render: function(roomId){
      LOG.log('THREAD.render:in',{roomId:roomId});
      try{ var out=_threads[roomId]||[]; LOG.log('THREAD.render:out',{out:{roomId:roomId,count:out.length}}); return out; }
      catch(e){ LOG.log('THREAD.render:err',{in:{roomId:roomId},e:String(e)},'warn'); return []; }
    },
    append: function(msg){
      LOG.log('THREAD.append:in',{msg:msg});
      try{
        if(!msg||!msg.roomId){ LOG.log('THREAD.append:err',{in:{msg:msg},e:'missing roomId'},'warn'); return null; }
        _threads[msg.roomId]=_threads[msg.roomId]||[];
        _threads[msg.roomId].push(msg);
        LOG.log('THREAD.append:out',{out:{roomId:msg.roomId,count:_threads[msg.roomId].length}});
        return msg;
      }catch(e){ LOG.log('THREAD.append:err',{in:{msg:msg},e:String(e)},'warn'); return null; }
    },
    postSystem: function(roomId,marker){
      LOG.log('THREAD.postSystem:in',{roomId:roomId,marker:marker});
      try{
        var m={roomId:roomId,system:true,text:marker,ts:Date.now()};
        _threads[roomId]=_threads[roomId]||[];
        _threads[roomId].push(m);
        LOG.log('THREAD.postSystem:out',{out:m});
        return m;
      }catch(e){ LOG.log('THREAD.postSystem:err',{in:{roomId:roomId,marker:marker},e:String(e)},'warn'); return null; }
    }
  };
})();
/* ===== #end module: THREAD ===== */
...
/* ===== #new module: CALL ===== */
/* checksum-before: n/a-new (dormant; not wired to RTC/STT/TRANSLATE/PB overlay mount points — see build log) */
var CALL = (function(){
  var _mounted = null;
  return {
    mount: function(roomCtx){
      LOG.log('CALL.mount:in',{roomCtx:roomCtx});
      try{
        if(!roomCtx||!roomCtx.id){ LOG.log('CALL.mount:err',{in:{roomCtx:roomCtx},e:'missing roomCtx.id'},'warn'); return null; }
        _mounted=roomCtx;
        var out={mounted:true,roomId:roomCtx.id};
        LOG.log('CALL.mount:out',{out:out});
        return out;
      }catch(e){ LOG.log('CALL.mount:err',{in:{roomCtx:roomCtx},e:String(e)},'warn'); return null; }
    },
    unmount: function(){
      LOG.log('CALL.unmount:in',{});
      try{
        var roomId=_mounted?_mounted.id:null;
        if(roomId) THREAD.postSystem(roomId,'call ended');
        _mounted=null;
        var out={unmounted:true,roomId:roomId};
        LOG.log('CALL.unmount:out',{out:out});
        return out;
      }catch(e){ LOG.log('CALL.unmount:err',{in:{},e:String(e)},'warn'); return null; }
    }
  };
})();
/* ===== #end module: CALL ===== */
...
/* ===== #new module: PB-DATA ===== */
/* checksum-before: n/a-new (WIRED this stage — pbGetCards/pbSaveCards/pbNorm now redirect here.
   Canonical fields per §4M.12 are added. The 8 named legacy fields (catalogIds, confidence,
   semanticRelationships, parentCategory, primaryTag, relatedIntents, intentId, fingerprint) are
   RETAINED, not dropped, in this stage — they are still load-bearing for live features not yet
   migrated: O-Ring (_qrOpen/_qrRender use relatedIntents), Translation Memory (_tmBuild/_tmCheck
   use fingerprint/confidence/intentId), and catalog filtering (_catId paths use catalogIds).
   Dropping them now would silently break those working, tested features. Full drop happens once
   those features are migrated onto the canonical schema — not yet scheduled in the plan. Same
   reasoning applies to tags/notes/savedBy/deletedAt, which pbBubbleHtml (still the live renderer
   this stage) reads directly and are not mentioned in §4M.12's list either way. */
var PB_DATA = (function(){
  function canonicalNorm(raw){
    ...
    return {
      ...
      categories: Array.isArray(raw.categories) && raw.categories.length ? raw.categories : ['unassigned'],
...
/* ===== #new module: PB-QUERY ===== */
/* checksum-before: n/a-new (one engine for overlay AND compose drawer; consumes PB-DATA, not pbSearch)
   Turn 07 Ship (2026-07-01): haystack extended to exact-match pbSearch's live behavior
   (source/target/notes/tags/clarifyChain/backtranslate/langs, verdict: and tag: prefixes)
   so pbSearch can delegate here with zero output change — this IS the engine now, not a copy. */
var PB_QUERY = {
  query: function(args){
    LOG.log('PB-QUERY.query:in',{args:args});
    try{
      args=args||{};
      var raw=(args.text||'').trim();
      var pair=args.pair||null;
      var cards=Array.isArray(args.cards)?args.cards:PB_DATA.getLive();
      if(!raw){
        var out0={cards:cards,total:cards.length};
        LOG.log('PB-QUERY.query:out',{out:{total:out0.total}});
        return out0;
      }
      ...
      var filtered=cards.filter(function(c){
        if(pair){
          var p=(c.sourceLang||'')+'-'+(c.targetLang||'');
      ...
      var out={cards:filtered,total:filtered.length};
      LOG.log('PB-QUERY.query:out',{out:{total:out.total}});
      return out;
    }catch(e){ LOG.log('PB-QUERY.query:err',{in:{args:args},e:String(e)},'warn'); return {cards:[],total:0}; }
  }
};
/* ===== #end module: PB-QUERY ===== */

/* ===== #new module: PB-RENDER (renderRow only this stage; renderCard ships Post-ship) ===== */
/* checksum-before: n/a-new (pure card→DOM; no storage, no globals; does not reuse pbIRowHtml's
   index-bound _pbICards lookup since renderRow takes the card directly per contract) */
var PB_RENDER = {
  renderRow: function(card){
    LOG.log('PB-RENDER.renderRow:in',{card:card});
    try{
      card=card||{};
      var row=document.createElement('div');
      row.className='pb-row';
      var srcSpan=document.createElement('span');
      srcSpan.className='pb-row-src';
      srcSpan.textContent=card.source||'';
      var tgtSpan=document.createElement('span');
      tgtSpan.className='pb-row-tgt';
      tgtSpan.textContent=card.target||'';
      row.appendChild(srcSpan);
      row.appendChild(tgtSpan);
      LOG.log('PB-RENDER.renderRow:out',{out:{id:card.id||null}});
      return row;
    }catch(e){ LOG.log('PB-RENDER.renderRow:err',{in:{card:card},e:String(e)},'warn'); return document.createElement('div'); }
  }
};
/* ===== #end module: PB-RENDER ===== */

/* ===== #new module: COMPOSE-SEAM (dual chat/search behavior + slide-up drawer) ===== */
/* checksum-before: n/a-new (dormant; NOT wired into the live chatKeydown/compose handlers —
   those stay untouched per §C; this is the contract-correct implementation proven in isolation,
   activated at its one call site in a later turn). */
var COMPOSE_SEAM = (function(){
  var _drawerOpen = false;
  function isPredicate(text){
    return typeof text==='string' && (text.charAt(0)==='/' || text.slice(0,2)==='..');
  }
  ...
  return {
    /* Returns {sendAllowed, drawerOpen, query} — guards BOTH Enter and Send so a predicate
       never reaches the transcript (graveyard G10). */
    handleSubmit: function(text){
      LOG.log('COMPOSE_SEAM.handleSubmit:in',{text:text});
      try{
        var norm=normalizePredicate(text||'');
        if(isPredicate(norm)){
          _drawerOpen=true;
          var q=norm.slice(1);
          var out={sendAllowed:false,drawerOpen:true,query:q};
          LOG.log('COMPOSE_SEAM.handleSubmit:out',{out:out});
          return out;
        }
        var out2={sendAllowed:true,drawerOpen:_drawerOpen,query:''};
        LOG.log('COMPOSE_SEAM.handleSubmit:out',{out:out2});
        return out2;
      }catch(e){ LOG.log('COMPOSE_SEAM.handleSubmit:err',{in:{text:text},e:String(e)},'warn'); return {sendAllowed:true,drawerOpen:false,query:''}; }
    },
    closeDrawer: function(){
      LOG.log('COMPOSE_SEAM.closeDrawer:in',{});
      try{ _drawerOpen=false; var out=true; LOG.log('COMPOSE_SEAM.closeDrawer:out',{out:out}); return out; }
      catch(e){ LOG.log('COMPOSE_SEAM.closeDrawer:err',{in:{},e:String(e)},'warn'); return false; }
    },
    isOpen: function(){
      LOG.log('COMPOSE_SEAM.isOpen:in',{});
      try{ var out=_drawerOpen; LOG.log('COMPOSE_SEAM.isOpen:out',{out:out}); return out; }
      catch(e){ LOG.log('COMPOSE_SEAM.isOpen:err',{in:{},e:String(e)},'warn'); return false; }
    }
  };
})();
/* ===== #end module: COMPOSE-SEAM ===== */

/* ===== #end Turn 06 Ship — Core UI + Shared Search Seam ===== */


/* ============================================================
   TURN 06 POST-SHIP — PB UI GROUP (ALL DORMANT)
   PB-SYNC, PB-USAGE, PB-RENDER.renderCard
   use.* flags all false; no call sites wired; old code untouched.
   ============================================================ */

/* ===== #new module: PB-SYNC ===== */
/* checksum-before: n/a-new. FIXED: G6 (offline upload retry) had no implementation --
   writeBack only logged pbsync_push_err on failure with no pending/retry path. Added
   pbsync_upload_pending logging plus a one-shot online-event listener that retries.
   Also: pull success now records _pbLastPull (filename+timestamp) and refreshes the
   overlay's sticky footer if it's open. */
var PB_SYNC = (function(){
  var REPO_OWNER='acmeproducts';
  var REPO_NAME='stuff';
  var PB_DIR='phrasebook';
  var DIRTY_KEY='tb_pb_dirty';
  // Post-ship fix (2026-07-01): _dirty used to live in memory only — an unclean close
  // (tab killed, not a normal hangUp) lost it, so a real unsynced local edit could sit
  // in localStorage indefinitely with GitHub never finding out. Now backed by
  // localStorage so it survives a reload, and _pbSyncOnEnter (below, at call start)
  // flushes it before that call's phrasebook loads — GitHub stays source of truth.
  var _dirty=(localStorage.getItem(DIRTY_KEY)==='1');
  var _retryArmed=false;
  function armReconnectRetry(){
    ...
    window.addEventListener('online',function _onReconnect(){
      ...
      _retryArmed=false;
      if(_dirty) writeBackImpl();
    });
  }
  function ghHeaders(){
    var pat=localStorage.getItem('tb_gh_pat')||'';
    var h={'Accept':'application/vnd.github.v3+json'};
    if(pat) h['Authorization']='token '+pat;
    return h;
  }
  function pairFilename(src,tgt,ver){
    var v=String(ver||1000);
    while(v.length<4) v='0'+v;
    return 'phrasebook-'+src+'-'+tgt+'-'+v+'.json';
  }
  function pullImpl(src,tgt){
    LOG.log('PB-SYNC.pull:in',{src:src,tgt:tgt});
    try{
      var pat=localStorage.getItem('tb_gh_pat')||'';
      if(!pat){
        var out={status:'no-pat',version:null,cards:[]};
        LOG.log('PB-SYNC.pull:out',{out:out});
        return Promise.resolve(out);
      }
      var url='https://api.github.com/repos/'+REPO_OWNER+'/'+REPO_NAME+'/contents/'+PB_DIR+'/?ref=main';
      return fetch(url,{headers:ghHeaders()}).then(function(r){
        if(!r.ok) throw new Error('list '+r.status);
        return r.json();
      }).then(function(files){
        var prefix='phrasebook-'+src+'-'+tgt+'-';
        var matches=(files||[]).filter(function(f){ return f.name.indexOf(prefix)===0 && f.name.slice(-5)==='.json' && parseInt(f.name.replace(prefix,'').replace('.json',''),10)>=1000; });
        if(!matches.length){
          var o={status:'no-pair-file',version:null,cards:[]};
          LOG.log('PB-SYNC.pull:out',{out:o});
          return o;
        }
        matches.sort(function(a,b){ return parseInt(b.name.replace(prefix,'').replace('.json',''),10)-parseInt(a.name.replace(prefix,'').replace('.json',''),10); });
        var best=matches[0];
        var ver=parseInt(best.name.replace(prefix,'').replace('.json',''),10);
        return fetch('https://raw.githubusercontent.com/'+REPO_OWNER+'/'+REPO_NAME+'/main/'+PB_DIR+'/'+best.name).then(function(r2){
          if(!r2.ok) throw new Error('fetch '+r2.status);
          return r2.json();
        }).then(function(data){
          var cards=Array.isArray(data)?data:(data.cards||[]);
          PB_DATA.save(cards.map(function(c){ return Object.assign({sourceLang:src,targetLang:tgt},c); }));
          _dirty=false;localStorage.removeItem(DIRTY_KEY);
          var o={status:'ok',version:ver,cards:PB_DATA.getLive()};
          if(typeof _pbLastPull!=='undefined'){
            _pbLastPull={filename:best.name,pulledAt:Date.now()};
            if(typeof pbRenderSyncFooter==='function')pbRenderSyncFooter();
            if(typeof pbOvUpdateSyncDot==='function')pbOvUpdateSyncDot('idle');
          }
          LOG.log('pbsync_pulled',{pair:src+'-'+tgt,version:ver,n:o.cards.length});
          LOG.log('PB-SYNC.pull:out',{out:{status:'ok',version:ver,n:o.cards.length}});
          return o;
        });
      }).catch(function(e){
        LOG.log('pbsync_pull_err',{e:String(e)},'warn');
        LOG.log('PB-SYNC.pull:err',{in:{src:src,tgt:tgt},e:String(e)},'warn');
        return {status:'error',version:null,cards:[]};
      });
    }catch(e){
      LOG.log('PB-SYNC.pull:err',{in:{src:src,tgt:tgt},e:String(e)},'warn');
      return Promise.resolve({status:'error',version:null,cards:[]});
    }
  }
  function writeBackImpl(){
    LOG.log('PB-SYNC.writeBack:in',{});
    try{
      if(!_dirty){
        var skip={status:'skipped'};
        LOG.log('pbsync_skipped_no_changes',{});
        LOG.log('PB-SYNC.writeBack:out',{out:skip});
        return Promise.resolve(skip);
      }
      var pat=localStorage.getItem('tb_gh_pat')||'';
      if(!pat){
        var nopat={status:'no-pat'};
        LOG.log('PB-SYNC.writeBack:out',{out:nopat});
        return Promise.resolve(nopat);
      }
      var cards=PB_DATA.getLive();
      // Pending cards must all have completed or pending verdicts — no structural filter here, just save current state
      var src=(cards[0]&&cards[0].sourceLang)||localStorage.getItem('tb_my_lang')||'en';
      var tgt=(cards[0]&&cards[0].targetLang)||localStorage.getItem('tb_their_lang')||'en';
      var prefix='phrasebook-'+src+'-'+tgt+'-';
      var listUrl='https://api.github.com/repos/'+REPO_OWNER+'/'+REPO_NAME+'/contents/'+PB_DIR+'/?ref=main';
      return fetch(listUrl,{headers:ghHeaders()}).then(function(r){
        if(!r.ok) throw new Error('list '+r.status);
        return r.json();
      }).then(function(files){
        var matches=(files||[]).filter(function(f){ return f.name.indexOf(prefix)===0 && f.name.slice(-5)==='.json' && parseInt(f.name.replace(prefix,'').replace('.json',''),10)>=1000; });
        var nextVer=1000;
        if(matches.length){ matches.sort(function(a,b){ return parseInt(b.name.replace(prefix,'').replace('.json',''),10)-parseInt(a.name.replace(prefix,'').replace('.json',''),10); }); nextVer=parseInt(matches[0].name.replace(prefix,'').replace('.json',''),10)+1; }
        var fname=pairFilename(src,tgt,nextVer);
        var content=btoa(unescape(encodeURIComponent(JSON.stringify(cards,null,2))));
        var putUrl='https://api.github.com/repos/'+REPO_OWNER+'/'+REPO_NAME+'/contents/'+PB_DIR+'/'+fname;
        var putH=Object.assign({'Content-Type':'application/json'},ghHeaders());
        return fetch(putUrl,{method:'PUT',headers:putH,body:JSON.stringify({message:'PB update '+src+'-'+tgt+' v'+nextVer,content:content,branch:'main'})}).then(function(r2){
          if(!r2.ok) throw new Error('put '+r2.status);
          return r2.json();
        }).then(function(d){
          _dirty=false;localStorage.removeItem(DIRTY_KEY);
          var o={status:'ok',version:nextVer};
          if(typeof _pbLastPull!=='undefined'){
            _pbLastPull={filename:fname,pulledAt:Date.now()};
            if(typeof pbRenderSyncFooter==='function')pbRenderSyncFooter();
            if(typeof pbOvUpdateSyncDot==='function')pbOvUpdateSyncDot('saved');
          }
          LOG.log('pbsync_upload_completed',{pair:src+'-'+tgt,version:nextVer});
          LOG.log('PB-SYNC.writeBack:out',{out:o});
          return o;
        });
      }).catch(function(e){
        LOG.log('pbsync_push_err',{e:String(e)},'warn');
        LOG.log('pbsync_upload_pending',{e:String(e)});
        armReconnectRetry();
        if(typeof pbOvUpdateSyncDot==='function')pbOvUpdateSyncDot('error');
        LOG.log('PB-SYNC.writeBack:err',{in:{},e:String(e)},'warn');
  ...
  return {
    pull: function(src,tgt){ return pullImpl(src,tgt); },
    writeBack: function(){ return writeBackImpl(); },
    markDirty: function(){
      LOG.log('PB-SYNC.markDirty:in',{});
      try{ _dirty=true;localStorage.setItem(DIRTY_KEY,'1'); LOG.log('PB-SYNC.markDirty:out',{out:true}); return true; }
      catch(e){ LOG.log('PB-SYNC.markDirty:err',{in:{},e:String(e)},'warn'); return false; }
    },
    isDirty: function(){
      LOG.log('PB-SYNC.isDirty:in',{});
      try{ var out=_dirty; LOG.log('PB-SYNC.isDirty:out',{out:out}); return out; }
      catch(e){ LOG.log('PB-SYNC.isDirty:err',{in:{},e:String(e)},'warn'); return false; }
    }
  };
})();
/* ===== #end module: PB-SYNC ===== */
...
/* ===== #new module: PB-USAGE ===== */
/* checksum-before: n/a-new. FIXED: recordUse previously mutated the object returned by
   PB_DATA.byId(), a fresh copy each call (not a live reference) since Base made PB_DATA
   storage-backed. That mutation was silently discarded. Now mutates within the full
   card list and calls PB_DATA.save() to actually persist. */
var PB_USAGE = {
  recordUse: function(cardId){
    LOG.log('PB-USAGE.recordUse:in',{cardId:cardId});
    try{
      var cards=PB_DATA.getCards();
      var card=cards.find(function(c){return c.id===cardId;});
      if(!card){
        LOG.log('PB-USAGE.recordUse:err',{in:{cardId:cardId},e:'card not found'},'warn');
        return null;
      }
      card.lastUsed=Date.now();
      card.usage=(typeof card.usage==='number'?card.usage:0)+1;
      card.updatedBy='local';
      PB_DATA.save(cards);
      PB_SYNC.markDirty();
      var out={cardId:cardId,usage:card.usage,lastUsed:card.lastUsed};
      LOG.log('PB-USAGE.recordUse:out',{out:out});
      return out;
    }catch(e){ LOG.log('PB-USAGE.recordUse:err',{in:{cardId:cardId},e:String(e)},'warn'); return null; }
  },
  getUsage: function(cardId){
    LOG.log('PB-USAGE.getUsage:in',{cardId:cardId});
    try{
      var card=PB_DATA.byId(cardId);
      if(!card){
        LOG.log('PB-USAGE.getUsage:err',{in:{cardId:cardId},e:'card not found'},'warn');
        return null;
      }
      var out={lastUsed:card.lastUsed||null,usage:card.usage||0};
      LOG.log('PB-USAGE.getUsage:out',{out:out});
      return out;
    }catch(e){ LOG.log('PB-USAGE.getUsage:err',{in:{cardId:cardId},e:String(e)},'warn'); return null; }
  }
};
/* ===== #end module: PB-USAGE ===== */

/* ===== #new module: PB-RENDER.renderCard ===== */
/* checksum-before: n/a-new (PB_RENDER var exists from Ship; this adds renderCard method to it)
   Layout authority: phrase-desk.html pbBubbleHtml — 4-row card, tag drawer, clarify drawer.
   Pure DOM construction; no storage, no globals, no event handlers wired (dormant). */
PB_RENDER.renderCard = function(card){
  LOG.log('PB-RENDER.renderCard:in',{card:card});
  try{
    card=card||{};
    var id=card.id||'';
    ...
    var tags=Array.isArray(card.categories)?card.categories:[];
    var srcLang=card.sourceLang||'en';
    var tgtLang=card.targetLang||'en';
    ...
    row1.className='flex border-b border-slate-100';
    row1.innerHTML=''
      +'<div class="w-1/2 min-w-0 px-3 py-2 flex items-start border-r border-slate-100">'
        +'<textarea class="pb-input font-medium text-slate-800 text-sm flex-1" id="pbsrc-'+id+'" data-cardid="'+id+'" rows="1">'+((card.source||'').replace(/</g,'&lt;').replace(/>/g,'&gt;'))+'</textarea>'
        +'<button type="button" class="shrink-0 ml-1.5 p-0.5 text-slate-300 hover:text-teal-500 self-start mt-0.5" data-cardid="'+id+'" data-side="src" data-lang="'+srcLang+'" data-action="tts">'
          +'<i data-lucide="volume-2" class="w-3 h-3"></i></button>'
      +'</div>'
      +'<div class="w-1/2 min-w-0 px-3 py-2 flex items-start bg-slate-50/50">'
        +'<div class="pb-input font-medium text-teal-700 text-sm flex-1" id="pbtgt-'+id+'">'+(card.target||'...')+'</div>'
        +'<button type="button" class="shrink-0 ml-1.5 p-0.5 text-slate-300 hover:text-teal-500 self-start mt-0.5" data-cardid="'+id+'" data-side="tgt" data-lang="'+tgtLang+'" data-action="tts">'
          +'<i data-lucide="volume-2" class="w-3 h-3"></i></button>'
      +'</div>';
    ...
    row2.className='px-3 py-2 border-b border-slate-100 flex items-start bg-slate-50/30';
    row2.innerHTML=''
      +'<div class="pb-input italic text-slate-500 text-sm flex-1" id="pb-bt-text-'+id+'">'+(backTxt||'...')+'</div>'
      +'<button type="button" class="shrink-0 ml-1.5 p-0.5 text-slate-300 hover:text-teal-500 self-start mt-0.5" data-cardid="'+id+'" data-side="bck" data-lang="'+srcLang+'" data-action="tts">'
        +'<i data-lucide="volume-2" class="w-3 h-3"></i></button>';
    wrap.appendChild(row2);

    // Row 3: Verdict
    var row3=document.createElement('div');
    row3.className='flex gap-2 px-3 py-2 border-b border-slate-100 bg-slate-50/50';
    row3.setAttribute('data-cardid',id);
    row3.innerHTML=''
      +'<input type="radio" name="v-'+id+'" id="vg-'+id+'" class="verdict-radio radio-good" value="good"'+(verdict==='good'?' checked':'')+' data-cardid="'+id+'" data-action="verdict">'
      +'<label for="vg-'+id+'" class="verdict-label text-xs font-bold"><i data-lucide="thumbs-up" class="w-3.5 h-3.5 mr-1"></i>Sounds Good</label>'
      +'<input type="radio" name="v-'+id+'" id="vf-'+id+'" class="verdict-radio radio-flag" value="flag"'+(verdict==='flag'?' checked':'')+' data-cardid="'+id+'" data-action="verdict">'
      +'<label for="vf-'+id+'" class="verdict-label text-xs font-bold"><i data-lucide="flag" class="w-3.5 h-3.5 mr-1"></i>Flag</label>';
    wrap.appendChild(row3);

    // Row 4: Footer (tags / clarify / trash)
    var row4=document.createElement('div');
    row4.className='grid grid-cols-3 divide-x divide-slate-200 bg-slate-50';
    row4.setAttribute('data-cardid',id);
    row4.innerHTML=''
      +'<button type="button" data-cardid="'+id+'" data-drawer="tags" class="py-1.5 flex justify-center items-center text-slate-400 hover:text-teal-600 hover:bg-slate-100"><i data-lucide="hash" class="w-3.5 h-3.5"></i></button>'
      +'<button type="button" data-cardid="'+id+'" data-drawer="clarify" class="py-1.5 flex justify-center items-center text-slate-400 hover:text-teal-600 hover:bg-slate-100"><i data-lucide="message-square" class="w-3.5 h-3.5"></i></button>'
      +'<button type="button" data-cardid="'+id+'" data-action="trash" class="py-1.5 flex justify-center items-center text-slate-400 hover:text-red-600 hover:bg-red-50"><i data-lucide="trash-2" class="w-3.5 h-3.5"></i></button>';
    wrap.appendChild(row4);

    // Tag drawer (closed by default)
    var tagsHtml=tags.map(function(t){
      return '<span class="inline-flex items-center gap-1 bg-slate-100 border border-slate-200 text-teal-700 px-1.5 py-0.5 rounded text-[11px] font-mono" data-cardid="'+id+'">'+t+'</span>';
    }).join('');
    var drawerTags=document.createElement('div');
    drawerTags.id='drawer-tags-'+id;
    drawerTags.className='hidden bg-slate-50 px-3 py-2 border-t border-slate-100';
    drawerTags.innerHTML=''
      +'<div class="flex flex-wrap gap-1 mb-1.5" id="pbtags-'+id+'">'+tagsHtml+'</div>'
      +'<div class="relative">'
        +'<input type="text" autocomplete="off" id="pb-ti-'+id+'" data-cardid="'+id+'" class="w-full bg-white border border-slate-200 rounded px-2 py-1.5 pr-6 text-xs outline-none focus:border-teal-500" placeholder="Type tag + Enter to add">'
        +'<button type="button" data-cardid="'+id+'" data-action="clear-tag-input" class="absolute right-1.5 top-1/2 -translate-y-1/2 text-slate-300 hover:text-slate-500 leading-none text-sm font-bold">&times;</button>'
      +'</div>'
      +'<div id="pb-ts-'+id+'" class="flex flex-wrap gap-1 mt-1.5"></div>';
    wrap.appendChild(drawerTags);

    // Clarify drawer (closed by default)
    var drawerClarify=document.createElement('div');
    drawerClarify.id='drawer-clarify-'+id;
    drawerClarify.className='hidden bg-slate-50 px-3 py-2 border-t border-slate-100';
    drawerClarify.innerHTML=''
      +'<div id="pb-cc-'+id+'" class="max-h-48 overflow-y-auto"></div>'
      +'<input type="text" autocomplete="off" id="pb-ci-'+id+'" data-cardid="'+id+'" class="w-full bg-white border border-slate-200 rounded px-2 py-1.5 text-xs outline-none focus:border-teal-500 mt-1" placeholder="Add note + Enter to log...">';
    wrap.appendChild(drawerClarify);

    LOG.log('PB-RENDER.renderCard:out',{out:{id:id}});
    return wrap;
  }catch(e){ LOG.log('PB-RENDER.renderCard:err',{in:{card:card},e:String(e)},'warn'); return document.createElement('div'); }
};
/* ===== #end module: PB-RENDER.renderCard ===== */

/* ===== #end Turn 06 Post-ship — PB UI group ===== */
