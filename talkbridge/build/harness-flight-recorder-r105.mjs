#!/usr/bin/env node
/* R10.5 integrated recorder gate. Static contract plus a live durable-object trace
...
await T('F1 immutable candidate identity appears in app, worker and relay',()=>{
  A(/\bBUILD\s*=\s*'R10\.5'/.test(part) && /\bBUILD\s*=\s*'R10\.5'/.test(built), 'app build id missing or divergent');
  A(/\bFR_BUILD\s*=\s*'R10\.5'/.test(sw), 'worker build id missing or divergent');
  A(/\bFR_BUILD\s*=\s*'R10\.5'/.test(relay), 'relay build id missing or divergent');
  A(built.includes('r10.5-app/1')&&sw.includes('r10.5-sw/1')&&relay.includes('r10.5-relay/1'),'layer versions missing');
});
...
await T('F6b diagnostics controls are hidden from normal use and enabled only by an explicit diagnostic URL',()=>{
  A(part.includes("searchParams.get('tbDiagnostics') !== '1'"),'normal UI exposes recorder controls');
});
...
await T('F11 mixed worker/relay versions visibly invalidate the run',()=>{
  A(part.includes('VERSION MISMATCH · INVALID RUN')&&part.includes("v.sw !== 'r10.5-sw/1'")&&part.includes("v.relay !== 'r10.5-relay/1'"),'version block missing');
});
...
await T('F13 recorder surrounds the corrected legacy notification without changing its envelope fields',()=>{
  for(const token of ["var tag = note.tag || ('tb-' + (tb.roomId || 'unknown'))","renotify: false","var title = note.title || 'TalkBridge'","journal('shown'","journal('failed'"])A(sw.includes(token),token+' changed');
});
...
await T('F16 relay instruments receive, socket delivery, recipient decision, push request/response and subscription',()=>{
  for(const token of ["'relay_receive'","'socket_delivery'","'presentation_owner'","'push_attempt'","'push_service'","'subscription'"])A(relay.includes(token),token+' missing');
});
...
{
  const {s,sockets}=mkSession();await s.ready;s.sessionName='room-a';
  ...
  await T('R2 related client receives a versioned, bounded, redacted slice',()=>{
    A(res.status===200&&body.version==='r10.5-relay/1'&&body.schemaVersion==='tbfr/1.0','manifest wrong');
