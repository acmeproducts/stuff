#!/usr/bin/env node
/* R10-CR1 relay gate (plan v20.9.0 §4.11.5). Drives the worker with the
...
await scenario('R10 read-only HTTPS reconciliation equals the socket answer; caller expiry word is not required', async () => {
  ...
  const d = await w.post('A', { type: 'diag' }); assert.equal(d.v, '6.0'); assert.ok(Array.isArray(d.events));
});
