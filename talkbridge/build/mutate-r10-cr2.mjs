#!/usr/bin/env node
/* R10-CR2 mutation gate (plan v20.9.0 §4.11.5): every scenario assertion must
...
const RELAY_HARNESS = path.resolve('talkbridge/build/harness-relay-r10-cr2.mjs');
const APP_HARNESS = path.resolve('talkbridge/build/harness-r10-cr2.mjs');
...
let bad = 0;
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'tb-cr2-mut-'));
...
const SW_HARNESS = path.resolve('talkbridge/build/harness-sw-r10-cr2.mjs');
