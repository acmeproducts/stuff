#!/usr/bin/env node
/* R10-CR2 app gate (plan v20.9.0 §4.11.5). The assembled candidate runs in a
...
async function runScenarios(html) {
  ...
  await scenario('S5b hidden on home → call → one OS request, no ring, no device-side alert; return presents the active call once', async () => {
    ...
    assert.equal(w.dbg('cr2_call_presented').length, 1);
    w.setHidden(true); await settle(); w.setHidden(false); await settle(250);
    assert.equal(w.dbg('cr2_call_presented').length, 1, 'never a second surface for the same call');
  ...
  await scenario('S6a warm tap on an active call → exact Accept/Decline surface, once; on an ended call → room with durable missed outcome, no ring', async () => {
    ...
    assert.equal(w.ring(), true, 'active call: the Accept/Decline surface'); assert.equal(w.dbg('cr2_call_presented').length, 1);
  ...
  await scenario('S6b cold tap → the app opens on the exact event: active call rings; ended call shows the room and outcome', async () => {
    ...
    assert.equal(w.ring(), true, 'active call: the answer surface'); assert.equal(w.dbg('cr2_call_presented').length, 1, 'presented once: ' + JSON.stringify(w.dbg().filter((l) => /^cr2_/.test(l.ev)).map((l) => l.ev)));
  ...
  await scenario('S8 leave the room → a call two seconds later → in-app surface, no OS request (lane continuity, G21)', async () => {
    ...
    assert.ok(w.dbg('cr2_leave_lane').length >= 1);
  });
  ...
  await scenario('S7 baseline parity: the frozen bytes are carried verbatim; one part appended; no buried mechanism; no new secret', async () => {
    ...
    assert.equal(w.win.osNotify._cr2Original.name === '' || typeof w.win.osNotify._cr2Original === 'function', true);
  });
...
const APP_MUTATIONS = [
  ...
  ['ring-while-hidden · a hidden app presents the ring screen', "    if (document.hidden) { cr2Log('ring_deferred_hidden'", "    if (false) { cr2Log('ring_deferred_hidden'"],
  ['local-missed-pill · the browser writes its own missed pill again', "    if (typeof text === 'string' && text.indexOf('Missed ') === 0) { cr2Log('local_missed_pill_dropped', { room: roomId }); return; }", "    /* local pill restored */"],
  ['duplicate-lane · concurrent signals open a second socket', "      if (ws && ws.readyState === 0 && room && ws._cr2Room === room.id) { cr2Log('connect_coalesced', { room: room.id }); return; }", "      /* no coalescing */"],
  ['no-seen · a visibly handled chat is never acknowledged', "cr2Send(S.roomId, { type: 'ev-seen', ids: [String(d.eventId)] });", "void 0;"],
  ['cold-route-lost · a cold tap lands on the homepage', "        setTimeout(function () { cr2RouteEvent(roomId, eventId, 'tap_cold'); }, 0);", "        void roomId;"],
  ['mute-unheard · a mute toggle is not reported until the next reconnect', "      if (cr2State.muteSig !== undefined && cr2State.muteSig !== sig) {", "      if (false) {"],
  ['hidden-unannounced · going hidden is not reported to the relay', "if (document.hidden) cr2Announce('hidden'); else { cr2Recover('visible'); cr2AnnounceWindow('visible'); }", "if (document.hidden) void 0; else { cr2Recover('visible'); cr2AnnounceWindow('visible'); }"],
  ['device-alert · the app raises its own OS notification beside the relay push', "  osNotify = function (title, body, roomId) { cr2Log('os_notify_owned_by_relay', { room: roomId }); };", "  osNotify = function (title, body, roomId) { return _cr2OsNotify.apply(this, arguments); };"],
  ['second-surface · an active call is re-presented on every return', "    if (!c || !c.callId || cr2State.rung[c.callId]) return;", "    if (!c || !c.callId) return;"],
  ['leave-laneless · leaving a room does not open its listener lane', "    try { LISTEN.sync(); cr2Log('leave_lane', { room: left }, 'ok'); cr2Recover('leave'); }", "    try { void left; }"],
  ['no-announce · the installed app never announces its window', "      cr2AnnounceWindow('boot');", "      void 0;"],
  ['stale-lane · an open callback from a replaced socket reconciles anyway', "  if (!current()) { cr2Log('open_stale', { room: roomId }, 'warn'); return; }", "  void current;"]
];
