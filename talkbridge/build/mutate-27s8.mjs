#!/usr/bin/env node
/* 27·ship candidate 8 mutation gate (§7.15 M5). Each mutation reintroduces,
...
import { assemble, PARTS } from './assemble-27s8.mjs';

const P = { d1: 0, c1: 1, v2: 2, c3: 3, c2: 4, s2: 5, f1: 6 };
...
const MUTATIONS = [
  ...
  { part: P.f1, catches: 'M3h.1', name: 'F-1 never tags the released sender → flip drops the far side (the c5 bug)', apply: (s) => s.replace("if (sender && track === null && sender.track && sender.track.kind === 'video') tag(sender);", '') },
  { part: P.f1, catches: 'M3h.1', name: 'F-1 tags but camSenders ignores the tag', apply: (s) => s.replace('if (s && s.__tbVideoSender && found.indexOf(s) === -1) found.push(s);', '') },
  { part: P.f1, catches: 'M3h.6', name: 'F-1 tags every released sender, audio included', apply: (s) => s.replace("if (sender && track === null && sender.track && sender.track.kind === 'video') tag(sender);", 'if (sender && track === null) tag(sender);') },
  { part: P.f1, catches: 'M2.2', name: 'F-1 replaces camSenders outright', apply: (s) => s.replace('var found = _camSenders.apply(this, arguments) || [];', 'var found = [];') },
...
const dir = mkdtempSync(path.join(tmpdir(), 'tb-27s8-mut-'));
...
for (let i = 0; i < MUTATIONS.length; i++) {
  ...
  let out = '', exit = 0;
  try { out = execFileSync('node', ['talkbridge/build/harness-27s8.mjs', builtPath], { encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'], env: { ...process.env, TB_PART_OVERRIDE: JSON.stringify(partPaths) } }); }
