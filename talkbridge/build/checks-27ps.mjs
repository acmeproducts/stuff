...
const MARKERS = ['GAP PART · C1-signal-queue.js', 'GAP PART · V2-relay-retry.js', 'GAP PART · C3-joiner-restart.js', 'GAP PART · C2-stall-frames.js', 'GAP PART · S2-back-absorb.js', 'GAP PART · F1-flip-keeps-sender.js', 'GAP PART · K1-device-ids.js', 'GAP PART · K2-pb-merge.js', 'GAP PART · K4-rename-lww.js', 'GAP PART · T1-render-coalesce.js', 'GAP PART · T2-log-hygiene.js', 'GAP PART · T3-wrap-map.js', 'GAP PART · D10-tag-enter.js'];
...
function checkStructure(html) {
  ...
  return 'tags balanced, thirteen parts present in order (six carried, K-1, K-2, K-4, T-1, T-2, T-3, D-10); D1 absent';
}
function checkWire(html) {
  ...
  for (const sym of ['function relaySend(', 'function relaySendWhenOpen(', 'function relayConnect(', 'CALL.runRecovery = function', 'CALL.startVideoWatchdog = function', 'CALL.stopVideoWatchdog = function', 'onSignal:async function', 'function tbSwapTap', 'function tbFlipCamera', 'history.pushState({tbCall:1}', 'function camSenders()', 'function replaceSenderTrack(', 'function uid()', 'function pbWriteBack()', 'function pbPull()', 'function onRoomNameSignal(', 'function renameRoom(', 'function renderTranscript()', 'function renderPanel()', 'function renderHome()', 'function log(', 'function pbAddTagTo(', 'function pbRerenderCard(', 'function renderPbList()']) if (html.slice(0, start).indexOf(sym) === -1) throw new Error('baseline symbol missing: ' + sym);
...
async function checkRuntime(html) {
  ...
  const dom = new JSDOM(html, { url: 'https://acmeproducts.github.io/stuff/bridge-turn27-post-ship.html', runScripts: 'dangerously', pretendToBeVisual: true, virtualConsole: vc,
    beforeParse(w) {
  ...
  dom.window.close();
  return 'boots clean; D1 gone; carried wrappers, K-1/K-2/K-4, T-1/T-2/T-3, D-10 installed; wrap_map logged; c5 swap/flip surface live';
}
const CHECKS = [
  ...
  { id: '4 runtime', run: checkRuntime, break: (h) => h.replace("if (typeof uid !== 'function' || typeof deviceId !== 'string' || !deviceId) return;", 'return;') }
];
let fail = 0;
