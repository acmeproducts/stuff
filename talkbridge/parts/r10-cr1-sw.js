/* TalkBridge service worker · R10-CR1 (plan v20.9.0 §4.11.4) · source: talkbridge/parts/r10-cr1-sw.js — assembled, never hand-edited.
   ...
   - A tap opens the EXACT event: a running app is told which event; a cold
     start carries it in the URL hash. The app resolves an active call to its
...
self.addEventListener('push', function (e) {
  e.waitUntil((function () {
    ...
    return journal('arrived', { id: ev.id, room: ev.room, kind: ev.kind }).then(function () { return withTimeout(loadCtx(), CTX_MS); }).then(function (ctx) {
      ...
      return self.registration.showNotification(d.title, { body: d.body, tag: d.tag, renotify: false, data: data })
...
self.addEventListener('notificationclick', function (e) {
  ...
  e.waitUntil(self.clients.matchAll({ type: 'window', includeUncontrolled: true }).then(function (list) {
    var app = null;
    for (var i = 0; i < list.length; i++) { if (String(list[i].url).indexOf(APP_FILE) >= 0) { app = list[i]; break; } }
    return journal('tapped', { id: data.eventId || null, room: data.roomId || null, kind: data.kind || null }).then(function () {
      if (app) {
        try { app.postMessage({ t: 'tb-open', roomId: data.roomId || null, eventId: data.eventId || null, callId: data.callId || null, kind: data.kind || null }); } catch (_) {}
