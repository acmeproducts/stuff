...
/* ─────────────────────────────────────────────────────────────────────────────
   ...
   J2 — RETIRED same day (research): iOS 16.4+ installs from the Share menu
        of every major browser, so the Safari detour is deleted, not shown.
...
/* J2 retired 2026-08-26 after research: iOS 16.4+ installs from the Share
   menu of Safari, Chrome, Edge, Firefox alike (MDN). The Safari detour was
   built to a pre-16.4 assumption and is deleted, not improved. The nudge
   below now serves every iOS browser. */

/* J3 — iPhone Safari, joined, not installed: ringing is the pitch. */
function n7InstallNudge() {
  try {
    var l = (typeof plLane === 'function') ? plLane() : 'other';
    if (l !== 'ios-safari' && l !== 'ios-other-browser') return;
...
function n7NameSync(v) {
  try {
    ...
    /* the URL itself is the handoff now: replace the live hash so Add to
       Home Screen bookmarks the invite WITH the typed name aboard */
