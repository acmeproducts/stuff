...
/* @contract
   Integrated R10.5 recorder: records and exports notification/call evidence. It must not choose,
...
(function () {
  'use strict';
  var SCHEMA = 'tbfr/1.0', BUILD = 'R10.5', APP_VERSION = 'r10.5-app/1';
  ...
  var allowed = {
    ...
    owner:1, lane:1, eventIdPresent:1
  };
  ...
  function human(rows) {
    ...
    if (v.sw && v.sw !== 'r10.5-sw/1') issues.push('VERSION MISMATCH: service worker ' + v.sw);
    if (v.relay && v.relay !== 'r10.5-relay/1') issues.push('VERSION MISMATCH: relay ' + v.relay);
    ...
    var out = ['TalkBridge R10.5 notification report', 'Build: ' + BUILD, 'Versions: app=' + v.app + ' sw=' + (v.sw || 'unknown') + ' relay=' + (v.relay || 'unknown'),
  ...
  function exportJsonl() { return fetchRelay().then(recordsForRun).then(function (rows) { download('talkbridge-r10.5.jsonl', 'application/x-ndjson', canonical(rows)); return rows; }); }
  function exportHuman() { return fetchRelay().then(recordsForRun).then(function (rows) { download('talkbridge-r10.5-report.txt', 'text/plain', human(rows)); return rows; }); }
  function exportBoth() { return fetchRelay().then(recordsForRun).then(function(rows){download('talkbridge-r10.5.jsonl','application/x-ndjson',canonical(rows));download('talkbridge-r10.5-report.txt','text/plain',human(rows));return rows;}); }
  ...
  function updateHealth() {
    ...
    var v = versions(), bad = (v.sw && v.sw !== 'r10.5-sw/1') || (v.relay && v.relay !== 'r10.5-relay/1');
    el.textContent = bad ? 'VERSION MISMATCH · INVALID RUN' : ('R10.5 · app ' + v.app + ' · sw ' + (v.sw || 'checking') + ' · relay ' + (v.relay || 'not fetched'));
    el.style.color = bad ? '#a32d2d' : '#315d5d';
  }
  function mountUi() {
    if (document.getElementById('tbfr-open')) return;
    try { if (new URL(location.href).searchParams.get('tbDiagnostics') !== '1') return; } catch (_) { return; }
    ...
    document.getElementById('tbfr-clear').onclick=function(){ if (confirm('Clear notification records on this device?')) clear(); }; document.getElementById('tbfr-close').onclick=function(){ modal.classList.remove('show'); };
    updateHealth();
  }
  ...
  if (typeof p4Offer === 'function') { var _p4OfferFr=p4Offer; p4Offer=function(roomId,d,lane){record('app','presentation_offer','received','observed',{message:d,roomId:roomId,detail:{lane:lane,eventIdPresent:!!(d&&d.eventId)}});return _p4OfferFr.apply(this,arguments);}; }
  if (typeof p4ApplyDecision === 'function') { var _p4DecisionFr=p4ApplyDecision; p4ApplyDecision=function(roomId,d,lane,original){record('app','presentation_owner','commit_received','observed',{message:d,roomId:roomId,detail:{owner:d&&d.owner,lane:lane,reason:d&&d.reason}});var r=_p4DecisionFr.apply(this,arguments);record('app','presentation_owner','applied','observed',{message:d,roomId:roomId,detail:{owner:d&&d.owner,lane:lane}});return r;}; }
  if (typeof p4ConfirmRoute === 'function') { var _p4ConfirmFr=p4ConfirmRoute; p4ConfirmRoute=function(route,actual){var r=_p4ConfirmFr.apply(this,arguments);record('app','notification_tap','navigation_result','observed',{message:{type:route&&route.type,eventId:route&&route.eventId,callId:route&&route.callId},roomId:route&&route.roomId,detail:{actual:actual,target:'event_room'}});return r;}; }
