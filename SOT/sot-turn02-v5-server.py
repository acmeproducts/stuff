...
class H(BaseHTTPRequestHandler):
 ...
 def do_GET(self):
  try:
   ...
   if p=='/api/health':return self._send({'ok':True,'version':m.VERSION,'schema':m.SCHEMA,'time':time.time(),'active_jobs':len(M.runtime)})
   ...
   if p=='/api/volumes':
    ...
    for base in ('/mnt','/media'):
     ...
     if q.exists():
      for x in q.iterdir():
       if x.is_dir():vols.append({'label':x.name,'path':str(x)})
    return self._send({'ok':True,'volumes':vols})
   if p.startswith('/api/folders'):
    from urllib.parse import urlparse,parse_qs
    q=parse_qs(urlparse(self.path).query);root=Path(q.get('path',['/mnt'])[0]).resolve();items=[]
    for x in root.iterdir():
     if x.is_dir() and not x.is_symlink():items.append({'name':x.name,'path':str(x)})
