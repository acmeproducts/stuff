...
echo '=== DOWNLOAD + OFFLINE UI GATES ==='
for f in sot-ui-6.8.1-setup-explorer-addon.js sot-ui-6.8.1-setup-status-hotfix.js; do
  curl -fsSL "$BASE/$f" -o "$TMP/$f"
  node --check "$TMP/$f"
done
cp -a "$UI" "$TMP/project.candidate.html"
python3 - "$TMP/project.candidate.html" "$TMP/sot-ui-6.8.1-setup-explorer-addon.js" "$TMP/sot-ui-6.8.1-setup-status-hotfix.js" <<'PY'
import pathlib,sys
ui=pathlib.Path(sys.argv[1]); h=ui.read_text()
if '2026.08.19.6.8-wsl-multiproject-reporting' not in h: raise SystemExit('Live UI is not 6.8')
if '2026.08.19.6.8.1-wsl-setup-explorer' in h: raise SystemExit('6.8.1 is already installed')
marker='</body></html>'
if marker not in h: raise SystemExit('UI body marker missing')
parts=[pathlib.Path(x).read_text() for x in sys.argv[2:]]
...
! grep -q 'Root-level files are intentionally ignored' "$TMP/sot-ui-6.8.1-setup-explorer-addon.js"

echo '=== INSTALL UI ONLY ==='
STAMP="$(date +%Y%m%d-%H%M%S)"
BAK="$UI.before-6.8.1-$STAMP"
cp -a "$UI" "$BAK"
rollback(){ echo '=== 6.8.1 UI GATE FAILED — RESTORE 6.8 UI ===' >&2; cp -a "$BAK" "$UI" || true; exit 1; }
trap rollback ERR
...
echo "UI:                $UI_BUILD"
echo 'Pane 2: explorer ribbon + sticky sortable folder table + mobile-primary layout'
echo "URL:               $PUBLIC"
trap - ERR
