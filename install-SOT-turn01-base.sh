...
RUN="$SOT_DIR/archive/$STAMP-turn01-r9-database-master-detail-release"
...
cleanup(){
  rc=$?
  set +e
  if [ -n "$ACTIVE_PID" ]; then kill "$ACTIVE_PID" >/dev/null 2>&1 || true; wait "$ACTIVE_PID" >/dev/null 2>&1 || true; fi
...
pass R9_BACKEND_PARSE 'R8 reconciliation + R9 catalog'

curl --retry 5 --retry-all-errors -fsSL "$RAW/$R9UI/SOT-turn01-base-r9.html" -o "$TMP/SOT-turn01-base.html"
...
pass R9_UI_CONTRACT 'dashboard bars + active-now + master/detail + database + deep dive'

sqlite3 "$DB" ".backup '$TMP/test.sqlite'"
SOT_DB_PATH="$TMP/test.sqlite" node - "$TMP/sot-api.js" > "$TMP/catalog.json" <<'NODE'
const a=require(process.argv[2]);
const c=a._test.ssotCatalog('content','',20,'');
const l=a._test.ssotCatalog('locations','',20,'');
const p=a._test.listProjects();
if(!Array.isArray(c.rows)||!Array.isArray(l.rows)||!Array.isArray(p)) throw Error('catalog contract');
console.log(JSON.stringify({content:c.rows.length,locations:l.rows.length,projects:p.length}));
NODE
...
const timer=setInterval(()=>{
  ...
  if(p && p.processing_state==='WIP' && Number(p.files_processed)===40 && Number(p.files_discovered)===100 && p.active_operation_id==='r9op'){
    clearInterval(timer);
    console.log(JSON.stringify({state:p.processing_state,files_processed:Number(p.files_processed),files_discovered:Number(p.files_discovered),operation:p.active_operation_id}));
    process.exit(0);
  }
  if(Date.now()>=deadline){ clearInterval(timer); console.error(JSON.stringify(p||null)); process.exit(1); }
},100);
NODE
ACTIVE_PID=$!
for i in {1..100}; do [ -f "$TMP/active.ready" ] && break; kill -0 "$ACTIVE_PID" >/dev/null 2>&1 || break; sleep 0.1; done
[ -f "$TMP/active.ready" ] || fail R9_ACTIVE_STARTUP 'backend fixture process did not become ready'
...
SQL
wait "$ACTIVE_PID" || fail R9_ACTIVE_BEHAVIOR 'initialized backend did not expose governed WIP fixture within 10s'
ACTIVE_PID=''
...
pass CUTOVER 'R9 catalog + database-centered dashboard installed'
...
for endpoint in 'turn01/ssot' 'turn01/catalog?view=content&limit=5' 'activity?limit=5' 'turn01/projects'; do
  code="$(curl --max-time 10 -sS -o /dev/null -w '%{http_code}' "http://127.0.0.1:18080/api/sot/$endpoint" || true)"
  [ "$code" = 200 ] || fail LIVE_R9_ENDPOINT "$endpoint HTTP=$code"
done
pass LIVE_R9_ENDPOINTS 'ssot + catalog + activity + project progress HTTP=200'
...
SUCCESS=1
pass RELEASE_READY 'R9 database-centered master/detail'
echo '=== TURN 01 BASE R9 READY FOR OWNER TEST ==='
