...
SQL
SOT_DB_PATH="$TMP/intel.sqlite" node - "$TMP/sot-api.js" > "$TMP/intel.json" <<'NODE'
const a=require(process.argv[2]); const g=a._test.storageIntelligence('',100),p=a._test.storageIntelligence('r10a',100); const d=g.duplicate_groups.find(x=>x.content_sha256==='a'.repeat(64));
if(!d||Number(d.copies)!==3||Number(d.projects)!==2||Number(d.reclaimable_bytes)!==200)throw Error('global duplicate truth '+JSON.stringify(d));
if(!String(d.locations).includes('/r10/a/dup.bin')||!String(d.locations).includes('/r10/b/dup.bin')||!String(d.locations).includes('/r10/c/dup.bin'))throw Error('locations missing');
if(Number(p.summary.duplicate_groups)!==1||Number(p.summary.duplicate_waste_bytes)!==100)throw Error('project duplicate math '+JSON.stringify(p.summary));
