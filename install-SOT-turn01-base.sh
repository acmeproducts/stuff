...
SRC_COMMIT='91f158c48eed9a46063a1258c9ecce73b69aa8e1'; V2_COMMIT='7fcd7c171237187a91ff27b4672f9c54640f8d16'; WORKER_COMMIT='ffaee4ad31fd35ed9000db932ddf81a16a83f44f'
...
python3 -m py_compile "$TMP/base.py" "$TMP/base22.py" "$TMP/coord.py" "$TMP/coord2.py" "$TMP/ui22.py" "$TMP/ui24.py" "$TMP/uiai.py" "$TMP/coordui.py" "$TMP/coord2ui.py" || fail PYTHON_PARSE failed
...
python3 "$TMP/ui22.py" "$TMP/pre.html" "$TMP/ui22.html"; python3 "$TMP/ui24.py" "$TMP/ui22.html" "$TMP/ui24.html"; python3 "$TMP/uiai.py" "$TMP/ui24.html" "$TMP/ui-base.html"; python3 "$TMP/coordui.py" "$TMP/ui-base.html" "$TMP/ui-c1.html"; python3 "$TMP/coord2ui.py" "$TMP/ui-c1.html" "$TMP/SOT-turn01-base.html" || fail UI_GENERATION failed
...
for x in ['function sotOperatorBusy()','state.selected=selected','state.tab=tab','cards.scrollTop=railY','Default Target','Default Backup','2-copy groups','3-copy groups','4+ copy groups','Previous / Stale Plan','Current Plan']:
 assert x in h,x
PY
...
SUCCESS=1; pass RELEASE_READY 'mechanical gates passed'; echo '=== TURN 01 COORDINATION 2 READY FOR TEST ==='; echo "TEST URL: $PUBLIC_URL"
