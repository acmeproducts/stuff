...
EXPECTED_BUILD='2026.08.29.sot-turn01-base-19'
PUBLIC_URL='https://oc-ref.fell-dojo.ts.net/report/SOT/SOT-turn01-base.html'
TMP="$(mktemp -d)"
STAMP="$(date +%Y%m%d-%H%M%S)"
QUAL_DIR="$ARCHIVE_ROOT/$STAMP-turn01-base19-qualification"
...
cleanup_windows_test(){
  ...
  if [ -x "$ps" ]; then
    SOT_TEST_PATH="$WINDOWS_TEST_PATH" "$ps" -NoProfile -NonInteractive -Command '$p=$env:SOT_TEST_PATH; if (Test-Path -LiteralPath $p) { Remove-Item -LiteralPath $p -Force -Recurse -ErrorAction SilentlyContinue }' >/dev/null 2>&1 || true
  fi
}
...
direct_generator=b0850879880eb921d3c72219e83674ae49f5ec87/generate-SOT-turn01-base19.py
...
echo '=== TURN 01 BASE-19 DIRECT CLEAN FULL QUALIFICATION ==='
...
declare -a URLS=(
 ...
 "$BASE/b0850879880eb921d3c72219e83674ae49f5ec87/generate-SOT-turn01-base19.py"
...
if python3 "$TMP/generate.py" "$TMP/base3.js" "$TMP/sot-api.js"; then pass GENERATE_BASE19 "bytes=$(stat -c %s "$TMP/sot-api.js") sha256=$(sha256sum "$TMP/sot-api.js"|awk '{print $1}')"; else fail GENERATE_BASE19 failed; fi
...
if grep -Fq '$args[0]' "$TMP/sot-api.js" || grep -Fq '$args[1]' "$TMP/sot-api.js"; then fail REJECTED_PS_ARGS 'rejected positional transport survived'; else pass REJECTED_PS_ARGS absent; fi
...
const api=require(process.argv[2]); const p=api._test.listProjects(); if(api.BUILD!=='2026.08.29.sot-turn01-base-19') throw Error('build='+api.BUILD); if(api.EXPECTED_MIGRATION!==4) throw Error('migration='+api.EXPECTED_MIGRATION); if(!Array.isArray(p)) throw Error('projects-not-array'); console.log(JSON.stringify({build:api.BUILD,migration:api.EXPECTED_MIGRATION,projects:p.length}));
NODE
...
# Throwaway exports exercise exact generated candidate helpers before any cutover.
...
NODE
  then pass "PRECUTOVER_DRIVE_${D}" "$(cat "$QUAL_DIR/precutover-drive-$D.json")"; else fail "PRECUTOVER_DRIVE_${D}" 'exact candidate Windows helper failed'; fi
...
# Exercise candidate Windows folder creation in the Windows user TEMP directory, then remove it.
...
TEST_NAME="sot-base19-qual-$STAMP"
...
NODE
then pass PRECUTOVER_FOLDER_CREATE "$(cat "$QUAL_DIR/create-folder.json")"; else fail PRECUTOVER_FOLDER_CREATE 'candidate folder creation failed'; fi
if SOT_TEST_PATH="$WINDOWS_TEST_PATH" "$POWERSHELL" -NoProfile -NonInteractive -Command '$p=$env:SOT_TEST_PATH; if (Test-Path -LiteralPath $p) { Remove-Item -LiteralPath $p -Force -Recurse -ErrorAction Stop }; if (Test-Path -LiteralPath $p) { exit 3 }'; then pass PRECUTOVER_FOLDER_CLEANUP "$WINDOWS_TEST_PATH removed"; WINDOWS_TEST_PATH=''; else fail PRECUTOVER_FOLDER_CLEANUP "$WINDOWS_TEST_PATH could not be removed"; fi
...
LIVE_ARCHIVE="$ARCHIVE_ROOT/$STAMP-turn01-base9-before-base19"
...
if live_meta="$(python3 -c "import json;x=json.load(open('$QUAL_DIR/live-health.json'));assert x.get('status')=='ok' and x.get('database_version')==4 and x.get('build')=='2026.08.29.sot-turn01-base-19',x;print('build='+x['build']+' schema='+str(x['database_version'])+' status='+x['status'])" 2>&1)"; then pass LIVE_HEALTH_CONTRACT "$live_meta"; else fail LIVE_HEALTH_CONTRACT "$live_meta"; fi
...
SUCCESS=1
pass QUALIFICATION 'Base-19 mechanically qualified'
echo '=== TURN 01 BASE-19 MECHANICALLY QUALIFIED ==='
