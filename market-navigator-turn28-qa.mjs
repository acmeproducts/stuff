...
const ARTIFACT='market-navigator-turn28-pre-ship.html';
const results=[];
...
(async()=>{
 ...
 try{
   ...
   await page.waitForFunction(()=>window.__mnShip25&&window.__mnShip25.ready()&&window.__mnTurn26&&window.__mnTurn27&&window.__mnTurn28,{timeout:30000});
   check('Turn28 corrective API present',await page.evaluate(()=>window.__mnTurn28.version==='turn28-crosshair-dual-axis-health-1'));
   check('Turn27 persistent API present',await page.evaluate(()=>window.__mnTurn27.version==='turn27-persistent-1'&&window.__mnTurn27.modelVersion()==='MN-PERSISTENT-1.0.0'));
   check('persistent evidence loaded',await page.evaluate(()=>window.__mnShip25.evidenceRevision()==='66b8487010768633'));
   check('interpretation default is Standard',await page.evaluate(()=>document.getElementById('analysisLevelDefault').value==='standard'&&window.__mnTurn27.interpretationLevel()==='standard'));
   check('three interpretation controls present',await page.locator('[data-interpret27]').count()===3);
   const topNav=await page.evaluate(()=>{let rail=document.getElementById('rail'),shell=document.getElementById('shell'),r=rail.getBoundingClientRect(),s=shell.getBoundingClientRect();return{rail:{left:r.left,top:r.top,right:r.right,bottom:r.bottom},shell:{left:s.left,top:s.top,right:s.right,bottom:s.bottom},width:innerWidth}});
   check('primary navigation is top-row not left rail',topNav.rail.left===0&&topNav.rail.right===topNav.width&&topNav.shell.left===0&&topNav.shell.top>=topNav.rail.bottom,JSON.stringify(topNav));
   ...
   check('native component values are distinct from governed signals',await page.evaluate(()=>{let r=window.__mnShip25.record('risk','1YR'),c=r.components[0];return Number.isFinite(c.baselineValue)&&Number.isFinite(c.endValue)&&Math.abs(c.endValue-c.orientedIndex)>1e-6}));
   check('fixed seven-component coefficients',await page.evaluate(()=>['risk','growth','macro'].every(k=>{let r=window.__mnShip25.record(k,'1YR');return r.componentsUsed===7&&r.components.every(c=>Math.abs(c.weight-1/7)<1e-12)&&/no reduced-set renormalization/.test(r.renormalizationRule)})));
   check('Turn26 API present',await page.evaluate(()=>window.__mnTurn26.version==='turn26-live-library-1'));
   // Turn 28 corrections: persistent/comparison dual axes, nearest-series
   ...
   for(const xf of [.2,.4,.6,.8])for(const yf of [.18,.36,.54,.72,.88]){
     const x=hover.x+hover.width*xf,y=hover.y+hover.height*yf;
     await page.mouse.move(x,y);await page.waitForTimeout(15);
     const name=(await page.locator('#nowTip strong').textContent().catch(()=>''))||'';
     if(name&&!hoverSeries.has(name))hoverSeries.set(name,{x,y});
   }
   check('crosshair hover follows nearest series',hoverSeries.size>=2,JSON.stringify([...hoverSeries.keys()]));
   const comparison=[...hoverSeries.entries()].find(([name])=>name!=='GRW');
 ...
 console.log('TURN28 QA PASS',results.length,'checks');
})().catch(e=>{console.error('TURN28 QA FAIL',e.stack||e);process.exit(1)});
