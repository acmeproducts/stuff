...
echo '=== DOWNLOAD + OFFLINE SYNTAX GATES ==='
FILES=(
...
echo '=== RECONSTRUCT CANDIDATE FROM VERIFIED 6.6 ==='
...
backend=['sot-backend-6.7-addon.js','sot-backend-6.7.1-hotfix.js','sot-backend-6.7.3-mount-safe-addon.js','sot-backend-6.8-multiproject-addon.js']
...
api.write_text(s)
frontend=['sot-ui-6.7-addon.js','sot-ui-6.7.1-hotfix.js','sot-ui-6.7.2-telemetry-hotfix.js','sot-ui-6.7.3-mount-safe-addon.js','sot-ui-6.8-multiproject-addon.js','sot-ui-6.8-nonblocking-hotfix.js']
marker='</body></html>'
...
echo '=== SCHEDULER + DB GATES ==='
curl -fsS http://127.0.0.1:18080/api/sot/scheduler/status -o "$TMP/scheduler.json"
python3 - "$TMP/scheduler.json" <<'PY'
import json,sys
x=json.load(open(sys.argv[1]))
if x.get('worker_pool')!=4 or len(x.get('workers') or [])!=4: raise SystemExit('scheduler shape failed: '+repr(x))
print('global workers:',x['worker_pool'],'queued projects:',len(x.get('queue') or []))
PY
...
if projects and projects[0].get('project_token'):
    t=projects[0]['project_token'];u='http://127.0.0.1:18080/api/sot/projects/'+urllib.parse.quote(t,safe='')+'/inventory/folders'
    y=json.loads(subprocess.check_output(['curl','-fsS',u],text=True))
    if 'folders' not in y: raise SystemExit('reporting folders shape failed')
    print('reporting folders gate:',len(y['folders']))
else: print('no projects present; route syntax covered')
PY
...
echo 'Worker model: 4 global workers shared across projects'
