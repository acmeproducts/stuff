...
BACKEND=Path('data/market-backend'); ROOT=Path('market-data'); SERIES_DIR=ROOT/'series'; INDEX_DIR=ROOT/'indexes'; VERSION='1.1.0'
...
CATEGORY={'risk':{'name':'Risk','components':['spy','wti']},'growth':{'name':'Growth','components':['qqq']},'macro':{'name':'Macro','components':['tenYear','wti']}}
def read(p,d=None):
...
def effect(sid,p):
 ...
 return clamp(p/8) if sid in ('spy','qqq') else -clamp(p/6) if sid=='tenYear' else -clamp(p/12) if sid=='wti' else None
...
def build_indexes(db):
 ...
 for key,cfg in CATEGORY.items():
  pts=category_history(cfg['components'],db);method='50 + 35 * mean(component effect); component effects use trailing 30-day percent changes'
  if key=='growth':method='Growth leadership proxy: QQQ trailing 30-day return mapped to 0–100; financing conditions are excluded from Growth and remain in Macro'
  obj={'schema':'market-navigator-index-v1','databaseVersion':VERSION,'id':key,'name':cfg['name'],'scale':{'min':0,'neutralBand':[42,58],'neutral':50,'max':100},'children':cfg['components'],'method':method,'observations':pts,'count':len(pts),'first':pts[0]['t'] if pts else None,'last':pts[-1]['t'] if pts else None};write(INDEX_DIR/f'{key}.json',obj);indexes[key]=obj
 pts=align_parent([indexes[k]['observations'] for k in ('risk','growth','macro')]);obj={'schema':'market-navigator-index-v1','databaseVersion':VERSION,'id':'regime','name':'Regime Sentiment Index','scale':{'min':0,'neutralBand':[42,58],'neutral':50,'max':100},'children':['risk','growth','macro'],'method':'arithmetic mean of persisted Risk, Growth, and Macro values at each date','observations':pts,'count':len(pts),'first':pts[0]['t'] if pts else None,'last':pts[-1]['t'] if pts else None};write(INDEX_DIR/'regime.json',obj);indexes['regime']=obj;return indexes
def validate(db,indexes):
 ...
 if indexes['growth']['children']!=['qqq']:errors.append('growth: financing conditions must not be an equal-weight Growth child')
...
def main():
 ...
 manifest={'schema':'market-navigator-database-v1','databaseVersion':VERSION,'generatedAt':dt.datetime.now(dt.timezone.utc).replace(microsecond=0).isoformat().replace('+00:00','Z'),'status':'ready' if not errors else 'invalid','marketSeriesReady':all(db[x]['count']>0 for x in ('spy','qqq','vix','tenYear','wti','brent','gold','dxy')),'fredMacroReady':all(db[x]['count']>0 for x in ('cpi','fedFunds','twoYear','thirtyYear')),'series':{k:{'count':v['count'],'first':v['first'],'last':v['last'],'provider':v['provider'],'status':v['status']} for k,v in db.items()},'indexes':{k:{'count':v['count'],'first':v['first'],'last':v['last'],'children':v['children'],'method':v['method']} for k,v in indexes.items()},'files':files,'validation':{'ok':not errors,'errors':errors},'contract':{'parentChild':'Regime = mean(Risk, Growth, Macro); each category is derived only from its declared component series','growth':'Growth measures growth-asset leadership; cost-of-capital/rate pressure belongs to Macro/financial conditions','stateDirectionContribution':'State classification, direction of travel, and child contribution are separate display concepts','neutralBand':[42,58],'uiReadsPersistedDataOnly':True,'allowSyntheticHistory':False,'displayCurrentEqualsLastPlottedPoint':True,'indexChangeUnit':'points','componentChangeUnit':'percent'}};manifest['revision']=hashlib.sha256(json.dumps(manifest,sort_keys=True,separators=(',',':')).encode()).hexdigest()[:16];write(ROOT/'manifest.json',manifest)
 if errors:raise SystemExit('Database validation failed: '+'; '.join(errors))
 print(json.dumps({'ok':True,'revision':manifest['revision'],'marketSeriesReady':manifest['marketSeriesReady'],'fredMacroReady':manifest['fredMacroReady'],'growthChildren':indexes['growth']['children'],'regimeObservations':indexes['regime']['count']},indent=2))
