...
if len(sys.argv) != 3:
    raise SystemExit('usage: patch-SOT-turn01-base-idle-refresh.py <base.html> <output.html>')

src = Path(sys.argv[1]).read_text()

src = src.replace(
    'Target and Backup are selected from storage volumes currently visible to WSL. No typed path is accepted.',
    'Target and Backup are selected from storage volumes discovered from Windows plus real Windows-backed WSL mounts. No typed path is accepted.',
    1
)
src = src.replace(
    "openModal(`${kind==='target'?'Target':'Backup'} — choose actual WSL storage`",
    "openModal(`${kind==='target'?'Target':'Backup'} — choose available storage`",
    1
)
src = src.replace(
    "No volumes currently available to WSL",
    "No Windows/WSL storage volumes are currently available",
    1
)
src = src.replace(
    "No storage volumes are currently visible to WSL.",
    "No Windows/WSL storage volumes are currently available.",
    1
)
...
required = [
    'function fullyIndexedStable(p)',
    "if(active){let p=await api('/projects')",
    "state.tab==='index'&&!fullyIndexedStable(psel)",
    'Windows plus real Windows-backed WSL mounts',
]
for marker in required:
    if marker not in src:
        raise SystemExit(f'idle refresh marker missing: {marker}')
