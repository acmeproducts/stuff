...
- Ribbon dividers are visible rules, and the boundaries around the source-group picker between the dimension selectors and the filter triggers are drawn heavier still.
- A source group is a named set of enabled sources, stored in `prism_source_groups_v1`. Applying one disables every source outside it. Groups are created from the currently enabled sources and can be renamed, updated and deleted. Seeded groups cover the shipped feed inventory only; groups for subjects the inventory does not carry are the owner's to create once matching sources exist.
