...
2. The Library has its own left rail for the Analysis cards (title, count, Omnisearch, cards), separate from the navigation drawer, with its own hamburger that behaves exactly like the navigation one: pinned at the top-left of the Library workspace, visible open or closed, it toggles an overlay rail that has zero width when closed so the workspace takes the whole screen. Scrim, Escape and choosing a card close it. There is no other collapse control and no `+` (2026-09-22). Closing must never destroy the rail state or Analysis selection.
...
## Owner amendment: Map ergonomics and the OG skin — 2026-09-20
- The surface selector gains `OG`, a third mode of the same Map surface. It is a skin, not a product: same corpus, filters, dimensions, selection, reader and Analysis handoff, rendered in the classic NewsMap visual language (hue by subject, lightness by recency, headline-dominant tiles, staggered reveal). It is an independent implementation of that look, not a copy of any third-party source. It is not a third top-level tab and does not alter the two-tab product architecture.
- Default dimensions are Region (group), Source (color) and Coverage momentum (size).
- The matching-story count sits immediately left of the omnisearch. It reads `N items` when nothing narrows the corpus and `M/N items` when the search, the time window, the enabled sources or an active filter do. The status line continues to report the whole corpus, so the two numbers are no longer mistaken for each other.
- Filter triggers sit on the dimension row, separated from the dimension selectors by a divider. The row below carries a legend of the active values for each of the three dimensions, one horizontally scrollable lane per dimension, each capped at one third of the width and ceding unused space. Every legend chip carries an `x` that excludes that value; adding values back remains the filter panel's job.
- Hovering a tile opens a card with the headline, its dimension chips, the summary and every eligible source as a direct link, so coverage is reachable without opening the reader. The card stays open while the pointer is inside it.
- Focusing a group replaces the small group badge with a full-width label at least 40px tall that states the focused group and that tapping it returns to all groups.
...
- Navigation is a left drawer that overlays the content. Closed, it has zero width: nothing but a hamburger toggle remains, fixed in the top-left corner, visible whether the drawer is open or closed, and the whole viewport belongs to the content. Open, it lists NewsMap, OG, Feed and Library, with Settings (the gear) at the bottom. The Library's Analysis rail is a second rail of the same kind with its own hamburger (see the Library contract, 2026-09-22). Choosing any entry closes the drawer. Tapping outside it or pressing Escape closes it.
- The header is one row: the omnisearch, the `#` group-tag control, and the results badge inside the search field. The badge reads `N` when nothing narrows the corpus and `M/N` when the search, the enabled sources or an active filter do. `N` is the seven-day corpus, not everything held in memory: stories older than the horizon can never be shown, so they count in neither number (corrected 2026-09-21). The presence indicator, build stamp, product tabs, surface dropdown and window dropdown are gone from the header; the presence line and build stamp live in Settings → Diagnostics.
- The ribbon is three rows, so the shell is exactly four rows above the content (corrected 2026-09-21, same day). Row two carries the three dimension dropdowns under small-caps captions `GROUP`, `COLOR`, `SIZE`; the captions are labels only. Row three is the filter row exactly as in the pre-ship build: one trigger per dimension, reading the dimension name over its current filter summary (`All`, the single value, or `n of N`); tapping it opens the same filter drawer as before, where values are toggled on and off exactly as before. Row four is the colour dimension's active values as chips, scrolling sideways when they do not fit. Tapping a chip's body zooms in to that value alone, the way tapping a group label does, and the map redraws; its `×` removes that value from the filter. While zoomed, row four is a single full-width label in the focused-group style, `✕` first, that zooms back out to exactly the filter in force before the zoom (the same three sources, say), not to all values (corrected 2026-09-21).
...
- Nothing beyond the controls named here is added to the shell. The legend lanes, ribbon dividers and ribbon Analyze described in the 2026-09-20 amendment are superseded by rows two to four above; the filter-trigger row survives as row three.
