...
2. The Analysis rail (title, count, Omnisearch, cards) lives inside the one navigation drawer and appears there whenever the Library is the view. The hamburger fixed in the top-left corner is its only toggle: open, the drawer shows the navigation and the Analysis rail; closed, it has zero width and the workspace takes the whole screen. There is no second collapse control and no `+` (2026-09-22). Choosing a card closes the drawer and shows that Analysis; closing must never destroy the rail state or Analysis selection.
