...
BASE=https://raw.githubusercontent.com/acmeproducts/stuff/52f6f6d17804bbe185b8d7361dc8b95088d1caf3
PUBLIC_URL='https://oc-ref.fell-dojo.ts.net/report/SOT/sot-turn01-r2-wizard.html?v=20260823-project-controls-3'
EXPECTED_BUILD=2026.08.23.sot-project-controls-3
TEMP_ROOT="$(mktemp -d)"
CUTOVER_STARTED=0
SERVICE_STOPPED=0
...
markers=['2026.08.23.sot-project-controls-3','2026.08.23.sot-clean-admin-1']
...
for attempt in {1..10}; do
  if curl --max-time 15 -fsS "$PUBLIC_URL" -o "$TEMP_ROOT/public.html" && \
     grep -q '2026.08.23.sot-project-controls-3' "$TEMP_ROOT/public.html"; then
    break
  fi
  sleep 2
done
grep -q '2026.08.23.sot-project-controls-3' "$TEMP_ROOT/public.html"
grep -q 'Folder Count' "$TEMP_ROOT/public.html"
grep -q 'Add entire volume' "$TEMP_ROOT/public.html"
grep -q 'data-run-control="toggle"' "$TEMP_ROOT/public.html"
grep -q 'data-run-control="stop"' "$TEMP_ROOT/public.html"

CUTOVER_STARTED=0
echo '=== SOT PROJECT CONTROLS 3 INSTALLED ==='
