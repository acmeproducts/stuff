...
const sourceRegistryFixture=JSON.parse(readFileSync(new URL('./data/market-backend/source-registry.json',import.meta.url)));
...
async function makePage({width=1280,height=800,registry=null,failAI=false,android=false}={}){
  ...
  await page.addInitScript(({registry})=>{
    ...
    window.__qaSpeech=speech;window.__opened=[];window.clean=s=>String(s||'').replace(/\s+/g,' ').trim();window.open=(u)=>{window.__opened.push(String(u));return null};
...
try{
  ...
  const cred=await makePage({width:412,height:915,registry:reg,failAI:true}),cp=cred.page;await cp.locator('#settingsGear').click();let key=cp.locator('#openrouterKey'),replace=cp.locator('[data-replace-key="openrouter"]');assert.equal(await key.inputValue(),'');assert.equal(await key.isHidden(),true,'registered key field hidden');let replaceState=await replace.evaluate(el=>({visible:!!(el.offsetWidth||el.offsetHeight||el.getClientRects().length),hidden:el.hidden,cls:el.className,parent:el.parentElement?.className||'',panel:document.querySelector('#cfgAi')?.className||''}));assert.equal(replaceState.visible,true,`Replace key explicit ${JSON.stringify(replaceState)}`);await replace.click();assert.equal(await key.isVisible(),true);await key.fill('bad-replacement');await cp.locator('#openrouterValidate').click();await cp.waitForFunction(()=>document.querySelector('#openrouterStatus')?.textContent.includes('QA provider failure'));let kept=await cp.evaluate(()=>JSON.parse(localStorage.getItem('marketNavigatorAIRegistryV1')).providers.openrouter);assert.equal(kept.key,'qa-key');assert.equal(kept.verified,true);await cred.context.close();

  const src=await makePage({width:412,height:915}),sp=src.page;await sp.locator('#settingsGear').click();await sp.locator('[data-cfgtab="sources"]').click();await sp.waitForFunction(()=>document.querySelector('#sourceStatus')?.textContent.includes('Canonical registry refreshed'));assert((await sp.locator('#sourceList [data-source-id]').count())>=3,'canonical registrations listed');assert.equal(await sp.locator('[data-source-id="custom_nvda"]').getAttribute('data-source-state'),'Active','completed NVDA promoted out of Processing');assert.match(await sp.locator('[data-source-id="custom_nvda"]').innerText(),/observations: 2514.*provider: Yahoo Finance.*fallback: false.*enabled horizons:.*5D.*last success:/s,'NVDA operational qualification detail');assert.equal(await sp.locator('[data-source-id="custom_gaamhx"]').getAttribute('data-source-state'),'Active');assert.match(await sp.locator('[data-source-id="custom_gaamhx"]').innerText(),/enabled horizons: 5D MTD/,'GAAMHX shows evidence-driven horizons');assert.doesNotMatch(await sp.locator('[data-source-id="custom_nvda"]').innerText(),/registered|pending/i,'completed source not stranded');await sp.locator('#sourceQuery').fill('Dow');await sp.locator('#sourceClass').selectOption('index');await sp.locator('#sourceResolve').click();assert.match(await sp.locator('#sourceResolution').innerText(),/Dow Jones Industrial Average/i);await sp.locator('#sourceRegister').click();let opened=await sp.evaluate(()=>window.__opened.at(-1)||'');assert(opened.includes('github.com/acmeproducts/stuff/issues/new'),'repository control-plane handoff');await sp.locator('#configClose').click();await enter(sp,'risk');await sp.locator('#nowAddSeries').click();await sp.locator('#nowPickerSearch').fill('NVDA');await sp.waitForFunction(()=>document.querySelector('[data-add-now="custom_nvda"]'));assert.equal(await sp.locator('[data-add-now="custom_nvda"]').isDisabled(),false,'Active qualified custom source chartable at 5D');await sp.locator('#nowPickerClose').click();let failedRegistry=structuredClone(sourceRegistryFixture),failedV=failedRegistry.registrations.find(x=>x.id==='custom_v');failedV.status='failed';failedV.failureReason='QA canonical evidence did not qualify';await sp.route('**/data/market-backend/source-registry.json',r=>r.fulfill({contentType:'application/json',body:JSON.stringify(failedRegistry)}));await sp.locator('#settingsGear').click();await sp.locator('[data-cfgtab="sources"]').click();await sp.locator('#sourceRefresh').click();await sp.waitForFunction(()=>document.querySelector('[data-source-id="custom_v"]')?.dataset.sourceState==='Failed');await sp.locator('#configClose').click();await sp.locator('#nowAddSeries').click();await sp.locator('#nowPickerSearch').fill('Visa');await sp.waitForFunction(()=>document.querySelector('[data-add-now="custom_v"]'));assert.equal(await sp.locator('[data-add-now="custom_v"]').isDisabled(),true,'Failed custom source not chartable');assert.equal(await sp.locator('[data-source-id="custom_v"]').getAttribute('data-source-state'),'Failed');await sp.locator('#nowPickerClose').click();await sp.locator('#hzs [data-h="1D"]').click();await sp.waitForFunction(()=>document.querySelector('#hzs [data-h="1D"]')?.classList.contains('on'));await sp.locator('#nowAddSeries').click();await sp.locator('#nowPickerSearch').fill('NVDA');await sp.waitForFunction(()=>document.querySelector('[data-add-now="custom_nvda"]'));assert.equal(await sp.locator('[data-add-now="custom_nvda"]').isDisabled(),true,'daily source cannot fabricate 1D intraday');assert.deepEqual(src.errors,[]);assert.deepEqual(src.failed,[]);await src.context.close();
