...
EXPECTED_BUILD='2026.08.29.sot-turn01-base-13'
PUBLIC_URL='https://oc-ref.fell-dojo.ts.net/report/SOT/SOT-turn01-base.html'
RUNTIME_BACKUP="$TMP/sot-api.before-base13.js"
HTML_BACKUP="$TMP/base.before-base13.html"
HAD_HTML=0
CUTOVER=0
SUCCESS=0
...
WINDOWS_BROWSE_PATCH_URL='https://raw.githubusercontent.com/acmeproducts/stuff/8686ecffd4ebb3f7a2615f781d6b9194d25be5f2/patch-SOT-turn01-base-windows-browse.py'
...
cleanup(){
  if [ "$CUTOVER" -eq 1 ] && [ "$SUCCESS" -ne 1 ]; then
    echo
    echo '=== BASE-13 GATE FAILED — AUTOMATIC ROLLBACK ==='
    sudo systemctl stop "$SERVICE" || true
    [ -f "$RUNTIME_BACKUP" ] && install -m 0644 "$RUNTIME_BACKUP" "$REPORT_ROOT/sot-api.js"
    if [ "$HAD_HTML" -eq 1 ] && [ -f "$HTML_BACKUP" ]; then install -m 0644 "$HTML_BACKUP" "$SOT_DIR/SOT-turn01-base.html"; else rm -f "$SOT_DIR/SOT-turn01-base.html"; fi
...
echo '=== TURN 01 BASE-13 ROLLBACK-FIRST CLEAN REBUILD ==='

# Owner rejected Base-12. Restore the accepted pre-Base12 live state before any new cutover.
...
if [ "$CURRENT_BUILD" = '2026.08.29.sot-turn01-base-12' ]; then
  ...
  [ -f "$ACCEPTED_ARCHIVE/sot-api.js" ] || { echo 'Accepted rollback archive is incomplete'; false; }
  echo "restoring accepted live state before rebuild: $ACCEPTED_ARCHIVE"
  ...
  echo -n 'rollback health: '; cat "$TMP/rollback-health.json"; echo
fi

curl --max-time 30 -fsSL "$BACKEND_PREBASE_URL" -o "$TMP/pre.js"
curl --max-time 30 -fsSL "$BACKEND_INTEGRATOR_URL" -o "$TMP/integrate.py"
curl --max-time 30 -fsSL "$WINDOWS_BROWSE_PATCH_URL" -o "$TMP/windows-browse.py"
...
python3 -m py_compile "$TMP/integrate.py" "$TMP/windows-browse.py" "$TMP/ui-integrate.py" "$TMP/ui-state.py" "$TMP/idle.py"
python3 "$TMP/integrate.py" "$TMP/pre.js" "$TMP/base3.js"
python3 "$TMP/windows-browse.py" "$TMP/base3.js" "$TMP/sot-api.js"
...
for marker in ["const BUILD = '2026.08.29.sot-turn01-base-13';",'function windowsListDirectories(value)','function windowsDirectoryExists(value)','target_browse_root:','backup_browse_root:','windowsListDirectories(requested)']:
...
if(api.BUILD!=='2026.08.29.sot-turn01-base-13') throw new Error('wrong candidate build');
console.log('temp DB/API preflight: ok; projects:',projects.length,'build:',api.BUILD);
NODE

echo '=== ARCHIVE ACCEPTED LIVE STATE BEFORE BASE-13 CUTOVER ==='
LIVE_ARCHIVE="$ARCHIVE_ROOT/$STAMP-turn01-accepted-before-base13"
...
printf '%s\n' 'Accepted live state immediately before governed Base-13 cutover.' > "$LIVE_ARCHIVE/ARCHIVE-MANIFEST.txt"
...
import json,sys
x=json.load(open(sys.argv[1])); assert x.get('status')=='ok',x; assert x.get('database_version')==4,x; assert x.get('build')=='2026.08.29.sot-turn01-base-13',x
...
v=json.load(open(sys.argv[2])); assert v.get('build')=='2026.08.29.sot-turn01-base-13',v
...
echo '=== LIVE PICKER STATE PERSISTENCE GATE ==='
...
import json,sys
x=json.load(open(sys.argv[1])); p=x.get('projects') or []
...
assert vols,'no volume available for picker persistence gate'
probe=vols[0]['path']
...
curl --max-time 10 -fsS -X PUT -H 'Content-Type: application/json' --data "{\"target_browse_root\":\"$PROBE\"}" "http://127.0.0.1:18080/api/sot/turn01/projects/$TOKEN/storage" -o "$TMP/storage-put.json"
...
SUCCESS=1
echo '=== TURN 01 BASE-13 MECHANICALLY QUALIFIED ==='
