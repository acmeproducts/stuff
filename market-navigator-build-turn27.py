#!/usr/bin/env python3
"""Build Turn 27 only from the accepted immutable Turn 26 artifact."""
from pathlib import Path
import hashlib

SRC=Path("market-navigator-turn26-ship.html")
OUT=Path("market-navigator-turn27-pre-ship.html")
...
def replace_once(text,old,new,label):
    ...
    if count!=1:
        raise SystemExit(f"Turn 27 build blocked: {label} expected once, found {count}")
...
def main():
    ...
    if len(raw)!=EXPECTED_BYTES or git_blob(raw)!=EXPECTED_GIT_BLOB:
        raise SystemExit("Turn 27 build blocked: accepted Turn 26 baseline blob/size mismatch")
    s=raw.decode()
...
"""
    s=replace_once(s,"</style></head>",css+"</style></head>","Turn 27 CSS")
...
"""
    ...
    s=replace_once(s,'<div id="turn26BuildMarker" class="hidden" data-build="2026.09.21.turn26-standalone-analysis-context-print"></div>',
        ...
        "Turn 27 build marker")
    s=s.replace('Market Navigator · Turn 25 Ship','Market Navigator · Turn 27 Persistent Candidate')
    OUT.write_text(s)
