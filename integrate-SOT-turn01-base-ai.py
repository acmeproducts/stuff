...
if len(sys.argv)!=3:
    raise SystemExit('usage: integrate-SOT-turn01-base-ai.py <clean-base24-behavior.html> <base.html>')
...
'''
# Insert at the complete declaration boundary. Searching for the inner
# ``function`` token in ``async function openConfig`` leaves a standalone
# ``async`` statement behind and is a runtime ReferenceError even though the
# resulting script remains syntactically valid.
insert=span(src,'openConfig')[0]
...
new="load().then(function(){var cards=document.getElementById('cards'),content=document.getElementById('content');if(!cards||!content||cards.querySelector('.errorBox')||content.querySelector('.errorBox'))throw new Error('initial application render failed');document.documentElement.setAttribute('data-sot-boot','base28-ok')}).catch(function(e){document.documentElement.setAttribute('data-sot-boot-error','initial-load: '+String(e&&e.message?e.message:e))});setInterval(async()=>{try{let [p,r]=await Promise.all([api('/projects'),api('/rollup')]);state.projects=Array.isArray(p)?p:(p.projects||[]);state.rollup=r;renderCards();let psel=selectedProject();if(psel&&state.tab==='index'&&['Queued','WIP','Paused'].includes(projectState(psel)))renderIndex(psel,true)}catch{}},3000);"
...
if re.search(r'(?m)^[ \t]*async[ \t]*(?:;)?[ \t]*$',src):
    raise SystemExit('standalone async runtime hazard generated')
if re.search(r'(?m)^[ \t]*async[ \t]*\r?\n',src):
    raise SystemExit('async line-terminator runtime hazard generated')
for name in ['openConfig','renderInsights','aiLoadModels','aiValidate','aiProjectContext','aiInfer']:
    span(src,name)
Path(sys.argv[2]).write_text(src)
print('Canonical Base storage defaults + operational AI + browser boot integrated from clean Base-24 behavior')
