...
if src.count(old)!=1: raise SystemExit(f'coordination UI refresh marker changed: {src.count(old)}')
...
load();setInterval(async()=>{try{let [p,r]=await Promise.all([api('/projects'),api('/rollup')]),next=Array.isArray(p)?p:(p.projects||[]),changed=projectDisplaySignature(next)!==projectDisplaySignature(state.projects),rollChanged=rollupDisplaySignature(r)!==rollupDisplaySignature(state.rollup);state.projects=next;state.rollup=r;if(!sotOperatorBusy()&&(changed||rollChanged)){let y=window.scrollY;renderCards();requestAnimationFrame(()=>window.scrollTo(0,y));let psel=selectedProject();if(psel&&state.tab==='index')renderIndex(psel)}}catch{}},3000);'''
