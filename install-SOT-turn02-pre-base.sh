...
RUN="$SOT_DIR/archive/$STAMP-turn02-pre-base-reset"
LOG="$RUN/reset.log"
...
cleanup(){
  ...
  if [ "$CUTOVER" -eq 1 ] && [ "$SUCCESS" -ne 1 ]; then
    ...
    rm -f "$SOT_DIR/SOT-turn02-pre-base.html"
    sudo systemctl start "$SERVICE" >/dev/null 2>&1 || true
    record PASS ROLLBACK 'restored archived Turn 01 database; removed Turn 02 UI'
  fi
...
# Archive before any destructive change.
...
# Fetch pinned DB manager, migrations, and standalone Turn 02 UI.
curl --retry 5 --retry-all-errors -fsSL "$RAW/$BASE/sot-db-manage.js" -o "$TMP/sot-db-manage.js"
for m in 001-initial.sql 002-project-list-metrics.sql 003-project-run-controls.sql 004-live-byte-progress.sql 005-project-coordination.sql 006-ssot-profile-tags.sql; do
  curl --retry 5 --retry-all-errors -fsSL "$RAW/$BASE/sot-db/migrations/$m" -o "$TMP/migrations/$m"
done
curl --retry 5 --retry-all-errors -fsSL "$RAW/$UI/SOT-turn02-pre-base.html" -o "$TMP/SOT-turn02-pre-base.html"

# Developer pass: build a completely fresh managed DB and validate the standalone UI.
...
for t in projects sources processing_runs content observations current_observations target_holdings backup_holdings plans plan_items actions certifications events tags tag_assignments; do
  n="$(sqlite3 "$TMP/fresh.sqlite" "select count(*) from $t")"
  [ "$n" = 0 ] || fail DEV_EMPTY "$t=$n"
done
...
if(api.EXPECTED_MIGRATION!==6) throw Error('backend migration contract is not 6');
const t=api._test;
const p=t.ssotProfile('',100);
if(p.model!=='ssot-profile-v1') throw Error('profile contract');
if(!Array.isArray(p.items)||p.items.length!==0) throw Error('fresh profile is not empty');
const d=t.discoverSources();
if(!Array.isArray(d.sources)||d.sources.length!==0) throw Error('fresh discover is not empty');
console.log('fresh backend fixture pass');
NODE
...
for x in ["SOT-turn02-pre-base-simple-mobile",'>Discover</button>','>Profile</button>','>Action</button>','Manual refresh only','Type tag and press Enter','No copy, delete, reconcile, or bulk actions']:
    assert x in h,x
for x in ['setInterval(', '>Dashboard</button>', '>Database</button>', '>Activity</button>', 'bulk delete']:
    assert x not in h,x
...
# Manager pass: enforce deliberately small pre-base scope.
...
! grep -Eqi 'project|dashboard|scheduler|bulk action|setInterval' "$TMP/SOT-turn02-pre-base.html" || fail MANAGER_SCOPE 'rejected complexity leaked into Turn 02 UI'
...
# Red-team pass before destructive cutover.
...
# Destructive reset only after Developer → Manager → Red-team PASS.
...
for endpoint in 'turn01/profile?limit=20' 'turn01/discover' 'turn01/tags'; do
  code="$(curl --max-time 10 -sS -o /dev/null -w '%{http_code}' "http://127.0.0.1:18080/api/sot/$endpoint" || true)"
