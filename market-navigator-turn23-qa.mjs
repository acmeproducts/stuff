...
async function railGeometry(width,height){
  ...
  let a=await geom();assert(Math.abs(a.card.t-a.view.t)<=2,'initial rail state chart pinned to workspace top');assert(Math.abs(a.card.b-a.view.b)<=2,'initial rail state chart pinned to workspace bottom');
